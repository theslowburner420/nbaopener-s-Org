import { Card, Rarity } from '../types';
import { AWARD_CARDS } from './awardCards';
import { DUO_AND_XFACTOR_CARDS } from './duoAndXFactorCards';
import { DYNASTY_CARDS } from './dynastyCards';
import { ALL_STAR_MVP_CARDS } from './allStarMvpCards';
import { SCREAM_CARDS } from './screamCards';
import { HIDDEN_GEMS_CARDS } from './hiddenGemsCards';

const teamColors: Record<string, string> = {
  'Los Angeles Lakers': '#FDB927',
  'Golden State Warriors': '#1D428A',
  'Boston Celtics': '#007A33',
  'Chicago Bulls': '#CE1141',
  'Miami Heat': '#98002E',
  'Cleveland Cavaliers': '#860038',
  'Oklahoma City Thunder': '#007AC1',
  'New York Knicks': '#006BB6',
  'San Antonio Spurs': '#C4CED4',
  'Detroit Pistons': '#1D428A',
  'Houston Rockets': '#CE1141',
  'Denver Nuggets': '#0E2240',
  'Charlotte Hornets': '#1D1160',
  'New Orleans Pelicans': '#0C2340',
  'Memphis Grizzlies': '#5D76A9',
  'Sacramento Kings': '#5A2D81',
  'Dallas Mavericks': '#00538C',
  'Utah Jazz': '#002B5C',
  'Orlando Magic': '#0077C0',
  'Minnesota Timberwolves': '#0C2340',
  'LA Clippers': '#C8102E',
  'Washington Wizards': '#002B5C',
  'Brooklyn Nets': '#000000',
  'Indiana Pacers': '#002D62',
  'Phoenix Suns': '#E56020',
  'Portland Trail Blazers': '#E03A3E',
  'Milwaukee Bucks': '#00471B',
  'Toronto Raptors': '#CE1141',
  'Philadelphia 76ers': '#006BB6',
  'Atlanta Hawks': '#E03A3E',
  'Seattle SuperSonics': '#00653A',
  'New Orleans Hornets': '#00848E',
};

const teamLogos: Record<string, string> = {
  'Brooklyn Nets': 'https://cdn.nba.com/logos/nba/1610612751/global/L/logo.svg',
  'Washington Wizards': 'https://cdn.nba.com/logos/nba/1610612764/global/L/logo.svg',
  'Memphis Grizzlies': 'https://cdn.nba.com/logos/nba/1610612763/global/L/logo.svg',
  'Cleveland Cavaliers': 'https://cdn.nba.com/logos/nba/1610612739/global/L/logo.svg',
  'Indiana Pacers': 'https://cdn.nba.com/logos/nba/1610612754/global/L/logo.svg',
  'Phoenix Suns': 'https://cdn.nba.com/logos/nba/1610612756/global/L/logo.svg',
  'Portland Trail Blazers': 'https://cdn.nba.com/logos/nba/1610612757/global/L/logo.svg',
  'Milwaukee Bucks': 'https://cdn.nba.com/logos/nba/1610612749/global/L/logo.svg',
  'Philadelphia 76ers': 'https://cdn.nba.com/logos/nba/1610612755/global/L/logo.svg',
  'Toronto Raptors': 'https://cdn.nba.com/logos/nba/1610612761/global/L/logo.svg',
  'Miami Heat': 'https://cdn.nba.com/logos/nba/1610612748/global/L/logo.svg',
  'Detroit Pistons': 'https://cdn.nba.com/logos/nba/1610612765/global/L/logo.svg',
  'San Antonio Spurs': 'https://cdn.nba.com/logos/nba/1610612759/global/L/logo.svg',
  'Orlando Magic': 'https://cdn.nba.com/logos/nba/1610612753/global/L/logo.svg',
  'New York Knicks': 'https://cdn.nba.com/logos/nba/1610612752/global/L/logo.svg',
  'Chicago Bulls': 'https://cdn.nba.com/logos/nba/1610612741/global/L/logo.svg',
  'Utah Jazz': 'https://cdn.nba.com/logos/nba/1610612762/global/L/logo.svg',
  'Golden State Warriors': 'https://cdn.nba.com/logos/nba/1610612744/global/L/logo.svg',
  'Boston Celtics': 'https://cdn.nba.com/logos/nba/1610612738/global/L/logo.svg',
  'Los Angeles Lakers': 'https://cdn.nba.com/logos/nba/1610612747/global/L/logo.svg',
  'Dallas Mavericks': 'https://cdn.nba.com/logos/nba/1610612742/global/L/logo.svg',
  'Atlanta Hawks': 'https://cdn.nba.com/logos/nba/1610612737/global/L/logo.svg',
  'Seattle SuperSonics': 'https://cdn.nba.com/logos/nba/1610612760/global/L/logo.svg',
  'New Orleans Hornets': 'https://cdn.nba.com/logos/nba/1610612740/global/L/logo.svg',
};

const teamCoaches: Record<string, string> = {
  'Atlanta Hawks': 'Quin Snyder',
  'Boston Celtics': 'Joe Mazzulla',
  'Brooklyn Nets': 'Jordi Fernández',
  'Charlotte Hornets': 'Charles Lee',
  'Chicago Bulls': 'Tiago Splitter',
  'Cleveland Cavaliers': 'Kenny Atkinson',
  'Dallas Mavericks': 'Dusty May',
  'Denver Nuggets': 'David Adelman',
  'Detroit Pistons': 'J.B. Bickerstaff',
  'Golden State Warriors': 'Steve Kerr',
  'Houston Rockets': 'Ime Udoka',
  'Indiana Pacers': 'Rick Carlisle',
  'LA Clippers': 'Tyronn Lue',
  'Los Angeles Lakers': 'JJ Redick',
  'Memphis Grizzlies': 'Tuomas Iisalo',
  'Miami Heat': 'Erik Spoelstra',
  'Milwaukee Bucks': 'Taylor Jenkins',
  'Minnesota Timberwolves': 'Chris Finch',
  'New Orleans Pelicans': 'Jamahl Mosley',
  'New York Knicks': 'Mike Brown',
  'Oklahoma City Thunder': 'Mark Daigneault',
  'Orlando Magic': 'Sean Sweeney',
  'Philadelphia 76ers': 'Nick Nurse',
  'Phoenix Suns': 'Jordan Ott',
  'Portland Trail Blazers': 'Micah Nori',
  'Sacramento Kings': 'Doug Christie',
  'San Antonio Spurs': 'Mitch Johnson',
  'Toronto Raptors': 'Darko Rajaković',
  'Utah Jazz': 'Will Hardy',
  'Washington Wizards': 'Brian Keefe',
};

const rawCards = [
  {
    id: "atl-001",
    name: "Jalen Johnson",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 22.5,
    reb: 10.3,
    ast: 7.9,
    age: 24,
    nbaId: 1630552,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630552.png",
    quote: "Atlanta's versatile forward doing it all on both ends."
  },
  {
    id: "atl-002",
    name: "Nickeil Alexander-Walker",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 20.8,
    reb: 3.4,
    ast: 3.7,
    age: 27,
    nbaId: 1629638,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629638.png",
    quote: "Dynamic scoring and improved playmaking in the Atlanta rotation."
  },
  {
    id: "atl-003",
    name: "CJ McCollum",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 18.2,
    reb: 3.4,
    ast: 4,
    age: 34,
    nbaId: 203468,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203468.png",
    quote: "Smooth veteran scoring and leadership for the Hawks backcourt."
  },
  {
    id: "atl-004",
    name: "Jonathan Kuminga",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.2,
    reb: 3.6,
    ast: 0.6,
    age: 23,
    nbaId: 1630228,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630228.png",
    quote: "Elite athleticism and rim pressure for the new-look Hawks."
  },
  {
    id: "atl-005",
    name: "Dyson Daniels",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G-F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 14.1,
    reb: 5.9,
    ast: 3.8,
    age: 22,
    nbaId: 1630713,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSKukEuD-lfVYh42bKX63QvOfdGnP2qnKE289PFEM8khA&s=10",
    quote: "Stat-sheet stuffer with elite defensive versatility."
  },
  {
    id: "atl-006",
    name: "Onyeka Okongwu",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.6,
    reb: 8.2,
    ast: 1.7,
    age: 25,
    nbaId: 1630168,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630168.png",
    quote: "Mobile big man protecting the paint and finishing strong."
  },
  {
    id: "atl-007",
    name: "Buddy Hield",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 3.1,
    ast: 2.3,
    age: 33,
    nbaId: 1627741,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627741.png",
    quote: "One of the game's premier shooters spacing the floor."
  },
  {
    id: "atl-008",
    name: "Corey Kispert",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.5,
    reb: 3,
    ast: 1.6,
    age: 26,
    nbaId: 1630557,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630557.png",
    quote: "Sharpshooting wing stretching the defense."
  },
  {
    id: "atl-009",
    name: "Luguentz Dort",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.7,
    reb: 3.7,
    ast: 1.8,
    age: 25,
    nbaId: 1629652,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629652.png",
    quote: "Shutdown defender from Montreal."
  },
  {
    id: "atl-010",
    name: "Aaron Wiggins",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 3.6,
    ast: 1.9,
    age: 25,
    nbaId: 1630598,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630598.png",
    quote: "Reliable vet off the bench."
  },
  {
    id: "atl-011",
    name: "Asa Newell",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "PF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.9,
    reb: 5.1,
    ast: 1.2,
    age: 20,
    nbaId: 1642235,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQkMwjbT9QLselMQIf4EhFIM93FEMaQ0_dHOdgu2svJKg&s=10",
    quote: "Highly-touted rookie forward ready to make an impact."
  },
  {
    id: "atl-012",
    name: "Gabe Vincent",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5.2,
    reb: 0.6,
    ast: 1.4,
    age: 29,
    nbaId: 1629216,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629216.png",
    quote: "Hard-nosed guard bringing toughness and outside shooting."
  },
  {
    id: "atl-013",
    name: "Tony Bradley",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 2,
    reb: 2.7,
    ast: 0.7,
    age: 27,
    nbaId: 1628396,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4065673.png",
    quote: "Reliable backup center providing minutes in the paint."
  },
  {
    id: "atl-014",
    name: "Mouhamed Gueye",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 1.6,
    reb: 1.6,
    ast: 0.4,
    age: 23,
    nbaId: 1631243,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631243.png",
    quote: "Long and athletic forward with high upside."
  },
  {
    id: "bos-001",
    name: "Jayson Tatum",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 21.8,
    reb: 10,
    ast: 5.3,
    age: 27,
    nbaId: 1628369,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628369.png",
    quote: "Celtic pride. A superstar built for the big moment."
  },
  {
    id: "bos-002",
    name: "Paul George",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 17.9,
    reb: 5,
    ast: 3.7,
    age: 21,
    nbaId: 202331,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202331.png",
    quote: "Veteran presence who brings scoring and experience on both ends."
  },
  {
    id: "bos-003",
    name: "Derrick White",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SG",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16.2,
    reb: 4.1,
    ast: 5,
    age: 31,
    nbaId: 1628401,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628401.png",
    quote: "Two-way excellence. The unsung hero of every win."
  },
  {
    id: "bos-004",
    name: "Payton Pritchard",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "PG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.6,
    reb: 3.5,
    ast: 4.8,
    age: 27,
    nbaId: 1630202,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630202.png",
    quote: "Sharpshooter and floor general off the bench."
  },
  {
    id: "bos-005",
    name: "Sam Hauser",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.1,
    reb: 3.8,
    ast: 1.5,
    age: 27,
    nbaId: 1630573,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630573.png",
    quote: "Elite corner shooter with champion DNA."
  },
  {
    id: "bos-006",
    name: "Baylor Scheierman",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 4,
    ast: 2.2,
    age: 24,
    nbaId: 1631248,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631248.png",
    quote: "Versatile wing with a sharp basketball IQ."
  },
  {
    id: "bos-007",
    name: "Neemias Queta",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9,
    reb: 7.6,
    ast: 1.3,
    age: 26,
    nbaId: 1629674,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629674.png",
    quote: "Shot-blocking specialist protecting the rim."
  },
  {
    id: "bos-008",
    name: "Mike Conley",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.5,
    reb: 2.3,
    ast: 4.5,
    age: 38,
    nbaId: 3195,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3195.png",
    quote: "Veteran floor general and leader in the locker room."
  },
  {
    id: "bos-009",
    name: "Jordan Walsh",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.8,
    reb: 3.9,
    ast: 1.4,
    age: 22,
    nbaId: 1641775,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641775.png",
    quote: "Athletic defender still developing his game."
  },
  {
    id: "bos-010",
    name: "Mitchell Robinson",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 7.4,
    reb: 9.2,
    ast: 0.8,
    age: 29,
    nbaId: 1629011,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629011.png",
    quote: "Rim protector when healthy."
  },
  {
    id: "bos-011",
    name: "Hugo González",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SG",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 4,
    reb: 3.5,
    ast: 0.6,
    age: 19,
    nbaId: 1642864,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642864.png",
    quote: "Spanish rookie with explosive athletic potential."
  },
  {
    id: "bos-012",
    name: "Ron Harper Jr.",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 3.4,
    reb: 1.5,
    ast: 0.6,
    age: 25,
    nbaId: 1631199,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631199.png",
    quote: "Physical wing fighting for every minute."
  },
  {
    id: "bkn-001",
    name: "Michael Porter Jr.",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 24.2,
    reb: 7.1,
    ast: 3,
    age: 25,
    nbaId: 1629008,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629008.png",
    quote: "Elite shooter with great size and length."
  },
  {
    id: "bkn-002",
    name: "Julius Randle",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21.1,
    reb: 6.7,
    ast: 5,
    age: 31,
    nbaId: 3064514,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3064514.png",
    quote: "Physical force in the paint with elite playmaking skills."
  },
  {
    id: "bkn-003",
    name: "Terance Mann",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "G-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 4.2,
    ast: 2.7,
    age: 25,
    nbaId: 1629611,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629611.png",
    quote: "Versatile glue guy who does whatever it takes to win."
  },
  {
    id: "bkn-004",
    name: "Noah Clowney",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 5.9,
    ast: 1.4,
    age: 25,
    nbaId: 1641730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641730.png",
    quote: "Versatile forward with rim protection and spacing."
  },
  {
    id: "bkn-005",
    name: "Moritz Wagner",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.9,
    reb: 4.8,
    ast: 1.3,
    age: 24,
    nbaId: 3150844,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3150844.png",
    quote: "High-energy big man who brings a spark off the bench."
  },
  {
    id: "bkn-006",
    name: "Danny Wolf",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 6.5,
    ast: 2.1,
    age: 25,
    nbaId: 1642874,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642874.png",
    quote: "Skilled big man with a soft touch around the rim."
  },
  {
    id: "bkn-007",
    name: "Egor Demin",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9,
    reb: 3.3,
    ast: 3.9,
    age: 25,
    nbaId: 1642856,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642856.png",
    quote: "Talented playmaker with great size for a guard."
  },
  {
    id: "bkn-008",
    name: "Day'Ron Sharpe",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.5,
    reb: 7.2,
    ast: 1.1,
    age: 25,
    nbaId: 1630549,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630549.png",
    quote: "Strong rebounder and physical presence inside."
  },
  {
    id: "bkn-009",
    name: "Josh Minott",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 4.5,
    ast: 1.1,
    age: 25,
    nbaId: 1631169,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631169.png",
    quote: "High-energy forward who impacts the game with hustle."
  },
  {
    id: "cha-001",
    name: "Brandon Miller",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.2,
    reb: 4.9,
    ast: 3.3,
    age: 22,
    nbaId: 1641706,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641706.png",
    quote: "I'm just getting started. The best is still to come."
  },
  {
    id: "cha-002",
    name: "Naz Reid",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.4,
    reb: 6.7,
    ast: 1.8,
    age: 25,
    nbaId: 4396971,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4396971.png",
    quote: "The best backup big in the league. Instant offense."
  },
  {
    id: "cha-003",
    name: "Kon Knueppel",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "G-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 4.4,
    ast: 2.6,
    age: 25,
    nbaId: 1642271,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642851.png",
    quote: "I let the game come to me and stay ready."
  },
  {
    id: "cha-004",
    name: "Grayson Allen",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 3.1,
    ast: 2.3,
    age: 27,
    nbaId: 1628960,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628960.png",
    quote: "Elite floor spacer and tough perimeter defender."
  },
  {
    id: "cha-005",
    name: "Tidjane Salaun",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.1,
    reb: 5.7,
    ast: 1.3,
    age: 20,
    nbaId: 1642275,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642275.png",
    quote: "I came to the NBA to prove I belong at this level."
  },
  {
    id: "cha-006",
    name: "Dorian Finney-Smith",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9,
    reb: 4.2,
    ast: 1.5,
    age: 25,
    nbaId: 1627827,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627827.png",
    quote: "Defensive stopper, team-first mentality."
  },
  {
    id: "cha-007",
    name: "Grant Williams",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.5,
    reb: 4,
    ast: 1.8,
    age: 24,
    nbaId: 1629684,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629684.png",
    quote: "I'm a team player first. Always."
  },
  {
    id: "cha-008",
    name: "Moussa Diabate",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "F-C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.2,
    reb: 8.8,
    ast: 1.9,
    age: 25,
    nbaId: 1631217,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631217.png",
    quote: "I go out there and compete every single night."
  },
  {
    id: "cha-009",
    name: "Ryan Kalkbrenner",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 7.9,
    reb: 5.7,
    ast: 0.8,
    age: 25,
    nbaId: 1641750,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641750.png",
    quote: "My job is to protect the rim and set screens."
  },
  {
    id: "cha-010",
    name: "Sion James",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5.4,
    reb: 3.5,
    ast: 2,
    age: 25,
    nbaId: 1642883,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642883.png",
    quote: "Every opportunity I get, I try to make the most of it."
  },
  {
    id: "chi-001",
    name: "Norman Powell",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21.7,
    reb: 3.5,
    ast: 2.5,
    age: 32,
    nbaId: 1626181,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626181.png",
    quote: "Norm is cooking every night."
  },
  {
    id: "chi-002",
    name: "Josh Giddey",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PG",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 16.8,
    reb: 7.2,
    ast: 7.1,
    age: 23,
    nbaId: 1630581,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630581.png",
    quote: "Triple-double machine. The future of the Bulls."
  },
  {
    id: "chi-003",
    name: "Matas Buzelis",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 13.4,
    reb: 5.1,
    ast: 2.4,
    age: 21,
    nbaId: 1641824,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641824.png",
    quote: "The rising star. Long, skilled and dangerous."
  },
  {
    id: "chi-004",
    name: "Nic Claxton",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 8.9,
    ast: 1.8,
    age: 26,
    nbaId: 1629651,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629651.png",
    quote: "Elite defensive anchor and rim runner."
  },
  {
    id: "chi-006",
    name: "Patrick Williams",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 4.3,
    ast: 2.1,
    age: 24,
    nbaId: 1630172,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630172.png",
    quote: "Developing forward with championship potential."
  },
  {
    id: "chi-007",
    name: "Rob Dillingham",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 10.8,
    reb: 2.9,
    ast: 4.6,
    age: 20,
    nbaId: 1642265,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642265.png",
    quote: "Lightning-quick rookie point guard."
  },
  {
    id: "chi-008",
    name: "Isaac Okoro",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 10.4,
    reb: 3.2,
    ast: 1.9,
    age: 24,
    nbaId: 1630171,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630171.png",
    quote: "Lockdown defender capable of guarding 1-4."
  },
  {
    id: "chi-009",
    name: "Noa Essengue",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.6,
    reb: 4.8,
    ast: 1.5,
    age: 19,
    nbaId: 1642855,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642855.png",
    quote: "French rookie with enormous long-term upside."
  },
  {
    id: "chi-010",
    name: "Jalen Smith",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.1,
    reb: 6.4,
    ast: 0.9,
    age: 25,
    nbaId: 1630188,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630188.png",
    quote: "Versatile big who can stretch the floor."
  },
  {
    id: "chi-011",
    name: "Tre Jones",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PG",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.7,
    reb: 2.8,
    ast: 4.9,
    age: 26,
    nbaId: 1630200,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630200.png",
    quote: "Defensive pest and steady floor general."
  },
  {
    id: "chi-012",
    name: "Guerschon Yabusele",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5.1,
    reb: 3.2,
    ast: 2,
    age: 29,
    nbaId: 1627824,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627824.png",
    quote: "Physical forward with a reliable mid-range touch."
  },
  {
    id: "chi-013",
    name: "Leonard Miller",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 4.8,
    reb: 3.8,
    ast: 0.6,
    age: 22,
    nbaId: 1631159,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631159.png",
    quote: "Young forward developing his two-way potential."
  },
  {
    id: "cle-001",
    name: "Donovan Mitchell",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 27.9,
    reb: 4.5,
    ast: 5.7,
    age: 29,
    nbaId: 1628378,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628378.png",
    quote: "Spida never misses big shots."
  },
  {
    id: "cle-002",
    name: "James Harden",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21.2,
    reb: 5.2,
    ast: 7.8,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "The Beard still orchestrates."
  },
  {
    id: "cle-003",
    name: "Evan Mobley",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "C-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.7,
    reb: 9.2,
    ast: 3.1,
    age: 24,
    nbaId: 1630596,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630596.png",
    quote: "Two-way monster from USC."
  },
  {
    id: "cle-004",
    name: "Jarrett Allen",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14,
    reb: 10.6,
    ast: 1.8,
    age: 27,
    nbaId: 1628386,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628386.png",
    quote: "Rim-running machine."
  },
  {
    id: "cle-005",
    name: "Dennis Schroder",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 3,
    ast: 5.3,
    age: 25,
    nbaId: 203471,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203471.png",
    quote: "Veteran backup general."
  },
  {
    id: "cle-006",
    name: "Max Strus",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.5,
    reb: 4,
    ast: 2.7,
    age: 25,
    nbaId: 1629622,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629622.png",
    quote: "Long-range specialist."
  },
  {
    id: "cle-007",
    name: "Sam Merrill",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.2,
    reb: 2.8,
    ast: 1.6,
    age: 25,
    nbaId: 1630241,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630241.png",
    quote: "Sharpshooter off the bench."
  },
  {
    id: "cle-008",
    name: "Jaylon Tyson",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.9,
    reb: 4.1,
    ast: 1.7,
    age: 22,
    nbaId: 1642281,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642281.png",
    quote: "Emerging young forward."
  },
  {
    id: "dal-001",
    name: "Cooper Flagg",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21,
    reb: 6.7,
    ast: 4.5,
    age: 25,
    nbaId: 1642843,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642843.png",
    quote: "I've been working for this my whole life. I'm ready."
  },
  {
    id: "dal-002",
    name: "Klay Thompson",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.6,
    reb: 3.1,
    ast: 2,
    age: 22,
    nbaId: 202691,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202691.png",
    quote: "I've been here before. The fire never goes out."
  },
  {
    id: "dal-003",
    name: "D'Angelo Russell",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.5,
    reb: 2.8,
    ast: 5.4,
    age: 25,
    nbaId: 1626156,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626156.png",
    quote: "Crafty guard with smooth shooting stroke."
  },
  {
    id: "dal-004",
    name: "P.J. Washington",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.4,
    reb: 6.3,
    ast: 1.8,
    age: 25,
    nbaId: 1629023,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629023.png",
    quote: "Versatility is my calling card on both ends."
  },
  {
    id: "dal-005",
    name: "Brandon Williams",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.8,
    reb: 2.9,
    ast: 3.8,
    age: 24,
    nbaId: 1630314,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630314.png",
    quote: "I give everything I have every time I step on the court."
  },
  {
    id: "dal-006",
    name: "Santi Aldama",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 6.8,
    ast: 2.4,
    age: 25,
    nbaId: 1630583,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630583.png",
    quote: "I work on my game every day to keep getting better."
  },
  {
    id: "dal-007",
    name: "Daniel Gafford",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.9,
    reb: 8.5,
    ast: 1.1,
    age: 25,
    nbaId: 1629655,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629655.png",
    quote: "I protect the rim and finish above the rest."
  },
  {
    id: "dal-008",
    name: "Caleb Martin",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 4.3,
    ast: 2.1,
    age: 25,
    nbaId: 1628997,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628997.png",
    quote: "I bring toughness and do the dirty work."
  },
  {
    id: "dal-009",
    name: "Naji Marshall",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 4.8,
    ast: 2.5,
    age: 25,
    nbaId: 1630230,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630230.png",
    quote: "I just stay ready and make plays when called upon."
  },
  {
    id: "dal-010",
    name: "Max Christie",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 3.5,
    ast: 2,
    age: 25,
    nbaId: 1631108,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631108.png",
    quote: "I stay locked in and shoot with confidence."
  },
  {
    id: "dal-011",
    name: "Dante Exum",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.5,
    reb: 2.5,
    ast: 3.5,
    age: 25,
    nbaId: 203957,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203957.png",
    quote: "Experienced guard providing steady playmaking."
  },
  {
    id: "dal-012",
    name: "Ryan Nembhard",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 6.4,
    reb: 1.9,
    ast: 4.9,
    age: 25,
    nbaId: 1642948,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642948.png",
    quote: "I run the offense and make my teammates better."
  },
  {
    id: "den-001",
    name: "Nikola Jokic",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 27.7,
    reb: 12.9,
    ast: 10.7,
    age: 30,
    nbaId: 203999,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203999.png",
    quote: "The game is meant to be enjoyed."
  },
  {
    id: "den-002",
    name: "Jamal Murray",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "PG",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 25.4,
    reb: 4.4,
    ast: 7.1,
    age: 28,
    nbaId: 1627750,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627750.png",
    quote: "Big moments find me prepared."
  },
  {
    id: "den-003",
    name: "Aaron Gordon",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "PF",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 15.6,
    reb: 6.3,
    ast: 3.1,
    age: 36,
    nbaId: 203932,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203932.png",
    quote: "I do what the team needs."
  },
  {
    id: "den-004",
    name: "Peyton Watson",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.9,
    reb: 4.9,
    ast: 2,
    age: 25,
    nbaId: 1631212,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631212.png",
    quote: "Every year I'm a better version."
  },
  {
    id: "den-005",
    name: "Cameron Johnson",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.4,
    reb: 4,
    ast: 2.5,
    age: 25,
    nbaId: 1629661,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629661.png",
    quote: "My shot is my identity."
  },
  {
    id: "den-006",
    name: "Christian Braun",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13,
    reb: 4.7,
    ast: 2.3,
    age: 25,
    nbaId: 1631128,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631128.png",
    quote: "Intensity has no days off."
  },
  {
    id: "den-007",
    name: "DaRon Holmes II",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 5.6,
    ast: 1.3,
    age: 25,
    nbaId: 1641747,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641747.png",
    quote: "The future starts now."
  },
  {
    id: "den-008",
    name: "Jonas Valanciunas",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.3,
    reb: 4.9,
    ast: 1.2,
    age: 33,
    nbaId: 202685,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202685.png",
    quote: "Every second in the paint counts."
  },
  {
    id: "den-009",
    name: "Zeke Nnaji",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "PF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.2,
    reb: 5,
    ast: 0.8,
    age: 25,
    nbaId: 1630192,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630192.png",
    quote: "I contribute when they need me."
  },
  {
    id: "det-001",
    name: "Cade Cunningham",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "PG",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 23.9,
    reb: 5.5,
    ast: 9.9,
    age: 24,
    nbaId: 1630595,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630595.png",
    quote: "Vision, patience, explosion — all in one."
  },
  {
    id: "det-002",
    name: "Jalen Duren",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 19.2,
    reb: 10.6,
    ast: 1.7,
    age: 21,
    nbaId: 1631105,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631105.png",
    quote: "The paint belongs to me."
  },
  {
    id: "det-003",
    name: "John Collins",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.2,
    reb: 7.1,
    ast: 1.8,
    age: 25,
    nbaId: 3908845,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3908845.png",
    quote: "High-flying forward with stretch potential."
  },
  {
    id: "det-004",
    name: "Ausar Thompson",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 13.8,
    reb: 6.4,
    ast: 2.9,
    age: 22,
    nbaId: 1641709,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641709.png",
    quote: "Energy and intensity, every single night."
  },
  {
    id: "det-005",
    name: "Duncan Robinson",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.8,
    reb: 2.6,
    ast: 2.4,
    age: 25,
    nbaId: 1629130,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629130.png",
    quote: "Three-point specialist, always ready."
  },
  {
    id: "det-006",
    name: "Taurean Prince",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 3.9,
    ast: 1.7,
    age: 25,
    nbaId: 1627752,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627752.png",
    quote: "Veteran 3-and-D forward with great defensive versatility."
  },
  {
    id: "det-007",
    name: "Paul Reed",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.9,
    reb: 6.8,
    ast: 1.2,
    age: 40,
    nbaId: 1630194,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630194.png",
    quote: "Ready whenever my number's called."
  },
  {
    id: "det-008",
    name: "Ronald Holland II",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "SF",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 5.8,
    ast: 1.3,
    age: 20,
    nbaId: 1641842,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641842.png",
    quote: "The next generation is here."
  },
  {
    id: "det-009",
    name: "Daniss Jenkins",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "PG",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.3,
    reb: 1.9,
    ast: 2.3,
    age: 25,
    nbaId: 1642450,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642450.png",
    quote: "Every opportunity, maximized."
  },
  {
    id: "gsw-001",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26.6,
    reb: 3.6,
    ast: 4.7,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "The greatest shooter in NBA history. Period."
  },
  {
    id: "gsw-002",
    name: "Jimmy Butler",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20,
    reb: 5.6,
    ast: 4.9,
    age: 36,
    nbaId: 202710,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202710.png",
    quote: "No fear. Just results when it matters most."
  },
  {
    id: "gsw-003",
    name: "Kristaps Porzingis",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16.8,
    reb: 7.1,
    ast: 2.1,
    age: 30,
    nbaId: 204001,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/204001.png",
    quote: "The unicorn reborn. Lethal stretch big with elite IQ."
  },
  {
    id: "gsw-004",
    name: "De'Anthony Melton",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13,
    reb: 3.8,
    ast: 2.3,
    age: 27,
    nbaId: 1629001,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629001.png",
    quote: "Pest defender and opportunistic scorer."
  },
  {
    id: "gsw-005",
    name: "Brandin Podziemski",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 5,
    ast: 3.5,
    age: 22,
    nbaId: 1641764,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641764.png",
    quote: "The rising star bringing new energy to the Bay."
  },
  {
    id: "gsw-006",
    name: "Moses Moody",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 3.7,
    ast: 1.6,
    age: 23,
    nbaId: 1630541,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630541.png",
    quote: "Athletic, efficient and improving every game."
  },
  {
    id: "gsw-007",
    name: "Draymond Green",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 10.2,
    reb: 6.7,
    ast: 5.8,
    age: 35,
    nbaId: 203110,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203110.png",
    quote: "The defensive anchor of a dynasty."
  },
  {
    id: "gsw-008",
    name: "Al Horford",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 6.2,
    ast: 2.4,
    age: 39,
    nbaId: 201143,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201143.png",
    quote: "Veteran anchor. IQ off the charts."
  },
  {
    id: "gsw-009",
    name: "Gui Santos",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.1,
    reb: 3.9,
    ast: 1.5,
    age: 23,
    nbaId: 4997536,
    imageUrl: "https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/full/4997536.png",
    quote: "Brazilian energy off the bench every night."
  },
  {
    id: "gsw-010",
    name: "Seth Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.8,
    reb: 2.2,
    ast: 3.4,
    age: 35,
    nbaId: 203552,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203552.png",
    quote: "A sharpshooter with the family touch."
  },
  {
    id: "gsw-011",
    name: "Gary Payton II",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.5,
    reb: 3.6,
    ast: 1.7,
    age: 33,
    nbaId: 1627780,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627780.png",
    quote: "The lockdown defender. Everywhere at once."
  },
  {
    id: "gsw-012",
    name: "Quinten Post",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7,
    reb: 3.5,
    ast: 1.7,
    age: 25,
    nbaId: 1642366,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642366.png",
    quote: "Skilled rookie center with a unique toolkit."
  },
  {
    id: "gsw-013",
    name: "Will Richard",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 3.1,
    reb: 2.1,
    ast: 0.8,
    age: 23,
    nbaId: 1642954,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642954.png",
    quote: "Athletic rookie wing earning his NBA minutes."
  },
  {
    id: "hou-001",
    name: "Kevin Durant",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "PF",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26,
    reb: 5.5,
    ast: 4.8,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "Scorer by nature, winner by choice."
  },
  {
    id: "hou-002",
    name: "Alperen Sengun",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.4,
    reb: 8.9,
    ast: 6.2,
    age: 23,
    nbaId: 1630578,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630578.png",
    quote: "The Turkish sensation redefining the center role."
  },
  {
    id: "hou-003",
    name: "Amen Thompson",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 15.6,
    reb: 7.2,
    ast: 4,
    age: 22,
    nbaId: 1641708,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641708.png",
    quote: "Elite athlete, elite energy."
  },
  {
    id: "hou-004",
    name: "Jabari Smith Jr.",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.4,
    reb: 8,
    ast: 1.5,
    age: 25,
    nbaId: 1631095,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631095.png",
    quote: "The cornerstone of Houston's future."
  },
  {
    id: "hou-005",
    name: "Tari Eason",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.2,
    reb: 6.8,
    ast: 1.7,
    age: 25,
    nbaId: 1631106,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631106.png",
    quote: "Hustle and heart in every possession."
  },
  {
    id: "hou-006",
    name: "Reed Sheppard",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.6,
    reb: 2.8,
    ast: 3.3,
    age: 20,
    nbaId: 1642263,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642263.png",
    quote: "Kentucky shooter lighting up the NBA."
  },
  {
    id: "hou-007",
    name: "Marcus Smart",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 2.9,
    ast: 3.6,
    age: 31,
    nbaId: 203935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203935.png",
    quote: "Defensive heart. Leader on and off the court."
  },
  {
    id: "hou-008",
    name: "Steven Adams",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.2,
    reb: 9,
    ast: 1.1,
    age: 32,
    nbaId: 203500,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203500.png",
    quote: "The enforcer. No soft plays."
  },
  {
    id: "hou-009",
    name: "Josh Okogie",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 4.8,
    reb: 2.6,
    ast: 0.8,
    age: 25,
    nbaId: 1629006,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629006.png",
    quote: "Lockdown defender off the bench."
  },
  {
    id: "hou-010",
    name: "Clint Capela",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 3.8,
    reb: 4.5,
    ast: 0.7,
    age: 31,
    nbaId: 203991,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203991.png",
    quote: "Still protecting the rim every night."
  },
  {
    id: "ind-001",
    name: "Pascal Siakam",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 24,
    reb: 6.6,
    ast: 3.8,
    age: 31,
    nbaId: 1627783,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627783.png",
    quote: "Versatile forward with championship experience."
  },
  {
    id: "ind-002",
    name: "Tyrese Haliburton",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 18.4,
    reb: 3.9,
    ast: 9.2,
    age: 25,
    nbaId: 1630169,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630169.png",
    quote: "Elite playmaker and the engine of the Pacers offense."
  },
  {
    id: "ind-003",
    name: "Ivica Zubac",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.2,
    reb: 10.1,
    ast: 1.4,
    age: 25,
    nbaId: 1627826,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627826.png",
    quote: "Reliable double-double machine in the paint."
  },
  {
    id: "ind-004",
    name: "Aaron Nesmith",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.8,
    reb: 4.1,
    ast: 1.9,
    age: 25,
    nbaId: 1630174,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630174.png",
    quote: "Tenacious defender and reliable floor spacer."
  },
  {
    id: "ind-005",
    name: "Kelly Oubre Jr.",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "G-F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.2,
    reb: 4.4,
    ast: 1.6,
    age: 25,
    nbaId: 1626162,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626162.png",
    quote: "A reliable scorer off the wing with defensive energy."
  },
  {
    id: "ind-006",
    name: "Obi Toppin",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 5.3,
    ast: 1.7,
    age: 25,
    nbaId: 1630167,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630167.png",
    quote: "High-flying forward who thrives in transition."
  },
  {
    id: "ind-007",
    name: "Andrew Nembhard",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 12.1,
    reb: 3.4,
    ast: 5.6,
    age: 25,
    nbaId: 1629614,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629614.png",
    quote: "Clutch performer and steady floor general."
  },
  {
    id: "ind-008",
    name: "Jarace Walker",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 5.2,
    ast: 1.6,
    age: 25,
    nbaId: 1641716,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641716.png",
    quote: "Highly skilled forward with elite defensive potential."
  },
  {
    id: "ind-009",
    name: "Jay Huff",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 3.8,
    ast: 1.4,
    age: 25,
    nbaId: 1630643,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630643.png",
    quote: "Mobile big man with shot-blocking and shooting ability."
  },
  {
    id: "ind-010",
    name: "T.J. McConnell",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 2.8,
    ast: 5.1,
    age: 25,
    nbaId: 204456,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/204456.png",
    quote: "High-energy guard and elite pest on defense."
  },
  {
    id: "ind-011",
    name: "Quenton Jackson",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.6,
    reb: 2.2,
    ast: 2.4,
    age: 25,
    nbaId: 1631245,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631245.png",
    quote: "Explosive guard with great defensive intensity."
  },
  {
    id: "lac-001",
    name: "Kawhi Leonard",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 27.9,
    reb: 6.4,
    ast: 3.6,
    age: 34,
    nbaId: 6450,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/6450.png",
    quote: "The Klaw. Two-way dominance and championship pedigree."
  },
  {
    id: "lac-002",
    name: "Bennedict Mathurin",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 19.9,
    reb: 5.9,
    ast: 2.3,
    age: 25,
    nbaId: 4683634,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4683634.png",
    quote: "Fearless scorer who attacks the rim relentlessly."
  },
  {
    id: "lac-003",
    name: "Darius Garland",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.6,
    reb: 2.7,
    ast: 6.8,
    age: 25,
    nbaId: 4396907,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4396907.png",
    quote: "Dynamic point guard with elite vision and scoring."
  },
  {
    id: "lac-004",
    name: "Rui Hachimura",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.6,
    reb: 4.5,
    ast: 1.4,
    age: 27,
    nbaId: 1629060,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629060.png",
    quote: "Physical and efficient from the wings."
  },
  {
    id: "lac-005",
    name: "Brook Lopez",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 12.8,
    reb: 5.1,
    ast: 1.4,
    age: 37,
    nbaId: 3448,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3448.png",
    quote: "Splash Mountain. Elite rim protection and spacing."
  },
  {
    id: "lac-006",
    name: "Derrick Jones Jr.",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 3.5,
    ast: 1.2,
    age: 25,
    nbaId: 3936099,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3936099.png",
    quote: "Elite athleticism and defensive versatility."
  },
  {
    id: "lac-007",
    name: "Jordan Miller",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 7.9,
    reb: 3.2,
    ast: 1.5,
    age: 22,
    nbaId: 4396818,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4396818.png",
    quote: "Versatile wing with a high basketball IQ."
  },
  {
    id: "lac-008",
    name: "Kris Dunn",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.3,
    reb: 3.3,
    ast: 3.6,
    age: 25,
    nbaId: 2991139,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/2991139.png",
    quote: "Lockdown defender and steady floor general."
  },
  {
    id: "lal-001",
    name: "Luka Doncic",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G-F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 33.5,
    reb: 7.7,
    ast: 8.3,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "The new face of Showtime. Unstoppable."
  },
  {
    id: "lal-002",
    name: "Austin Reaves",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 23.3,
    reb: 4.7,
    ast: 5.5,
    age: 27,
    nbaId: 1630559,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630559.png",
    quote: "Ice in his veins. The heart of this team."
  },
  {
    id: "lal-003",
    name: "Collin Sexton",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "PG",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 16.2,
    reb: 3,
    ast: 4.1,
    age: 27,
    nbaId: 1628384,
    imageUrl: "https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/full/4277811.png&w=350&h=254",
    quote: "Explosive scorer who attacks the paint fearlessly."
  },
  {
    id: "lal-004",
    name: "Dalton Knecht",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G-F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 14,
    reb: 4,
    ast: 1.8,
    age: 24,
    nbaId: 1642261,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642261.png",
    quote: "Rookie sharpshooter with a bright future."
  },
  {
    id: "lal-005",
    name: "Quentin Grimes",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 3.2,
    ast: 2.1,
    age: 25,
    nbaId: 1630524,
    imageUrl: "https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/full/4397014.png&w=350&h=254",
    quote: "Solid two-way guard who gives the team consistent floor spacing."
  },
  {
    id: "lal-006",
    name: "Ziaire Williams",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.6,
    reb: 3.8,
    ast: 1.6,
    age: 24,
    nbaId: 1630533,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630533.png",
    quote: "Athletic wing with defensive versatility."
  },
  {
    id: "lal-007",
    name: "Jake LaRavia",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.1,
    reb: 4,
    ast: 1.9,
    age: 24,
    nbaId: 1631222,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631222.png",
    quote: "Versatile forward ready to step up."
  },
  {
    id: "lal-008",
    name: "Jarred Vanderbilt",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 6.2,
    ast: 1.4,
    age: 26,
    nbaId: 1629020,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629020.png",
    quote: "Relentless energy and effort every night."
  },
  {
    id: "lal-009",
    name: "Matisse Thybulle",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.7,
    reb: 3.3,
    ast: 1.5,
    age: 25,
    nbaId: 1629680,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629680.png",
    quote: "Elite defensive specialist with active hands."
  },
  {
    id: "lal-010",
    name: "Maxi Kleber",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 1.9,
    reb: 1.7,
    ast: 0.7,
    age: 33,
    nbaId: 1628467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628467.png",
    quote: "Stretch big with championship experience."
  },
  {
    id: "lal-011",
    name: "Bronny James",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 1.7,
    reb: 0.6,
    ast: 1.3,
    age: 21,
    nbaId: 1642355,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642355.png",
    quote: "The next chapter of a legendary bloodline."
  },
  {
    id: "lal-012",
    name: "Adou Thiero",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 1.4,
    reb: 1.1,
    ast: 0.3,
    age: 20,
    nbaId: 1642876,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642876.png",
    quote: "Young athletic wing with elite potential."
  },
  {
    id: "mem-001",
    name: "Jerami Grant",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.8,
    reb: 3.9,
    ast: 2.1,
    age: 25,
    nbaId: 203924,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203924.png",
    quote: "Versatile two-way forward and reliable veteran scorer."
  },
  {
    id: "mem-002",
    name: "GG Jackson II",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.1,
    reb: 5.4,
    ast: 1.6,
    age: 25,
    nbaId: 1641713,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641713.png",
    quote: "I'm still young, I'm hungry, and I want to be great."
  },
  {
    id: "mem-003",
    name: "Zach Edey",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 9.2,
    ast: 1.2,
    age: 23,
    nbaId: 1641744,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641744.png",
    quote: "Size is an advantage I'm going to keep using."
  },
  {
    id: "mem-004",
    name: "Isaiah Stewart",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 7.8,
    ast: 1.4,
    age: 25,
    nbaId: 1630191,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630191.png",
    quote: "Dirty work wins championships."
  },
  {
    id: "mem-005",
    name: "Ty Jerome",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 2.5,
    ast: 3.8,
    age: 25,
    nbaId: 1629660,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629660.png",
    quote: "Hard work beats talent when talent doesn't work hard."
  },
  {
    id: "mem-006",
    name: "Jaylen Wells",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 3.6,
    ast: 1.7,
    age: 25,
    nbaId: 1642377,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642377.png",
    quote: "Consistency is what separates good players from great ones."
  },
  {
    id: "mem-007",
    name: "Cedric Coward",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.2,
    reb: 3.4,
    ast: 1.5,
    age: 25,
    nbaId: 1642907,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642907.png",
    quote: "I just stay ready and compete every time I step out there."
  },
  {
    id: "mem-008",
    name: "Cam Spencer",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 2.1,
    ast: 2.5,
    age: 25,
    nbaId: 1642285,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642285.png",
    quote: "I make plays and keep guys around me sharp."
  },
  {
    id: "mem-009",
    name: "Olivier-Maxence Prosper",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.2,
    reb: 3.1,
    ast: 1.2,
    age: 25,
    nbaId: 1641765,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641765.png",
    quote: "Every rep, every game — I'm building something special."
  },
  {
    id: "mia-001",
    name: "Giannis Antetokounmpo",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 27.6,
    reb: 9.8,
    ast: 5.4,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "The Greek Freak. Two-time MVP and champion."
  },
  {
    id: "mia-002",
    name: "Bam Adebayo",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "C-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 20.1,
    reb: 10,
    ast: 3.2,
    age: 28,
    nbaId: 1628389,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628389.png",
    quote: "South Beach's anchor."
  },
  {
    id: "mia-003",
    name: "Andrew Wiggins",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 15.8,
    reb: 4.6,
    ast: 2.4,
    age: 31,
    nbaId: 203952,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203952.png",
    quote: "Former #1 pick still delivers."
  },
  {
    id: "mia-004",
    name: "Tim Hardaway Jr.",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.6,
    reb: 2.8,
    ast: 1.9,
    age: 25,
    nbaId: 203501,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203501.png",
    quote: "Always ready to shoot."
  },
  {
    id: "mia-005",
    name: "Bobby Portis",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F-C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 7.1,
    ast: 1.6,
    age: 25,
    nbaId: 1626171,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626171.png",
    quote: "The heartbeat of Milwaukee. High energy and rebounding."
  },
  {
    id: "mia-006",
    name: "Terry Rozier",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12,
    reb: 3.2,
    ast: 3.5,
    age: 31,
    nbaId: 1626179,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626179.png",
    quote: "Scary Terry in the 305."
  },
  {
    id: "mia-007",
    name: "Nikola Jovic",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 5.1,
    ast: 2.7,
    age: 22,
    nbaId: 1631107,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631107.png",
    quote: "Serbian stretch four."
  },
  {
    id: "mia-008",
    name: "Davion Mitchell",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 2.9,
    ast: 4.8,
    age: 27,
    nbaId: 1630558,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630558.png",
    quote: "Point guard from Baylor."
  },
  {
    id: "mia-009",
    name: "Kel'el Ware",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.1,
    reb: 9.2,
    ast: 0.6,
    age: 21,
    nbaId: 1642276,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642276.png",
    quote: "The Heat's big future."
  },
  {
    id: "mia-010",
    name: "Simone Fontecchio",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.7,
    reb: 3.8,
    ast: 1.5,
    age: 30,
    nbaId: 1631323,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631323.png",
    quote: "European sniper."
  },
  {
    id: "mil-001",
    name: "Tyler Herro",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.5,
    reb: 4.8,
    ast: 4.1,
    age: 26,
    nbaId: 1629639,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629639.png",
    quote: "Built different in Miami."
  },
  {
    id: "mil-002",
    name: "Ryan Rollins",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16.8,
    reb: 4.6,
    ast: 5.6,
    age: 25,
    nbaId: 1631157,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631157.png",
    quote: "Emerging scoring guard with great potential."
  },
  {
    id: "mil-003",
    name: "Kyle Kuzma",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 16.2,
    reb: 6.1,
    ast: 2.8,
    age: 25,
    nbaId: 1628398,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628398.png",
    quote: "Versatile forward with championship experience."
  },
  {
    id: "mil-004",
    name: "Myles Turner",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 15.8,
    reb: 7.4,
    ast: 1.6,
    age: 29,
    nbaId: 1626167,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626167.png",
    quote: "Elite rim protector and floor-spacing big man."
  },
  {
    id: "mil-005",
    name: "Kevin Porter Jr.",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.1,
    reb: 4.2,
    ast: 4.9,
    age: 25,
    nbaId: 1629645,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629645.png",
    quote: "Dynamic playmaker with elite handles."
  },
  {
    id: "mil-006",
    name: "Jaime Jaquez Jr.",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 13.4,
    reb: 4.7,
    ast: 2.6,
    age: 24,
    nbaId: 1631170,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631170.png",
    quote: "Hard-nosed UCLA product."
  },
  {
    id: "mil-007",
    name: "Gary Trent Jr.",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 13.2,
    reb: 2.9,
    ast: 1.7,
    age: 25,
    nbaId: 1629018,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629018.png",
    quote: "Reliable 3-and-D perimeter threat."
  },
  {
    id: "mil-008",
    name: "Caris LeVert",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "SG",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 12.8,
    reb: 3.6,
    ast: 3.8,
    age: 25,
    nbaId: 1627747,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627747.png",
    quote: "Veteran energy off the bench."
  },
  {
    id: "mil-009",
    name: "Ousmane Dieng",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 4.1,
    ast: 2.3,
    age: 25,
    nbaId: 1631172,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631172.png",
    quote: "Talented young wing with great size and length."
  },
  {
    id: "mil-010",
    name: "A.J. Green",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.7,
    reb: 2.4,
    ast: 1.9,
    age: 35,
    nbaId: 1631260,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631260.png",
    quote: "Sharpshooter with a quick release."
  },
  {
    id: "min-001",
    name: "Anthony Edwards",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 28.8,
    reb: 5,
    ast: 3.7,
    age: 24,
    nbaId: 4594268,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4594268.png",
    quote: "Ant-Man. The most explosive scorer in the league."
  },
  {
    id: "min-002",
    name: "LaMelo Ball",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.1,
    reb: 4.8,
    ast: 7.1,
    age: 24,
    nbaId: 1630163,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630163.png",
    quote: "When I'm on the court, I just play free."
  },
  {
    id: "min-003",
    name: "Jaden McDaniels",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.4,
    reb: 5.7,
    ast: 2,
    age: 25,
    nbaId: 4431671,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4431671.png",
    quote: "Lockdown defender with a smooth offensive game."
  },
  {
    id: "min-004",
    name: "Donte DiVincenzo",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 13,
    reb: 3.8,
    ast: 3.7,
    age: 25,
    nbaId: 3934673,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3934673.png",
    quote: "The Big Ragu. Relentless energy and clutch shooting."
  },
  {
    id: "min-005",
    name: "Rudy Gobert",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.6,
    reb: 12.2,
    ast: 1.3,
    age: 33,
    nbaId: 3032976,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/3032976.png",
    quote: "The Stifle Tower. Defensive anchor and rim protector."
  },
  {
    id: "min-006",
    name: "Ayo Dosunmu",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 3.4,
    ast: 4.2,
    age: 25,
    nbaId: 1630245,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630245.png",
    quote: "Versatile guard who brings toughness and playmaking."
  },
  {
    id: "min-007",
    name: "Bones Hyland",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.4,
    reb: 2.8,
    ast: 3.6,
    age: 25,
    nbaId: 4592492,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4592492.png",
    quote: "Electric scorer with deep range and flashy handles."
  },
  {
    id: "min-008",
    name: "Terrence Shannon Jr.",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.4,
    reb: 3,
    ast: 1.5,
    age: 25,
    nbaId: 4432847,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4432847.png",
    quote: "Athletic rookie wing with elite transition scoring."
  },
  {
    id: "min-009",
    name: "Jaylen Clark",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 3.8,
    reb: 1.8,
    ast: 0.6,
    age: 25,
    nbaId: 4432247,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4432247.png",
    quote: "Defensive specialist with great instincts."
  },
  {
    id: "nop-001",
    name: "Trey Murphy III",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21.5,
    reb: 5.7,
    ast: 3.8,
    age: 25,
    nbaId: 1630530,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630530.png",
    quote: "I just want to keep improving and help this team win."
  },
  {
    id: "nop-002",
    name: "Zion Williamson",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21,
    reb: 5.7,
    ast: 3.2,
    age: 25,
    nbaId: 1629627,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629627.png",
    quote: "When I'm healthy and locked in, nobody can stop me."
  },
  {
    id: "nop-003",
    name: "Jordan Poole",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 17.4,
    reb: 2.9,
    ast: 4.2,
    age: 25,
    nbaId: 1629673,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629673.png",
    quote: "I always show up when it matters."
  },
  {
    id: "nop-004",
    name: "Dejounte Murray",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 16.8,
    reb: 5.4,
    ast: 6.2,
    age: 28,
    nbaId: 1627749,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627749.png",
    quote: "I never let my circumstances define where I'm going."
  },
  {
    id: "nop-005",
    name: "Jeremiah Fears",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.1,
    reb: 3.6,
    ast: 3.2,
    age: 25,
    nbaId: 1642847,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642847.png",
    quote: "I play with a chip on my shoulder every night."
  },
  {
    id: "nop-006",
    name: "Yves Missi",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.8,
    reb: 8.4,
    ast: 1.3,
    age: 21,
    nbaId: 1642274,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642274.png",
    quote: "I protect the paint and do the dirty work."
  },
  {
    id: "nop-007",
    name: "Saddiq Bey",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 4.8,
    ast: 1.6,
    age: 25,
    nbaId: 1630180,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630180.png",
    quote: "I take pride in being a two-way player."
  },
  {
    id: "nop-008",
    name: "Herbert Jones",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 4.1,
    ast: 2.4,
    age: 25,
    nbaId: 1630529,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630529.png",
    quote: "Defense is a mindset. I live by it."
  },
  {
    id: "nop-009",
    name: "Derik Queen",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F-C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 6.2,
    ast: 1.4,
    age: 25,
    nbaId: 1642852,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642852.png",
    quote: "I just want to be the best version of myself every day."
  },
  {
    id: "nop-010",
    name: "Bryce McGowens",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.1,
    reb: 2.1,
    ast: 1.5,
    age: 25,
    nbaId: 1631121,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631121.png",
    quote: "I take every chance I get and make the most of it."
  },
  {
    id: "nyx-001",
    name: "Jalen Brunson",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26,
    reb: 3.3,
    ast: 6.8,
    age: 29,
    nbaId: 1628973,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628973.png",
    quote: "MSG belongs to Jalen Brunson."
  },
  {
    id: "nyx-002",
    name: "Karl-Anthony Towns",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.1,
    reb: 11.9,
    ast: 3,
    age: 30,
    nbaId: 1626157,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626157.png",
    quote: "KAT dominates the paint in New York."
  },
  {
    id: "nyx-003",
    name: "Mikal Bridges",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 17.4,
    reb: 4.2,
    ast: 3.1,
    age: 29,
    nbaId: 1628969,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628969.png",
    quote: "Mikal Bridges never takes a play off."
  },
  {
    id: "nyx-004",
    name: "OG Anunoby",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16.8,
    reb: 5.1,
    ast: 2,
    age: 25,
    nbaId: 1628384,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628384.png",
    quote: "OG locks down every night."
  },
  {
    id: "nyx-005",
    name: "Jordan Clarkson",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 14.2,
    reb: 3.1,
    ast: 2.7,
    age: 25,
    nbaId: 203903,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203903.png",
    quote: "Volume scorer off the bench."
  },
  {
    id: "nyx-006",
    name: "Josh Hart",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.9,
    reb: 8.7,
    ast: 4.6,
    age: 25,
    nbaId: 1628404,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628404.png",
    quote: "The hustle king of New York."
  },
  {
    id: "nyx-007",
    name: "Miles McBride",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.4,
    reb: 2.8,
    ast: 2.9,
    age: 25,
    nbaId: 1630540,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630540.png",
    quote: "Deuce McBride coming off the bench."
  },
  {
    id: "nyx-008",
    name: "Andre Drummond",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 10.4,
    ast: 0.9,
    age: 25,
    nbaId: 203083,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203083.png",
    quote: "The hustle machine — no one crashes the boards harder."
  },
  {
    id: "okc-001",
    name: "Shai Gilgeous-Alexander",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 31.1,
    reb: 4.3,
    ast: 6.6,
    age: 27,
    nbaId: 1628983,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628983.png",
    quote: "SGA is the best player alive."
  },
  {
    id: "okc-002",
    name: "Jalen Williams",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 19.6,
    reb: 5,
    ast: 4.7,
    age: 24,
    nbaId: 1631114,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631114.png",
    quote: "J-Dub runs this offence."
  },
  {
    id: "okc-003",
    name: "Chet Holmgren",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16.2,
    reb: 8.8,
    ast: 2.3,
    age: 23,
    nbaId: 1631096,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631096.png",
    quote: "The Slim Reaper blocks everything."
  },
  {
    id: "okc-004",
    name: "Jared McCain",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.9,
    reb: 3.1,
    ast: 3.3,
    age: 21,
    nbaId: 1642272,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642272.png",
    quote: "Rookie sharpshooter from Duke."
  },
  {
    id: "okc-005",
    name: "Isaiah Hartenstein",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 11,
    reb: 9.3,
    ast: 2,
    age: 25,
    nbaId: 1628392,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628392.png",
    quote: "Dirty work every single night."
  },
  {
    id: "okc-006",
    name: "Cason Wallace",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 3.3,
    ast: 2.5,
    age: 25,
    nbaId: 1641717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641717.png",
    quote: "Next gen Oklahoma City product."
  },
  {
    id: "okc-007",
    name: "Alex Caruso",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.2,
    reb: 3.7,
    ast: 2.8,
    age: 25,
    nbaId: 1627936,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627936.png",
    quote: "The Bald Mamba never quits."
  },
  {
    id: "okc-008",
    name: "Jaylin Williams",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.6,
    reb: 5.5,
    ast: 2,
    age: 24,
    nbaId: 1631119,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631119.png",
    quote: "Energy big from Arkansas."
  },
  {
    id: "orl-001",
    name: "Paolo Banchero",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 22.2,
    reb: 8.4,
    ast: 5.2,
    age: 23,
    nbaId: 4432573,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4432573.png",
    quote: "The cornerstone of the Magic. A versatile force."
  },
  {
    id: "orl-002",
    name: "Franz Wagner",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.6,
    reb: 5.2,
    ast: 3.3,
    age: 24,
    nbaId: 4566434,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4566434.png",
    quote: "Elite skill set and basketball IQ. A rising star."
  },
  {
    id: "orl-003",
    name: "Desmond Bane",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.1,
    reb: 4.1,
    ast: 4.1,
    age: 25,
    nbaId: 4066320,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4066320.png",
    quote: "One of the best shooters in the league. Pure stroke."
  },
  {
    id: "orl-004",
    name: "Jalen Suggs",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.8,
    reb: 4.3,
    ast: 4.2,
    age: 25,
    nbaId: 4432165,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4432165.png",
    quote: "Defensive pest and high-energy floor general."
  },
  {
    id: "orl-005",
    name: "Nikola Vucevic",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 13.8,
    reb: 9.4,
    ast: 2.4,
    age: 35,
    nbaId: 202696,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202696.png",
    quote: "Veteran big man, dominant force in the paint."
  },
  {
    id: "orl-006",
    name: "Wendell Carter Jr.",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 9.1,
    ast: 2.1,
    age: 25,
    nbaId: 4277847,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4277847.png",
    quote: "Reliable big man who anchors the defense and hits the glass."
  },
  {
    id: "orl-007",
    name: "Anthony Black",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 3.8,
    ast: 3.6,
    age: 25,
    nbaId: 4712849,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4712849.png",
    quote: "Versatile guard with elite playmaking and defensive potential."
  },
  {
    id: "orl-008",
    name: "Tristan da Silva",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 4.6,
    ast: 1.7,
    age: 25,
    nbaId: 4702382,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4702382.png",
    quote: "Skilled rookie forward with a high basketball IQ."
  },
  {
    id: "orl-009",
    name: "Goga Bitadze",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.7,
    reb: 6.9,
    ast: 1.3,
    age: 25,
    nbaId: 4348700,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/4348700.png",
    quote: "Physical interior presence and reliable rebounder."
  },
  {
    id: "orl-010",
    name: "Jett Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5.4,
    reb: 1.7,
    ast: 0.9,
    age: 25,
    nbaId: 5105806,
    imageUrl: "https://a.espncdn.com/i/headshots/nba/players/full/5105806.png",
    quote: "Young sharpshooter with elite scoring potential."
  },
  {
    id: "phi-001",
    name: "Jaylen Brown",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SF",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 28.7,
    reb: 6.9,
    ast: 5.1,
    age: 29,
    nbaId: 1627759,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627759.png",
    quote: "The engine. Relentless, explosive and unstoppable."
  },
  {
    id: "phi-002",
    name: "Tyrese Maxey",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 28.3,
    reb: 4.1,
    ast: 6.6,
    age: 25,
    nbaId: 1630178,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630178.png",
    quote: "The undisputed leader of Philly, carrying the team every night."
  },
  {
    id: "phi-003",
    name: "Joel Embiid",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26.9,
    reb: 7.7,
    ast: 3.9,
    age: 31,
    nbaId: 203954,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203954.png",
    quote: "When healthy, still one of the most unstoppable big men in the league."
  },
  {
    id: "phi-004",
    name: "LeBron James",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 20.9,
    reb: 6.1,
    ast: 7.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "The King still reigns at 41. Timeless."
  },
  {
    id: "phi-005",
    name: "Anfernee Simons",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SG",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.4,
    reb: 3.2,
    ast: 4.1,
    age: 26,
    nbaId: 1629014,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629014.png",
    quote: "Silky scorer with unlimited range."
  },
  {
    id: "phi-006",
    name: "VJ Edgecombe",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14,
    reb: 6.1,
    ast: 3.4,
    age: 25,
    nbaId: 1642250,
    imageUrl: "https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/full/5124612.png&w=350&h=254",
    quote: "The exciting rookie proving he belongs in the NBA spotlight."
  },
  {
    id: "phi-007",
    name: "Dominick Barlow",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.6,
    reb: 5.1,
    ast: 1.2,
    age: 25,
    nbaId: 1631230,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631230.png",
    quote: "A rising young forward making the most of his minutes."
  },
  {
    id: "phi-008",
    name: "Dean Wade",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.9,
    reb: 4.8,
    ast: 1.4,
    age: 25,
    nbaId: 1629731,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629731.png",
    quote: "Versatile veteran wing."
  },
  {
    id: "phx-001",
    name: "Devin Booker",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26.1,
    reb: 3.9,
    ast: 6,
    age: 29,
    nbaId: 1626164,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626164.png",
    quote: "Elite three-level scorer and the face of the Suns."
  },
  {
    id: "phx-002",
    name: "Dillon Brooks",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.2,
    reb: 3.6,
    ast: 1.8,
    age: 25,
    nbaId: 1628415,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628415.png",
    quote: "Tenacious defender and high-energy competitor."
  },
  {
    id: "phx-003",
    name: "Jalen Green",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.6,
    reb: 4.6,
    ast: 3.4,
    age: 35,
    nbaId: 1630224,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630224.png",
    quote: "Explosive athlete with elite scoring potential."
  },
  {
    id: "phx-004",
    name: "Miles Bridges",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 17.2,
    reb: 6.4,
    ast: 2.9,
    age: 29,
    nbaId: 1628970,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628970.png",
    quote: "I use everything I've been through as fuel."
  },
  {
    id: "phx-005",
    name: "Mark Williams",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.8,
    reb: 9.6,
    ast: 1.4,
    age: 24,
    nbaId: 1631109,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631109.png",
    quote: "Dominant interior presence and elite rebounder."
  },
  {
    id: "phx-006",
    name: "Ryan Dunn",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.2,
    reb: 5.1,
    ast: 1.4,
    age: 25,
    nbaId: 1642346,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642346.png",
    quote: "Elite defensive prospect with great versatility."
  },
  {
    id: "phx-007",
    name: "Royce O'Neale",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.5,
    reb: 5.2,
    ast: 2.8,
    age: 25,
    nbaId: 1628400,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628400.png",
    quote: "Versatile defender and reliable three-point threat."
  },
  {
    id: "phx-008",
    name: "Collin Gillespie",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 2.6,
    ast: 3.8,
    age: 25,
    nbaId: 1631221,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631221.png",
    quote: "Steady point guard with great shooting range."
  },
  {
    id: "phx-009",
    name: "Oso Ighodaro",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.9,
    reb: 5.4,
    ast: 1.2,
    age: 25,
    nbaId: 1642345,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642345.png",
    quote: "Skilled big man with excellent passing ability."
  },
  {
    id: "phx-010",
    name: "Nick Richards",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5.8,
    reb: 5.1,
    ast: 0.8,
    age: 28,
    nbaId: 1630208,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630208.png",
    quote: "Physical interior presence. Reliable off the bench."
  },
  {
    id: "phx-011",
    name: "Khaman Maluach",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 4.8,
    reb: 5.1,
    ast: 0.5,
    age: 25,
    nbaId: 1642863,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642863.png",
    quote: "Intriguing young big man with massive wingspan."
  },
  {
    id: "por-001",
    name: "Damian Lillard",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 24.6,
    reb: 4.2,
    ast: 6.8,
    age: 35,
    nbaId: 203081,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203081.png",
    quote: "The greatest Trail Blazer of all time returning home."
  },
  {
    id: "por-002",
    name: "Deni Avdija",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 24.2,
    reb: 6.9,
    ast: 6.7,
    age: 25,
    nbaId: 1630166,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630166.png",
    quote: "Versatile forward with elite playmaking and scoring ability."
  },
  {
    id: "por-003",
    name: "Ja Morant",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 22.8,
    reb: 5.1,
    ast: 7.4,
    age: 26,
    nbaId: 1629630,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629630.png",
    quote: "I just hoop. Nobody can take that away from me."
  },
  {
    id: "por-004",
    name: "Shaedon Sharpe",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 20.8,
    reb: 4.3,
    ast: 2.6,
    age: 25,
    nbaId: 1631101,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631101.png",
    quote: "Explosive scoring guard with unlimited verticality."
  },
  {
    id: "por-005",
    name: "Scoot Henderson",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.4,
    reb: 3,
    ast: 6.1,
    age: 21,
    nbaId: 1630703,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630703.png",
    quote: "Dynamic point guard with elite speed and vision."
  },
  {
    id: "por-006",
    name: "Jrue Holiday",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14,
    reb: 4.5,
    ast: 5.7,
    age: 35,
    nbaId: 201950,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201950.png",
    quote: "Elite perimeter defender and championship-winning veteran."
  },
  {
    id: "por-007",
    name: "Jeremy Sochan",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 6.2,
    ast: 2.4,
    age: 25,
    nbaId: 1631110,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631110.png",
    quote: "Versatile young defender."
  },
  {
    id: "por-008",
    name: "Toumani Camara",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.2,
    reb: 5.7,
    ast: 1.8,
    age: 25,
    nbaId: 1641739,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641739.png",
    quote: "Tenacious defender and high-IQ role player."
  },
  {
    id: "por-009",
    name: "Donovan Clingan",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 9.2,
    ast: 1.1,
    age: 21,
    nbaId: 1642270,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642270.png",
    quote: "Dominant interior presence and elite rim protector."
  },
  {
    id: "por-010",
    name: "Robert Williams III",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 7.8,
    ast: 1,
    age: 24,
    nbaId: 1629057,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629057.png",
    quote: "High-impact defensive big man with elite shot-blocking."
  },
  {
    id: "por-011",
    name: "Yang Hansen",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 6.1,
    reb: 5.5,
    ast: 1.2,
    age: 25,
    nbaId: 1642905,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642905.png",
    quote: "Intriguing young big man with great size and potential."
  },
  {
    id: "sac-001",
    name: "Zach LaVine",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 21.4,
    reb: 4.1,
    ast: 3.8,
    age: 25,
    nbaId: 203897,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203897.png",
    quote: "I just go out and play my game. No shortcuts."
  },
  {
    id: "sac-002",
    name: "Domantas Sabonis",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "F-C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 18.9,
    reb: 12.4,
    ast: 6.1,
    age: 29,
    nbaId: 1627734,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627734.png",
    quote: "I find a way to be impactful every single night."
  },
  {
    id: "sac-003",
    name: "DeMar DeRozan",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "G-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.2,
    reb: 3,
    ast: 4.1,
    age: 36,
    nbaId: 201942,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201942.png",
    quote: "The mid-range is never dead. Not while I'm playing."
  },
  {
    id: "sac-004",
    name: "Russell Westbrook",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 15.2,
    reb: 5.4,
    ast: 6.7,
    age: 25,
    nbaId: 201566,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201566.png",
    quote: "I play every game like it could be my last."
  },
  {
    id: "sac-005",
    name: "Keegan Murray",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.6,
    reb: 5.8,
    ast: 1.7,
    age: 28,
    nbaId: 1631099,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1631099.png",
    quote: "I'm hungry and I show it every night on the floor."
  },
  {
    id: "sac-006",
    name: "Malik Monk",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.2,
    reb: 3.1,
    ast: 4.2,
    age: 25,
    nbaId: 1628370,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628370.png",
    quote: "When I'm in rhythm, I can score from anywhere."
  },
  {
    id: "sac-007",
    name: "Precious Achiuwa",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.8,
    reb: 6.4,
    ast: 1.3,
    age: 25,
    nbaId: 1630173,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630173.png",
    quote: "I bring the energy and rebounding this team needs."
  },
  {
    id: "sac-008",
    name: "Nique Clifford",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.2,
    reb: 3.4,
    ast: 2.1,
    age: 25,
    nbaId: 1642363,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642363.png",
    quote: "I fight for my spot every single day."
  },
  {
    id: "sac-009",
    name: "Maxime Raynaud",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 8.1,
    reb: 5.8,
    ast: 1,
    age: 25,
    nbaId: 1642875,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642875.png",
    quote: "I'm proving every day that I belong in this league."
  },
  {
    id: "sas-001",
    name: "Victor Wembanyama",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F-C",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 25,
    reb: 11.5,
    ast: 3.1,
    age: 21,
    nbaId: 1641705,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641705.png",
    quote: "The alien has arrived in San Antonio."
  },
  {
    id: "sas-002",
    name: "De'Aaron Fox",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.9,
    reb: 4.5,
    ast: 7.5,
    age: 28,
    nbaId: 1628368,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628368.png",
    quote: "Fox is too fast for everyone."
  },
  {
    id: "sas-003",
    name: "Stephon Castle",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 16,
    reb: 4.7,
    ast: 5,
    age: 20,
    nbaId: 1642264,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642264.png",
    quote: "Back-to-back Rookie of the Year."
  },
  {
    id: "sas-004",
    name: "Devin Vassell",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 15.6,
    reb: 4.1,
    ast: 3,
    age: 25,
    nbaId: 1630170,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630170.png",
    quote: "San Antonio's perimeter weapon."
  },
  {
    id: "sas-005",
    name: "Tobias Harris",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "PF",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14.6,
    reb: 5.8,
    ast: 2.3,
    age: 25,
    nbaId: 202699,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202699.png",
    quote: "Veteran presence when it matters most."
  },
  {
    id: "sas-006",
    name: "Dylan Harper",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 14,
    reb: 5.5,
    ast: 2.6,
    age: 25,
    nbaId: 1642844,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642844.png",
    quote: "2025 Draft #2 pick, the future."
  },
  {
    id: "sas-007",
    name: "Keldon Johnson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.2,
    reb: 5,
    ast: 2,
    age: 25,
    nbaId: 1629640,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629640.png",
    quote: "Kentucky product grinding every game."
  },
  {
    id: "sas-008",
    name: "Harrison Barnes",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11,
    reb: 4.2,
    ast: 1.5,
    age: 24,
    nbaId: 203084,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203084.png",
    quote: "Veteran leadership in San Antonio."
  },
  {
    id: "sas-009",
    name: "Julian Champagnie",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 4.5,
    ast: 1.3,
    age: 25,
    nbaId: 1630577,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630577.png",
    quote: "St. John's product finding his role."
  },
  {
    id: "sas-010",
    name: "Luke Kornet",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.4,
    reb: 6.7,
    ast: 1.1,
    age: 25,
    nbaId: 1628436,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628436.png",
    quote: "Stretch center protecting the rim."
  },
  {
    id: "sas-011",
    name: "Carter Bryant",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 5,
    reb: 3,
    ast: 1,
    age: 25,
    nbaId: 1642868,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642868.png",
    quote: "Young forward with high ceiling."
  },
  {
    id: "tor-001",
    name: "Brandon Ingram",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 21.5,
    reb: 5.6,
    ast: 3.7,
    age: 28,
    nbaId: 1627742,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627742.png",
    quote: "Elite scorer and playmaker for the North."
  },
  {
    id: "tor-002",
    name: "Scottie Barnes",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 19.8,
    reb: 7.4,
    ast: 5.9,
    age: 24,
    nbaId: 1630567,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630567.png",
    quote: "The versatile engine of the Raptors future."
  },
  {
    id: "tor-003",
    name: "RJ Barrett",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "G-F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 18.6,
    reb: 5.8,
    ast: 3.4,
    age: 25,
    nbaId: 1629628,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629628.png",
    quote: "Homegrown talent with a relentless drive."
  },
  {
    id: "tor-004",
    name: "Immanuel Quickley",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 16.2,
    reb: 3.9,
    ast: 5.1,
    age: 25,
    nbaId: 1630193,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630193.png",
    quote: "Dynamic floor general with elite range."
  },
  {
    id: "tor-005",
    name: "Jakob Poeltl",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.4,
    reb: 9.8,
    ast: 2.3,
    age: 25,
    nbaId: 1627751,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1627751.png",
    quote: "Elite rim protector and efficient finisher."
  },
  {
    id: "tor-006",
    name: "Gradey Dick",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "G-F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 12.1,
    reb: 3.4,
    ast: 1.8,
    age: 25,
    nbaId: 1641711,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641711.png",
    quote: "Sharpshooter with elite range and IQ."
  },
  {
    id: "tor-007",
    name: "Collin Murray-Boyles",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 6.2,
    ast: 2.1,
    age: 28,
    nbaId: 1642867,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642867.png",
    quote: "Promising young forward with high motor."
  },
  {
    id: "tor-008",
    name: "Sandro Mamukelashvili",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "F-C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.8,
    reb: 4.8,
    ast: 1.9,
    age: 25,
    nbaId: 1630572,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630572.png",
    quote: "Versatile big man with great vision."
  },
  {
    id: "tor-009",
    name: "Jamal Shead",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 8.1,
    reb: 2.4,
    ast: 4.6,
    age: 25,
    nbaId: 1642274,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642347.png",
    quote: "Tenacious defender and floor leader."
  },
  {
    id: "tor-010",
    name: "Ja'Kobe Walter",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.1,
    reb: 2.4,
    ast: 1.1,
    age: 21,
    nbaId: 1642266,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642266.png",
    quote: "Explosive rookie guard with scoring touch."
  },
  {
    id: "utah-001",
    name: "Lauri Markkanen",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 26.7,
    reb: 6.9,
    ast: 2.1,
    age: 25,
    nbaId: 1628374,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628374.png",
    quote: "I've evolved into a scorer nobody can stop."
  },
  {
    id: "utah-002",
    name: "Keyonte George",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "G",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 23.6,
    reb: 3.7,
    ast: 6.1,
    age: 21,
    nbaId: 1641718,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641718.png",
    quote: "I play with confidence and let my game speak."
  },
  {
    id: "utah-003",
    name: "Ace Bailey",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.8,
    reb: 5.1,
    ast: 2.4,
    age: 25,
    nbaId: 1642846,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642846.png",
    quote: "I'm just getting started — the ceiling is sky high."
  },
  {
    id: "utah-004",
    name: "Jusuf Nurkic",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.6,
    reb: 9.8,
    ast: 3.2,
    age: 31,
    nbaId: 203994,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203994.png",
    quote: "I bring the physicality and toughness inside."
  },
  {
    id: "utah-005",
    name: "Isaiah Collier",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.8,
    reb: 2.9,
    ast: 4.6,
    age: 25,
    nbaId: 1641729,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/260x190/1641729.png",
    quote: "I see the game differently. I make everyone better."
  },
  {
    id: "utah-006",
    name: "Kyle Filipowski",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F-C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 10.4,
    reb: 6.8,
    ast: 1.6,
    age: 25,
    nbaId: 1642271,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642271.png",
    quote: "I play smart and bring versatility every game."
  },
  {
    id: "utah-007",
    name: "Brice Sensabaugh",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.6,
    reb: 3.1,
    ast: 1.5,
    age: 25,
    nbaId: 1641729,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641729.png",
    quote: "I've put in the work and now it's time to show it."
  },
  {
    id: "utah-008",
    name: "Cody Williams",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 9.2,
    reb: 3.4,
    ast: 1.8,
    age: 24,
    nbaId: 1642262,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642262.png",
    quote: "I'm developing fast and hungry for more minutes."
  },
  {
    id: "utah-009",
    name: "Svi Mykhailiuk",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 9.1,
    reb: 2.5,
    ast: 1.9,
    age: 25,
    nbaId: 1629004,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629004.png",
    quote: "I stay ready and shoot when I get the chance."
  },
  {
    id: "utah-010",
    name: "Jaxson Hayes",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 7.5,
    reb: 4.1,
    ast: 0.9,
    age: 25,
    nbaId: 1629637,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629637.png",
    quote: "Explosive rim-runner with elite athleticism."
  },
  {
    id: "utah-011",
    name: "Kevin Love",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F-C",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "BENCH EDITION",
    isHistorical: false,
    pts: 6.7,
    reb: 5.8,
    ast: 1.8,
    age: 25,
    nbaId: 201567,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201567.png",
    quote: "I've been through everything. Experience is undefeated."
  },
  {
    id: "was-001",
    name: "Trae Young",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "G",
    rarity: "franchise" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 22.4,
    reb: 3.1,
    ast: 10.8,
    age: 25,
    nbaId: 1629027,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629027.png",
    quote: "Elite playmaker and deep-range threat."
  },
  {
    id: "was-002",
    name: "Anthony Davis",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "F-C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "FRANCHISE EDITION",
    isHistorical: false,
    pts: 20.4,
    reb: 11.1,
    ast: 2.8,
    age: 32,
    nbaId: 203076,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203076.png",
    quote: "Dominant force on both ends of the floor."
  },
  {
    id: "was-003",
    name: "Deandre Ayton",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "C",
    rarity: "allstar" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.8,
    reb: 10.4,
    ast: 1.6,
    age: 27,
    nbaId: 1629028,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629028.png",
    quote: "Dominant in the paint, unstoppable finisher."
  },
  {
    id: "was-004",
    name: "Alex Sarr",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "C",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 14.6,
    reb: 8.2,
    ast: 2.1,
    age: 20,
    nbaId: 1642259,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642259.png",
    quote: "Versatile big man with elite defensive potential."
  },
  {
    id: "was-005",
    name: "Bilal Coulibaly",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "F",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 13.2,
    reb: 5.4,
    ast: 2.6,
    age: 25,
    nbaId: 1641731,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1641731.png",
    quote: "Athletic forward with high defensive ceiling."
  },
  {
    id: "was-006",
    name: "Jaden Hardy",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.8,
    reb: 1.8,
    ast: 1,
    age: 25,
    nbaId: 1630702,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1630702.png",
    quote: "Fearless scorer who can heat up in an instant."
  },
  {
    id: "was-007",
    name: "Tre Johnson",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "G",
    rarity: "starter" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 12.1,
    reb: 3.2,
    ast: 2.8,
    age: 25,
    nbaId: 1642848,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642848.png",
    quote: "Highly touted prospect with elite scoring tools."
  },
  {
    id: "was-008",
    name: "Bub Carrington",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "G",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "STARTER EDITION",
    isHistorical: false,
    pts: 11.4,
    reb: 3.6,
    ast: 4.2,
    age: 20,
    nbaId: 1642267,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642267.png",
    quote: "Versatile guard with great court vision."
  },
  {
    id: "was-009",
    name: "Kyshawn George",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "G-F",
    rarity: "bench" as const,
    category: "Base" as const,
    subtitle: "ALLSTAR EDITION",
    isHistorical: false,
    pts: 10.6,
    reb: 4.3,
    ast: 2.4,
    age: 21,
    nbaId: 1642273,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1642273.png",
    quote: "Dynamic wing with playmaking and scoring ability."
  },
  {
    id: "coach-atl",
    name: "Quin Snyder",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 504,
    reb: 399,
    ast: 0,
    age: 25,
    nbaId: 203104,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQYamuJVIBP4VnB-a-zsSgkAhFMt_GXONsthQ2uIQe9Og&s=10",
    quote: "A tactical genius crafting high-powered offenses with relentless attention to detail."
  },
  {
    id: "coach-bos",
    name: "Joe Mazzulla",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 238,
    reb: 90,
    ast: 1,
    age: 25,
    nbaId: 1610612738,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS6wiPEyCb_k4aYHEPUBmeUrGUQY4Lba7FrDr_3dI6O1Q&s=10",
    quote: "Leading the Celtics with relentless focus and tactical precision."
  },
  {
    id: "coach-bkn",
    name: "Jordi Fernández",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 46,
    reb: 118,
    ast: 0,
    age: 25,
    nbaId: 1610612751,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcShOj9XVfVnpY_5hLlIDtHKMofkt9SS4UkRIjZ3SMkx1A&s=10",
    quote: "The first Catalan head coach in NBA history, blending tactical precision with elite player development."
  },
  {
    id: "coach-cha",
    name: "Charles Lee",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 63,
    reb: 101,
    ast: 0,
    age: 25,
    nbaId: 1610612766,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRFhalkh7tUX-DMRQnmohMOMNlUCfThN-RsCPwrlxbDgg&s=10",
    quote: "Bringing a championship-winning culture and elite tactical discipline to Charlotte."
  },
  {
    id: "coach-chi",
    name: "Tiago Splitter",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 42,
    reb: 39,
    ast: 0,
    age: 25,
    nbaId: 201168,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRh5bHCnIaqQJqZLAnUfbBxu4okcPG9B7aYUHkZvFrh3w&s=10",
    quote: "Bringing championship player experience and tactical precision as head coach to Chicago."
  },
  {
    id: "coach-cle",
    name: "Kenny Atkinson",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 234,
    reb: 238,
    ast: 0,
    age: 25,
    nbaId: 1610612739,
    imageUrl: "https://ca-times.brightspotcdn.com/dims4/default/a2d8e46/2147483647/strip/true/crop/2621x1763+0+0/resize/1200x807!/quality/75/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F7c%2Fa1%2F95619af60ccfbc75d41a7cf0c8b4%2Fe21446e8e4dc4a2cb02d9c4859f4aeab",
    quote: "Bringing explosive offense and relentless player development to Cleveland."
  },
  {
    id: "coach-dal",
    name: "Dusty May",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 0,
    reb: 0,
    ast: 0,
    age: 25,
    nbaId: 1610612742,
    imageUrl: "https://platform.maizenbrew.com/wp-content/uploads/sites/102/2026/08/gettyimages-2283504915.jpg?quality=90&strip=all&crop=0%2C8.3333333333333%2C100%2C83.333333333333&w=2400",
    quote: "Architect of high-tempo basketball taking over the helm in Dallas."
  },
  {
    id: "coach-den",
    name: "David Adelman",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 57,
    reb: 28,
    ast: 0,
    age: 25,
    nbaId: 1610612743,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcShD6JS30mDvOgZN5Gu8KggMHWx-7Y2TxWebjI_F2-PFQ&s=10",
    quote: "A brilliant offensive mind continuing Denver's championship pedigree."
  },
  {
    id: "coach-det",
    name: "J.B. Bickerstaff",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 359,
    reb: 350,
    ast: 0,
    age: 25,
    nbaId: 203106,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQbaQIg4i47HgsoaAPxtMMNktqfS0OBgttxy9oQfYRLUw&s=10",
    quote: "Establishing a hard-nosed defensive culture and championship habits in Detroit."
  },
  {
    id: "coach-gsw",
    name: "Steve Kerr",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 604,
    reb: 353,
    ast: 4,
    age: 25,
    nbaId: 920,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTI2-q4HnLlG265KnqBJB1dqecpXgjf6Pp7x6NUVLpudw&s=10",
    quote: "4-time championship head coach orchestrating Golden State's historic Dynasty."
  },
  {
    id: "coach-hou",
    name: "Ime Udoka",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 196,
    reb: 132,
    ast: 0,
    age: 25,
    nbaId: 200787,
    imageUrl: "https://bostonglobe-prod.cdn.arcpublishing.com/resizer/v2/P4Y55QXL2JEVRZSZKGNRZMGBUQ.jpg?auth=a54e650c95c5737e81c50f9d8e5938b886d728b3fa34ec20e3f49637dcf828cb&width=1440",
    quote: "Instilling fierce accountability, physical defense, and competitive fire in Houston."
  },
  {
    id: "coach-ind",
    name: "Rick Carlisle",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 1012,
    reb: 923,
    ast: 1,
    age: 25,
    nbaId: 1628,
    imageUrl: "https://www.unocontraunoweb.com/wp-content/uploads/2026/01/Rick-1000.jpg",
    quote: "Championship architect over 1,000 wins power-charging Indiana's historic offense."
  },
  {
    id: "coach-lac",
    name: "Tyronn Lue",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 404,
    reb: 289,
    ast: 1,
    age: 25,
    nbaId: 1731,
    imageUrl: "https://objetos-xlk.estaticos-marca.com/uploads/2024/07/05/17170055384495.jpeg",
    quote: "Master playoff tactician renowned for elite in-game adjustments and leadership."
  },
  {
    id: "coach-lal",
    name: "JJ Redick",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 103,
    reb: 61,
    ast: 0,
    age: 25,
    nbaId: 200755,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTFsT7f1YRlDqsdNiWf21n7Ltf6Kza-xZtpjsFfatURPA&s=10",
    quote: "Bringing modern analytical vision, high-IQ spacing, and detail to the Lakers."
  },
  {
    id: "coach-mem",
    name: "Tuomas Iisalo",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 29,
    reb: 62,
    ast: 0,
    age: 25,
    nbaId: 1610612763,
    imageUrl: "https://preview.redd.it/tuomas-iisalo-took-over-as-the-head-coach-for-one-game-this-v0-ssiz4x1ekhre1.png?width=640&crop=smart&auto=webp&s=b992656f648e4367675bceca6a0152a9f38fa19f",
    quote: "Bringing European tactical innovation and high-intensity basketball to Memphis."
  },
  {
    id: "coach-mia",
    name: "Erik Spoelstra",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 830,
    reb: 611,
    ast: 2,
    age: 25,
    nbaId: 1610612748,
    imageUrl: "https://estaticos-cdn.prensaiberica.es/clip/e59a6236-97f4-4f02-8543-eea42cfa74f1_16-9-discover-aspect-ratio_default_0.jpg",
    quote: "2-time champion standard-bearer of Heat Culture and tactical brilliance."
  },
  {
    id: "coach-mil",
    name: "Taylor Jenkins",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 250,
    reb: 214,
    ast: 0,
    age: 25,
    nbaId: 1610612749,
    imageUrl: "https://ca-times.brightspotcdn.com/dims4/default/d553116/2147483647/strip/true/crop/4412x2942+0+0/resize/1200x800!/quality/75/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2Ffa%2F91%2Fef79e2eda3ca211128c60f572ebf%2F3be1d0afb03d44e9abc7fb3fb4eb6d86",
    quote: "Bringing modern defensive intensity and tactical execution to Milwaukee."
  },
  {
    id: "coach-min",
    name: "Chris Finch",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 258,
    reb: 193,
    ast: 0,
    age: 25,
    nbaId: 1610612750,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR9xPhFfuWQN8yRC-umEAKqwrhTLjJBIPJVaqlpFm5eZYZJ7Xmdk-4qr7QK&s=10",
    quote: "Architect of Minnesota's elite defense and modern offensive flow."
  },
  {
    id: "coach-nop",
    name: "Jamahl Mosley",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 189,
    reb: 221,
    ast: 0,
    age: 25,
    nbaId: 1610612740,
    imageUrl: "https://ca-times.brightspotcdn.com/dims4/default/379c0b7/2147483647/strip/true/crop/2358x1572+0+0/resize/1200x800!/quality/75/?url=https%3A%2F%2Fcalifornia-times-brightspot.s3.amazonaws.com%2F79%2F4f%2Fde11f4e34d0f85e662d4b8131b38%2F191b012258244778b570dbe6b8b9619b.jpg",
    quote: "Building a foundation of defensive grit and selfless basketball in New Orleans."
  },
  {
    id: "coach-nyk",
    name: "Mike Brown",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 507,
    reb: 333,
    ast: 1,
    age: 25,
    nbaId: 1610612752,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8uzMqhWuMSupyFvRkG7AOi_rc5tRwQA4YDNaFmzW5Fw&s=10",
    quote: "Bringing high-speed execution and championship defensive principles to New York."
  },
  {
    id: "coach-okc",
    name: "Mark Daigneault",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 275,
    reb: 207,
    ast: 1,
    age: 25,
    nbaId: 1610612760,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTbre6n-x1nzd6DbCVFP6_dFgMJWGavCUb_woZahatAYQ&s=10",
    quote: "Coach of the Year establishing Oklahoma City's position atop the Western Conference."
  },
  {
    id: "coach-orl",
    name: "Sean Sweeney",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 0,
    reb: 0,
    ast: 0,
    age: 25,
    nbaId: 1610612753,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR025pmxgGfPDuh57fLa41ZFaBgqXYgRHsPILfsUHu0F-0PwkU3L0jZma69&s=10",
    quote: "Elite defensive mind stepping up to lead Orlando's talented core."
  },
  {
    id: "coach-phi",
    name: "Nick Nurse",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 343,
    reb: 293,
    ast: 1,
    age: 25,
    nbaId: 1610612755,
    imageUrl: "https://cloudfront-us-east-1.images.arcpublishing.com/pmn/ZVLQ33RQUZDJ64KUTUVU36N4KU.jpg",
    quote: "Championship tactician known for creative defensive schemes and relentless mindset."
  },
  {
    id: "coach-phx",
    name: "Jordan Ott",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 45,
    reb: 37,
    ast: 0,
    age: 25,
    nbaId: 1610612756,
    imageUrl: "https://cdn.allcitynetwork.com/wp-content/uploads/sites/4/2025/11/24111027/USATSI_27520104-1024x683.jpg.webp",
    quote: "Modern analytical mind leading Phoenix's high-powered attack."
  },
  {
    id: "coach-por",
    name: "Micah Nori",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 0,
    reb: 0,
    ast: 0,
    age: 25,
    nbaId: 1610612757,
    imageUrl: "https://s7d2.scene7.com/is/image/TWCNews/nori-cavs-AP24141030167851-ohio-060324",
    quote: "Master strategist taking the helm in Portland."
  },
  {
    id: "coach-sac",
    name: "Doug Christie",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 49,
    reb: 84,
    ast: 0,
    age: 25,
    nbaId: 947,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQv5eh0IiCVvXtDHULmIj3nk5OqTgXEKp05DqberjAP2Q&s=10",
    quote: "Kings legend bringing heart, defense, and intensity to the Sacramento bench."
  },
  {
    id: "coach-sas",
    name: "Mitch Johnson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 94,
    reb: 65,
    ast: 0,
    age: 25,
    nbaId: 1610612759,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZ3aI_4ZmVeJsv_oOib0irVPUVWYYP2VGQjiaTQCAT3w&s=10",
    quote: "Guiding San Antonio's elite young core with modern pace and disciplined execution."
  },
  {
    id: "coach-tor",
    name: "Darko Rajaković",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 101,
    reb: 145,
    ast: 0,
    age: 25,
    nbaId: 1610612761,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT6mrDx7cj-ZKRJWrqFrxTlHbDtxDN8H52CK94dVl2AXA&s=10",
    quote: "Implementing a selfless, fast-paced culture in the North."
  },
  {
    id: "coach-uta",
    name: "Will Hardy",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 107,
    reb: 221,
    ast: 0,
    age: 25,
    nbaId: 1610612762,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSaQ4AscGe2FE1vxZ_RTTHtgcTGiGH98_AWAHp3cwf6cA&s=10",
    quote: "A brilliant young mind focused on playing the right way with high-octane energy."
  },
  {
    id: "coach-was",
    name: "Brian Keefe",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "Coach",
    rarity: "coach" as const,
    category: "Coach" as const,
    subtitle: "HEAD COACH",
    isHistorical: false,
    pts: 43,
    reb: 160,
    ast: 0,
    age: 25,
    nbaId: 1610612764,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQM_nptrT46VNw812SCGP1JkaFupvJcfL3GqtNFutYPTQ&s=10",
    quote: "A dedicated teacher focused on building winning habits and elite player development."
  },
  {
    id: "logo-bos",
    name: "Boston Celtics Shield",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 56,
    reb: 26,
    ast: 18,
    age: 25,
    nbaId: 1610612738,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612738/global/L/logo.svg",
    quote: "18-Time NBA Champions. A legacy of greatness."
  },
  {
    id: "logo-nyk",
    name: "New York Knicks Shield",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 53,
    reb: 29,
    ast: 2,
    age: 25,
    nbaId: 1610612752,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612752/global/L/logo.svg",
    quote: "2-Time NBA Champions. The pulse of New York City."
  },
  {
    id: "logo-cle",
    name: "Cleveland Cavaliers Shield",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 52,
    reb: 30,
    ast: 1,
    age: 25,
    nbaId: 1610612739,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612739/global/L/logo.svg",
    quote: "2016 NBA Champions. Believeland forever."
  },
  {
    id: "logo-mil",
    name: "Milwaukee Bucks Shield",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 49,
    reb: 33,
    ast: 2,
    age: 25,
    nbaId: 1610612749,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612749/global/L/logo.svg",
    quote: "2-Time NBA Champions. Fear the Deer."
  },
  {
    id: "logo-ind",
    name: "Indiana Pacers Shield",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 48,
    reb: 34,
    ast: 0,
    age: 25,
    nbaId: 1610612754,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612754/global/L/logo.svg",
    quote: "Built on tradition. Chasing the first ring in Indiana."
  },
  {
    id: "logo-phi",
    name: "Philadelphia 76ers Shield",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 45,
    reb: 37,
    ast: 3,
    age: 25,
    nbaId: 1610612755,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612755/global/L/logo.svg",
    quote: "3-Time NBA Champions. Trust the process and the legacy."
  },
  {
    id: "logo-orl",
    name: "Orlando Magic Shield",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 45,
    reb: 37,
    ast: 0,
    age: 25,
    nbaId: 1610612753,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612753/global/L/logo.svg",
    quote: "Magic in the making. Pure talent rising in Orlando."
  },
  {
    id: "logo-cha",
    name: "Charlotte Hornets Shield",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 44,
    reb: 38,
    ast: 0,
    age: 25,
    nbaId: 1610612766,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612766/global/L/logo.svg",
    quote: "The Hive is alive. Charlotte's high-flying basketball."
  },
  {
    id: "logo-mia",
    name: "Miami Heat Shield",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 43,
    reb: 39,
    ast: 3,
    age: 25,
    nbaId: 1610612748,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612748/global/L/logo.svg",
    quote: "3-Time NBA Champions. Heat Culture is built differently."
  },
  {
    id: "logo-atl",
    name: "Atlanta Hawks Shield",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 42,
    reb: 40,
    ast: 1,
    age: 25,
    nbaId: 1610612737,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612737/global/L/logo.svg",
    quote: "1958 NBA Champions. True to Atlanta."
  },
  {
    id: "logo-chi",
    name: "Chicago Bulls Shield",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 38,
    reb: 44,
    ast: 6,
    age: 25,
    nbaId: 1610612741,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612741/global/L/logo.svg",
    quote: "6-Time NBA Champions. The spirit of the 90s dynasty."
  },
  {
    id: "logo-tor",
    name: "Toronto Raptors Shield",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 35,
    reb: 47,
    ast: 1,
    age: 25,
    nbaId: 1610612761,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612761/global/L/logo.svg",
    quote: "2019 NBA Champions. We The North."
  },
  {
    id: "logo-was",
    name: "Washington Wizards Shield",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 25,
    reb: 57,
    ast: 1,
    age: 25,
    nbaId: 1610612764,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612764/global/L/logo.svg",
    quote: "1978 NBA Champions. Rebuilding the magic in DC."
  },
  {
    id: "logo-bkn",
    name: "Brooklyn Nets Shield",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 20,
    reb: 62,
    ast: 0,
    age: 25,
    nbaId: 1610612751,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612751/global/L/logo.svg",
    quote: "Street style, Brooklyn grit. Chasing the ultimate prize."
  },
  {
    id: "logo-okc",
    name: "Oklahoma City Thunder Shield",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 64,
    reb: 18,
    ast: 1,
    age: 25,
    nbaId: 1610612760,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612760/global/L/logo.svg",
    quote: "1979 NBA Champions. A storm is brewing in OKC."
  },
  {
    id: "logo-hou",
    name: "Houston Rockets Shield",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 54,
    reb: 28,
    ast: 2,
    age: 25,
    nbaId: 1610612745,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612745/global/L/logo.svg",
    quote: "2-Time NBA Champions. Launching into a new era."
  },
  {
    id: "logo-lal",
    name: "Los Angeles Lakers Shield",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 53,
    reb: 29,
    ast: 17,
    age: 25,
    nbaId: 1610612747,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612747/global/L/logo.svg",
    quote: "17-Time NBA Champions. Showtime never ends."
  },
  {
    id: "logo-den",
    name: "Denver Nuggets Shield",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 51,
    reb: 31,
    ast: 1,
    age: 25,
    nbaId: 1610612743,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612743/global/L/logo.svg",
    quote: "2023 NBA Champions. Mile High basketball at its peak."
  },
  {
    id: "logo-lac",
    name: "LA Clippers Shield",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 49,
    reb: 33,
    ast: 0,
    age: 25,
    nbaId: 1610612746,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612746/global/L/logo.svg",
    quote: "Clippers Spirit. A new identity in Los Angeles."
  },
  {
    id: "logo-phx",
    name: "Phoenix Suns Shield",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 47,
    reb: 35,
    ast: 0,
    age: 25,
    nbaId: 1610612756,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612756/global/L/logo.svg",
    quote: "Rise Up. Phoenix chasing that elusive first title."
  },
  {
    id: "logo-min",
    name: "Minnesota Timberwolves Shield",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 46,
    reb: 36,
    ast: 0,
    age: 25,
    nbaId: 1610612750,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612750/global/L/logo.svg",
    quote: "Raised by Wolves. A pack hungry for success."
  },
  {
    id: "logo-sas",
    name: "San Antonio Spurs Shield",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 43,
    reb: 39,
    ast: 5,
    age: 25,
    nbaId: 1610612759,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612759/global/L/logo.svg",
    quote: "5-Time NBA Champions. Excellence is a habit."
  },
  {
    id: "logo-por",
    name: "Portland Trail Blazers Shield",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 41,
    reb: 41,
    ast: 1,
    age: 25,
    nbaId: 1610612757,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612757/global/L/logo.svg",
    quote: "1977 NBA Champions. Rip City pride."
  },
  {
    id: "logo-sac",
    name: "Sacramento Kings Shield",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 40,
    reb: 42,
    ast: 1,
    age: 25,
    nbaId: 1610612758,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612758/global/L/logo.svg",
    quote: "1951 NBA Champions. Light the Beam."
  },
  {
    id: "logo-gsw",
    name: "Golden State Warriors Shield",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 37,
    reb: 45,
    ast: 7,
    age: 25,
    nbaId: 1610612744,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612744/global/L/logo.svg",
    quote: "7-Time NBA Champions. The Dub Nation dynasty."
  },
  {
    id: "logo-dal",
    name: "Dallas Mavericks Shield",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 26,
    reb: 56,
    ast: 1,
    age: 25,
    nbaId: 1610612742,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612742/global/L/logo.svg",
    quote: "2011 NBA Champions. MFFL - Mavs Fan For Life."
  },
  {
    id: "logo-nop",
    name: "New Orleans Pelicans Shield",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 26,
    reb: 56,
    ast: 0,
    age: 25,
    nbaId: 1610612740,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612740/global/L/logo.svg",
    quote: "Pelicans Pride. Let the good times roll in NOLA."
  },
  {
    id: "logo-mem",
    name: "Memphis Grizzlies Shield",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 25,
    reb: 57,
    ast: 0,
    age: 25,
    nbaId: 1610612763,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612763/global/L/logo.svg",
    quote: "Grit and Grind. Memphis is never satisfied."
  },
  {
    id: "logo-uta",
    name: "Utah Jazz Shield",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "TEAM",
    rarity: "logo" as const,
    category: "Logo" as const,
    subtitle: "NBA LOGO SERIES",
    series: "NBA Team Logo Series",
    isHistorical: false,
    pts: 22,
    reb: 60,
    ast: 0,
    age: 25,
    nbaId: 1610612762,
    imageUrl: "https://cdn.nba.com/logos/nba/1610612762/global/L/logo.svg",
    quote: "Elevating the game in Utah. Chasing the peak."
  },
  {
    id: "arena-bos",
    name: "TD Garden",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19580,
    reb: 1995,
    ast: 0,
    age: 25,
    nbaId: 10001,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/59e05ff4c819893fc2a09372320a20136eccd343.jpg",
    quote: "The heart of Boston sports, home to the championship-winning Celtics."
  },
  {
    id: "arena-nyk",
    name: "Madison Square Garden",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19812,
    reb: 1968,
    ast: 0,
    age: 25,
    nbaId: 10002,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/6a4562864ec9bc1c78d57ef22b881316330331ae.jpg",
    quote: "The World's Most Famous Arena, iconic in every sense."
  },
  {
    id: "arena-bkn",
    name: "Barclays Center",
    team: "Brooklyn Nets",
    teamAbbr: "BKN",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17732,
    reb: 2012,
    ast: 0,
    age: 25,
    nbaId: 10003,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/0c6c4ca1653c7991bf0a01dd9f7df7a8b3c9d66f.jpg",
    quote: "Brooklyn's modern architectural marvel and hub for the Nets."
  },
  {
    id: "arena-phi",
    name: "Wells Fargo Center",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 20478,
    reb: 1996,
    ast: 0,
    age: 25,
    nbaId: 10004,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/ef0be57a74ad420b2f4ecdd455867fd4d63d255e.jpg",
    quote: "A loud and energetic home for the 76ers in Philadelphia."
  },
  {
    id: "arena-tor",
    name: "Scotiabank Arena",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19800,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10005,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/870cacd5b19acd2d41d27301b6d2ef31e6ba60b8.jpg",
    quote: "Where the Raptors guard the North in front of passionate fans."
  },
  {
    id: "arena-chi",
    name: "United Center",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 20917,
    reb: 1994,
    ast: 0,
    age: 25,
    nbaId: 10006,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/a2729fd93344f025ba2bd876a53124b19ddb5e2.jpg",
    quote: "The house that Michael Jordan built, still a fortress for the Bulls."
  },
  {
    id: "arena-cle",
    name: "Rocket Mortgage FieldHouse",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19432,
    reb: 1994,
    ast: 0,
    age: 25,
    nbaId: 10007,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/caf9d52bb9171f9ebd7426a07fc2ead14f8fdab4.jpg",
    quote: "A vibrant and modern fieldhouse for Cleveland basketball."
  },
  {
    id: "arena-det",
    name: "Little Caesars Arena",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 20332,
    reb: 2017,
    ast: 0,
    age: 25,
    nbaId: 10008,
    imageUrl: "https://picsum.photos/seed/littlecaesars/800/500",
    quote: "The state-of-the-art home in the heart of Hockeytown for the Pistons."
  },
  {
    id: "arena-ind",
    name: "Gainbridge Fieldhouse",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17923,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10009,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/64ec5a952acb87e42f1bd2374e3b5e5b0ae13df9.jpg",
    quote: "A traditional yet modern basketball cathedral in Indiana."
  },
  {
    id: "arena-mil",
    name: "Fiserv Forum",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17500,
    reb: 2018,
    ast: 0,
    age: 25,
    nbaId: 10010,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/4e4feace824c32c4dd019fb2defed47cac6273a3.jpg",
    quote: "A world-class arena that fueled the Bucks to a championship."
  },
  {
    id: "arena-atl",
    name: "State Farm Arena",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 16600,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10011,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/fb86bae8426802a606f82e790c91e6c2f05f83a4.jpg",
    quote: "The transformed and energetic nest of the Atlanta Hawks."
  },
  {
    id: "arena-cha",
    name: "Spectrum Center",
    team: "Charlotte Hornets",
    teamAbbr: "CHA",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19077,
    reb: 2005,
    ast: 0,
    age: 25,
    nbaId: 10012,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/44dcf41d4ef1bdbc4bfc28e5a91ad861e8f8d35a.jpg",
    quote: "The colorful hive where the Hornets buzz every night."
  },
  {
    id: "arena-mia",
    name: "Kaseya Center",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19600,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10013,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/1e8089a9f7174d86be34068953d77fce1d11242b.jpg",
    quote: "The tropical home of Heat Culture on the Miami waterfront."
  },
  {
    id: "arena-orl",
    name: "Kia Center",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18846,
    reb: 2010,
    ast: 0,
    age: 25,
    nbaId: 10014,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/d5bfe27a42eab5833d120b36efb12936bac93918.jpg",
    quote: "Where the Magic happens in center of Orlando."
  },
  {
    id: "arena-was",
    name: "Capital One Arena",
    team: "Washington Wizards",
    teamAbbr: "WAS",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 20356,
    reb: 1997,
    ast: 0,
    age: 25,
    nbaId: 10015,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/d740ccaaade949715cd9747aa8cfa81cd4e40b25.jpg",
    quote: "The downtown hub for basketball in the nation's capital."
  },
  {
    id: "arena-lal",
    name: "Crypto.com Arena",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19068,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10016,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/3258c7482a053a2c70c9df6248cb2df75813b3da.jpg",
    quote: "The glamorous center of Los Angeles sports and entertainment."
  },
  {
    id: "arena-lac",
    name: "Intuit Dome",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18000,
    reb: 2024,
    ast: 0,
    age: 25,
    nbaId: 10017,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/4ce40e30b3ae51639b851dacdfe310c5bc362904.jpg",
    quote: "The futuristic and fan-focused new home for the Clippers."
  },
  {
    id: "arena-gsw",
    name: "Chase Center",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18064,
    reb: 2019,
    ast: 0,
    age: 25,
    nbaId: 10018,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/0da34a5efdb45396254cebe7657d6f65fec47370.jpg",
    quote: "The waterfront home of the high-flying Warriors in San Francisco."
  },
  {
    id: "arena-phx",
    name: "Footprint Center",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17071,
    reb: 1992,
    ast: 0,
    age: 25,
    nbaId: 10019,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/f7614dcbe79412bf665270fc080ddc02046af82e.jpg",
    quote: "The desert oasis where the Suns shine brightest."
  },
  {
    id: "arena-sac",
    name: "Golden 1 Center",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17583,
    reb: 2016,
    ast: 0,
    age: 25,
    nbaId: 10020,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/af3fb855f340320507ddd5db07323c8ea28f79a6.jpg",
    quote: "Sacramento's vibrant arena and home to the Beam."
  },
  {
    id: "arena-por",
    name: "Moda Center",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19441,
    reb: 1995,
    ast: 0,
    age: 25,
    nbaId: 10021,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/0b295c0b54e02290a221ffb9805f4de7e1b98dd8.jpg",
    quote: "Rip City's loyal home, known for its intense atmosphere."
  },
  {
    id: "arena-okc",
    name: "Paycom Center",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18203,
    reb: 2002,
    ast: 0,
    age: 25,
    nbaId: 10022,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/a6c5e34097ef3d625caa7d7768c15d539c84f208.jpg",
    quote: "The electric and loud center of the Thunder storm."
  },
  {
    id: "arena-uta",
    name: "Delta Center",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18306,
    reb: 1991,
    ast: 0,
    age: 25,
    nbaId: 10023,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/89c807c50ecbda0461c216428527726b65912440.jpg",
    quote: "The high-altitude home of the Jazz and their loyal fans."
  },
  {
    id: "arena-den",
    name: "Ball Arena",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19520,
    reb: 1999,
    ast: 0,
    age: 24,
    nbaId: 10024,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/1880e45443518b1eb5f86fb630f7b92365c3dfeb.jpg",
    quote: "Where the Nuggets play at a mile high in Denver."
  },
  {
    id: "arena-min",
    name: "Target Center",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18978,
    reb: 1990,
    ast: 0,
    age: 25,
    nbaId: 10025,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/9ddad5677df4a824af2df33018472b602516f6a4.jpg",
    quote: "The pack's home in downtown Minneapolis."
  },
  {
    id: "arena-dal",
    name: "American Airlines Center",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 19200,
    reb: 2001,
    ast: 0,
    age: 25,
    nbaId: 10026,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/5a7dd3735a8dc5cd31dc420c0830480242955df7.jpg",
    quote: "A big, bold arena for a big, bold Mavericks team."
  },
  {
    id: "arena-hou",
    name: "Toyota Center",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18055,
    reb: 2003,
    ast: 0,
    age: 25,
    nbaId: 10027,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/8be0ce44abe7c5297b2410f298a2436a381388c9.jpg",
    quote: "The launchpad for Rockets greatness in Houston."
  },
  {
    id: "arena-mem",
    name: "FedExForum",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 17794,
    reb: 2004,
    ast: 0,
    age: 25,
    nbaId: 10028,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/667779eef37f133b1766062274c07024f6f232f7.jpg",
    quote: "The gritty and soulful home for Grizzles basketball."
  },
  {
    id: "arena-nop",
    name: "Smoothie King Center",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 16867,
    reb: 1999,
    ast: 0,
    age: 25,
    nbaId: 10029,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/92f532fad162c1fd64178e8cf458d93acf6bcd66.jpg",
    quote: "A vibrant nest for the Pelicans in the Big Easy."
  },
  {
    id: "arena-sas",
    name: "Frost Bank Center",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "ARENA",
    rarity: "arena" as const,
    category: "Arena" as const,
    subtitle: "OFFICIAL TEAM ARENA",
    series: "NBA Arena Series",
    isHistorical: false,
    pts: 18354,
    reb: 2002,
    ast: 0,
    age: 25,
    nbaId: 10030,
    imageUrl: "https://pplx-res.cloudinary.com/image/upload/pplx_search_images/3939d29ef38847833e57027d36d16842fc6cd79a.jpg",
    quote: "The fortress of Spurs excellence and tradition."
  },
  {
    id: "draft26-1",
    name: "AJ Dybantsa",
    team: "BYU",
    teamAbbr: "BYU",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #1 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 25.5,
    reb: 6.8,
    ast: 3.7,
    age: 25,
    nbaId: 5142718,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142718.png",
    quote: "The consensus #1 pick. A scoring machine with NBA size and length."
  },
  {
    id: "draft26-2",
    name: "Darryn Peterson",
    team: "Kansas",
    teamAbbr: "KAN",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #2 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 20.5,
    reb: 4.3,
    ast: 3.8,
    age: 25,
    nbaId: 5041955,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5041955.png",
    quote: "Elite shot creator with deep range and polished offensive game."
  },
  {
    id: "draft26-3",
    name: "Cameron Boozer",
    team: "Duke",
    teamAbbr: "DUK",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #3 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 22.5,
    reb: 10.2,
    ast: 4.1,
    age: 25,
    nbaId: 5041935,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5041935.png",
    quote: "Double-double machine with high basketball IQ and interior dominance."
  },
  {
    id: "draft26-4",
    name: "Caleb Wilson",
    team: "N. Carolina",
    teamAbbr: "UNC",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #4 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 17.2,
    reb: 8.9,
    ast: 2.1,
    age: 25,
    nbaId: 5142800,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142800.png",
    quote: "Versatile forward with elite defensive potential and growing offensive skill."
  },
  {
    id: "draft26-5",
    name: "Keaton Wagler",
    team: "Illinois",
    teamAbbr: "ILL",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #5 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 17.9,
    reb: 3.1,
    ast: 4.4,
    age: 25,
    nbaId: 5142856,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142856.png",
    quote: "Crafty playmaker with great vision and ability to control the tempo."
  },
  {
    id: "draft26-6",
    name: "Darius Acuff Jr.",
    team: "Arkansas",
    teamAbbr: "ARK",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #6 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 18.4,
    reb: 3.9,
    ast: 5.1,
    age: 25,
    nbaId: 5175005,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175005.png",
    quote: "High-octane scorer and distributor heading to Musselman's squad."
  },
  {
    id: "draft26-7",
    name: "Kingston Flemings",
    team: "Houston",
    teamAbbr: "HOU",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #7 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 17.2,
    reb: 3.5,
    ast: 4.8,
    age: 25,
    nbaId: 5174981,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5174981.png",
    quote: "Lockdown defender with explosive transition finishing."
  },
  {
    id: "draft26-8",
    name: "Nate Ament",
    team: "Tennessee",
    teamAbbr: "TEN",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #8 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 14.8,
    reb: 7.2,
    ast: 2.3,
    age: 25,
    nbaId: 5142833,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142833.png",
    quote: "Long, athletic wing with a high ceiling as a two-way force."
  },
  {
    id: "draft26-9",
    name: "Mikel Brown Jr.",
    team: "Louisville",
    teamAbbr: "LOU",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #9 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 19.4,
    reb: 4.1,
    ast: 4.7,
    age: 29,
    nbaId: 5174983,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5174983.png",
    quote: "Dynamic point guard with elite ball-handling and playmaking chops."
  },
  {
    id: "draft26-10",
    name: "Brayden Burries",
    team: "Arizona",
    teamAbbr: "ARI",
    position: "SG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #10 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 15.3,
    reb: 3.8,
    ast: 3.2,
    age: 25,
    nbaId: 5175054,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175054.png",
    quote: "Strong-bodied guard who can overpower defenders and hit outside shots."
  },
  {
    id: "draft26-11",
    name: "Labaron Philon Jr.",
    team: "Alabama",
    teamAbbr: "ALA",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #11 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 21.5,
    reb: 3.2,
    ast: 4.6,
    age: 25,
    nbaId: 5041964,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5041964.png",
    quote: "Lightning quick guard perfectly suited for Alabama's fast-paced system."
  },
  {
    id: "draft26-12",
    name: "Yaxel Lendeborg",
    team: "Michigan",
    teamAbbr: "MCH",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #12 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 15.7,
    reb: 9.4,
    ast: 2.1,
    age: 25,
    nbaId: 4700908,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F4700908.png",
    quote: "Productive big man with strong rebounding and shot-blocking instincts."
  },
  {
    id: "draft26-13",
    name: "Thomas Haugh",
    team: "Florida",
    teamAbbr: "FLA",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #13 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 14.1,
    reb: 6.2,
    ast: 2.4,
    age: 25,
    nbaId: 4683777,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F4683777.png",
    quote: "Skilled wing with good size and shooting ability."
  },
  {
    id: "draft26-14",
    name: "Aday Mara",
    team: "Michigan",
    teamAbbr: "MCH",
    position: "C",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #14 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 12.8,
    reb: 8.1,
    ast: 1.4,
    age: 25,
    nbaId: 5174984,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5174983.png",
    quote: "Spanish giant with elite passing and shot-blocking at 7'3\"."
  },
  {
    id: "draft26-15",
    name: "Hannes Steinbach",
    team: "Washington",
    teamAbbr: "WAS",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #15 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 13.6,
    reb: 7.8,
    ast: 1.9,
    age: 25,
    nbaId: 5175032,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175032.png",
    quote: "German prospect with high motor and versatile inside-out game."
  },
  {
    id: "draft26-16",
    name: "Koa Peat",
    team: "Arizona",
    teamAbbr: "ARI",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #16 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 12.4,
    reb: 6.8,
    ast: 1.6,
    age: 25,
    nbaId: 5175052,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175052.png",
    quote: "Physical forward who plays with toughness and finishes strong at the rim."
  },
  {
    id: "draft26-17",
    name: "Jayden Quaintance",
    team: "Kentucky",
    teamAbbr: "KEN",
    position: "C",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #17 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 11.9,
    reb: 7.4,
    ast: 0.8,
    age: 25,
    nbaId: 5142826,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142826.png",
    quote: "Extremely young big man with massive defensive potential and length."
  },
  {
    id: "draft26-18",
    name: "Chris Cenac Jr.",
    team: "Houston",
    teamAbbr: "HOU",
    position: "C",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #18 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 13.2,
    reb: 8.9,
    ast: 0.9,
    age: 25,
    nbaId: 5042032,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5042032.png",
    quote: "Mobile center with great footwork and interior presence."
  },
  {
    id: "draft26-19",
    name: "Isaiah Evans",
    team: "Duke",
    teamAbbr: "DUK",
    position: "SG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #19 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 14.3,
    reb: 4.1,
    ast: 2.2,
    age: 25,
    nbaId: 5175007,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175007.png",
    quote: "Smooth shooting guard with \"Baby Ingram\" vibes and elite scoring potential."
  },
  {
    id: "draft26-20",
    name: "Ebuka Okorie",
    team: "Stanford",
    teamAbbr: "STA",
    position: "SG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #20 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 16.8,
    reb: 4.3,
    ast: 3.7,
    age: 25,
    nbaId: 5174990,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5174990.png",
    quote: "Nigerian prospect with elite athletic profile and defensive versatility."
  },
  {
    id: "draft26-21",
    name: "Dailyn Swain",
    team: "Texas",
    teamAbbr: "TEX",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #21 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 15.2,
    reb: 5.4,
    ast: 2.8,
    age: 25,
    nbaId: 4901888,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F4901888.png",
    quote: "Long wing with multi-positional defensive ability and playmaking upside."
  },
  {
    id: "draft26-22",
    name: "Bennett Stirtz",
    team: "Iowa",
    teamAbbr: "IOW",
    position: "PG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #22 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 19.2,
    reb: 3.7,
    ast: 5.4,
    age: 25,
    nbaId: 4683815,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F4683815.png",
    quote: "High-IQ floor general with efficient scoring and floor spacing."
  },
  {
    id: "draft26-23",
    name: "Morez Johnson Jr.",
    team: "Michigan",
    teamAbbr: "MCH",
    position: "C",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #23 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 11.2,
    reb: 8.7,
    ast: 0.6,
    age: 25,
    nbaId: 5041996,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5041996.png",
    quote: "Physical center who dominates the glass and protects the paint."
  },
  {
    id: "draft26-24",
    name: "Nikolas Khamenia",
    team: "Duke",
    teamAbbr: "DUK",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #24 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 12.7,
    reb: 5.9,
    ast: 1.3,
    age: 25,
    nbaId: 5175008,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175008.png",
    quote: "Skilled forward with excellent size and perimeter ability."
  },
  {
    id: "draft26-25",
    name: "Christian Anderson",
    team: "Texas Tech",
    teamAbbr: "TTU",
    position: "SG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #25 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 16.3,
    reb: 3.8,
    ast: 3.5,
    age: 25,
    nbaId: 5042020,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5042020.png",
    quote: "Dynamic scorer who can heat up quickly from beyond the arc."
  },
  {
    id: "draft26-26",
    name: "Kam Williams",
    team: "Kentucky",
    teamAbbr: "KEN",
    position: "SF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #26 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 13.4,
    reb: 5.7,
    ast: 1.9,
    age: 24,
    nbaId: 5142825,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5142825.png",
    quote: "Athletic wing with strong defensive engagement and transition scoring."
  },
  {
    id: "draft26-27",
    name: "Braylon Mullins",
    team: "UConn",
    teamAbbr: "CON",
    position: "SG",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #27 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 14.8,
    reb: 4.1,
    ast: 3.9,
    age: 25,
    nbaId: 5175040,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175040.png",
    quote: "Versatile guard with high motor and winning intangibles for UConn."
  },
  {
    id: "draft26-28",
    name: "Henri Veesaar",
    team: "N. Carolina",
    teamAbbr: "UNC",
    position: "C",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #28 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 10.8,
    reb: 7.2,
    ast: 1.1,
    age: 25,
    nbaId: 5175011,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175011.png",
    quote: "Estonian big with modern skill set and stretch potential."
  },
  {
    id: "draft26-29",
    name: "JT Toppin",
    team: "Texas Tech",
    teamAbbr: "TTU",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #29 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 15.8,
    reb: 8.4,
    ast: 1.7,
    age: 25,
    nbaId: 4683786,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F4683786.png",
    quote: "Relentless worker on the glass with high efficiency around the rim."
  },
  {
    id: "draft26-30",
    name: "Karim Lopez",
    team: "México / NBL",
    teamAbbr: "MEX",
    position: "PF",
    rarity: "draft2026" as const,
    category: "Draft 2026" as const,
    subtitle: "PICK #30 • 2026 PROSPECT",
    series: "2026 Prospect Class",
    isHistorical: false,
    pts: 14.2,
    reb: 7.6,
    ast: 1.8,
    age: 37,
    nbaId: 5175095,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fmens-college-basketball%2Fplayers%2Ffull%2F5175095.png",
    quote: "Elite Mexican prospect developing in the NBL Next Stars program."
  },
  {
    id: "score-1980",
    name: "George Gervin",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1979-80 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1979-80",
    isHistorical: true,
    pts: 33.1,
    reb: 5.2,
    ast: 2.9,
    age: 21,
    nbaId: 76804,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/gervige01.jpg",
    quote: "Coolness under pressure and the most fluid finger roll in history."
  },
  {
    id: "score-1981",
    name: "Adrian Dantley",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1980-81 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1980-81",
    isHistorical: true,
    pts: 30.7,
    reb: 7,
    ast: 2.2,
    age: 25,
    nbaId: 76505,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/dantlad01.jpg",
    quote: "The most efficient volume scorer of his era."
  },
  {
    id: "score-1982",
    name: "George Gervin",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1981-82 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1981-82",
    isHistorical: true,
    pts: 32.3,
    reb: 4.6,
    ast: 2.8,
    age: 21,
    nbaId: 76804,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/gervige01.jpg",
    quote: "If you can stay cool, you can win."
  },
  {
    id: "score-1983",
    name: "Alex English",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1982-83 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1982-83",
    isHistorical: true,
    pts: 28.4,
    reb: 6,
    ast: 3.7,
    age: 25,
    nbaId: 76673,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/englial01.jpg",
    quote: "Poetry in motion on the basketball court."
  },
  {
    id: "score-1984",
    name: "Adrian Dantley",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1983-84 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1983-84",
    isHistorical: true,
    pts: 30.6,
    reb: 6,
    ast: 2.8,
    age: 25,
    nbaId: 76505,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/dantlad01.jpg",
    quote: "Methodical, relentless, and impossible to stop without fouling."
  },
  {
    id: "score-1985",
    name: "Bernard King",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1984-85 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1984-85",
    isHistorical: true,
    pts: 32.9,
    reb: 5.8,
    ast: 3,
    age: 25,
    nbaId: 77263,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/kingbe01.jpg",
    quote: "When he was on, there was absolutely nobody who could guard him."
  },
  {
    id: "score-1986",
    name: "Dominique Wilkins",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1985-86 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1985-86",
    isHistorical: true,
    pts: 30.3,
    reb: 7.8,
    ast: 2.7,
    age: 25,
    nbaId: 78534,
    imageUrl: "https://www.basketball-reference.com/req/202106291/images/headshots/wilkido01.jpg",
    quote: "Gravity was merely a suggestion to Dominique."
  },
  {
    id: "score-1987",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1986-87 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1986-87",
    isHistorical: true,
    pts: 37.1,
    reb: 5.2,
    ast: 4.6,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "I can accept failure, everyone fails at something. But I can't accept not trying."
  },
  {
    id: "score-1988",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1987-88 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1987-88",
    isHistorical: true,
    pts: 35,
    reb: 5.5,
    ast: 5.9,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "Talent wins games, but teamwork and intelligence win championships."
  },
  {
    id: "score-1989",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1988-89 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1988-89",
    isHistorical: true,
    pts: 32.5,
    reb: 8,
    ast: 8,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "You must expect great things of yourself before you can do them."
  },
  {
    id: "score-1990",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1989-90 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1989-90",
    isHistorical: true,
    pts: 33.6,
    reb: 6.9,
    ast: 6.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "The key to success is failure."
  },
  {
    id: "score-1991",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1990-91 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1990-91",
    isHistorical: true,
    pts: 31.5,
    reb: 6,
    ast: 5.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "I realized that if I was going to achieve anything in life I had to be aggressive."
  },
  {
    id: "score-1992",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1991-92 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1991-92",
    isHistorical: true,
    pts: 30.1,
    reb: 6.4,
    ast: 6.1,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "Limits, like fears, are often just an illusion."
  },
  {
    id: "score-1993",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1992-93 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1992-93",
    isHistorical: true,
    pts: 32.6,
    reb: 6.7,
    ast: 5.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "Obstacles don't have to stop you."
  },
  {
    id: "score-1994",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1993-94 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1993-94",
    isHistorical: true,
    pts: 29.8,
    reb: 10.7,
    ast: 4.8,
    age: 25,
    nbaId: 95,
    imageUrl: "https://sanantoniosports.org/wp-content/uploads/2024/04/Running-1.jpg",
    quote: "Leadership is a high toll to pay."
  },
  {
    id: "score-1995",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1994-95 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1994-95",
    isHistorical: true,
    pts: 27.6,
    reb: 10.8,
    ast: 2.9,
    age: 25,
    nbaId: 95,
    imageUrl: "https://sanantoniosports.org/wp-content/uploads/2024/04/Running-1.jpg",
    quote: "You can't take anything for granted."
  },
  {
    id: "score-1996",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1995-96 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1995-96",
    isHistorical: true,
    pts: 30.4,
    reb: 6.6,
    ast: 4.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "Step by step. I can't see any other way of accomplishing anything."
  },
  {
    id: "score-1997",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1996-97 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1996-97",
    isHistorical: true,
    pts: 29.6,
    reb: 5.9,
    ast: 4.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "To learn to succeed, you must first learn to fail."
  },
  {
    id: "score-1998",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1997-98 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1997-98",
    isHistorical: true,
    pts: 28.7,
    reb: 5.8,
    ast: 3.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.britannica.com/96/195196-050-3909D5BD/Michael-Jordan-1988.jpg",
    quote: "Always turn a negative situation into a positive situation."
  },
  {
    id: "score-1999",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1998-99 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1998-99",
    isHistorical: true,
    pts: 26.8,
    reb: 4.9,
    ast: 4.6,
    age: 27,
    nbaId: 947,
    imageUrl: "https://i.ytimg.com/vi/2REkZG-dhHc/hqdefault.jpg",
    quote: "I don't want to be the next Michael Jordan, I only want to be Allen Iverson."
  },
  {
    id: "score-2000",
    name: "Shaquille O'Neal",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "1999-00 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "1999-00",
    isHistorical: true,
    pts: 29.7,
    reb: 13.6,
    ast: 3.8,
    age: 25,
    nbaId: 406,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F614.png",
    quote: "Me shooting 40% at the foul line is just God's way to say nobody's perfect."
  },
  {
    id: "score-2001",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2000-01 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2000-01",
    isHistorical: true,
    pts: 31.1,
    reb: 3.8,
    ast: 4.6,
    age: 27,
    nbaId: 947,
    imageUrl: "https://i.ytimg.com/vi/2REkZG-dhHc/hqdefault.jpg",
    quote: "I play every game like it's my last."
  },
  {
    id: "score-2002",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2001-02 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2001-02",
    isHistorical: true,
    pts: 31.4,
    reb: 3.5,
    ast: 5.5,
    age: 27,
    nbaId: 947,
    imageUrl: "https://i.ytimg.com/vi/2REkZG-dhHc/hqdefault.jpg",
    quote: "I gave everything I had to basketball."
  },
  {
    id: "score-2003",
    name: "Tracy McGrady",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2002-03 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2002-03",
    isHistorical: true,
    pts: 32.1,
    reb: 6.5,
    ast: 5.5,
    age: 25,
    nbaId: 1515,
    imageUrl: "https://platform.sbnation.com/wp-content/uploads/sites/2/chorus/uploads/chorus_asset/file/9202649/2733366.jpg?quality=90&strip=all&crop=2.925%2C0%2C94.15%2C100&w=2400",
    quote: "I was born to score."
  },
  {
    id: "score-2004",
    name: "Tracy McGrady",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2003-04 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2003-04",
    isHistorical: true,
    pts: 28,
    reb: 6,
    ast: 5.5,
    age: 25,
    nbaId: 1515,
    imageUrl: "https://platform.sbnation.com/wp-content/uploads/sites/2/chorus/uploads/chorus_asset/file/9202649/2733366.jpg?quality=90&strip=all&crop=2.925%2C0%2C94.15%2C100&w=2400",
    quote: "No one is unbeatable."
  },
  {
    id: "score-2005",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2004-05 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2004-05",
    isHistorical: true,
    pts: 30.7,
    reb: 3.7,
    ast: 7.9,
    age: 27,
    nbaId: 947,
    imageUrl: "https://i.ytimg.com/vi/2REkZG-dhHc/hqdefault.jpg",
    quote: "Only the strong survive."
  },
  {
    id: "score-2006",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2005-06 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2005-06",
    isHistorical: true,
    pts: 35.4,
    reb: 5.3,
    ast: 4.5,
    age: 25,
    nbaId: 977,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F110.png",
    quote: "Everything negative - pressure, challenges - is all an opportunity for me to rise."
  },
  {
    id: "score-2007",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2006-07 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2006-07",
    isHistorical: true,
    pts: 31.6,
    reb: 5.7,
    ast: 5.4,
    age: 25,
    nbaId: 977,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F110.png",
    quote: "The most important thing is to try and inspire people so that they can be great in whatever they want to do."
  },
  {
    id: "score-2008",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2007-08 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2007-08",
    isHistorical: true,
    pts: 30,
    reb: 7.9,
    ast: 7.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F1966.png",
    quote: "I like being poked. I don't like to be poked that many times."
  },
  {
    id: "score-2009",
    name: "Dwyane Wade",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2008-09 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2008-09",
    isHistorical: true,
    pts: 30.2,
    reb: 5,
    ast: 7.5,
    age: 25,
    nbaId: 2548,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F1987.png",
    quote: "My nickname is Flash for a reason."
  },
  {
    id: "score-2010",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2009-10 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2009-10",
    isHistorical: true,
    pts: 30.1,
    reb: 7.6,
    ast: 2.8,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3202.png",
    quote: "Hard work beats talent when talent fails to work hard."
  },
  {
    id: "score-2011",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2010-11 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2010-11",
    isHistorical: true,
    pts: 27.7,
    reb: 6.8,
    ast: 2.7,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3202.png",
    quote: "I've just gotta keep working."
  },
  {
    id: "score-2012",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2011-12 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2011-12",
    isHistorical: true,
    pts: 28,
    reb: 8,
    ast: 3.5,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3202.png",
    quote: "I'm just trying to be the best I can be."
  },
  {
    id: "score-2013",
    name: "Carmelo Anthony",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2012-13 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2012-13",
    isHistorical: true,
    pts: 28.7,
    reb: 6.9,
    ast: 2.6,
    age: 25,
    nbaId: 2546,
    imageUrl: "https://media.cnn.com/api/v1/images/stellar/prod/230522103741-03-carmelo-anthony-file.jpg?c=original",
    quote: "Stay Me7o."
  },
  {
    id: "score-2014",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "SF",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2013-14 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2013-14",
    isHistorical: true,
    pts: 32,
    reb: 7.4,
    ast: 5.5,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3202.png",
    quote: "You the real MVP."
  },
  {
    id: "score-2015",
    name: "Russell Westbrook",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2014-15 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2014-15",
    isHistorical: true,
    pts: 28.1,
    reb: 7.3,
    ast: 8.6,
    age: 25,
    nbaId: 201566,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3468.png",
    quote: "Why Not?"
  },
  {
    id: "score-2016",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2015-16 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2015-16",
    isHistorical: true,
    pts: 30.1,
    reb: 5.4,
    ast: 6.7,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3975.png",
    quote: "I can do all things through Christ who strengthens me."
  },
  {
    id: "score-2017",
    name: "Russell Westbrook",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2016-17 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2016-17",
    isHistorical: true,
    pts: 31.6,
    reb: 10.7,
    ast: 10.4,
    age: 25,
    nbaId: 201566,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3468.png",
    quote: "I'm competitive. I love to play the game."
  },
  {
    id: "score-2018",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2017-18 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2017-18",
    isHistorical: true,
    pts: 30.4,
    reb: 5.4,
    ast: 8.8,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3992.png",
    quote: "Fear the Beard."
  },
  {
    id: "score-2019",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2018-19 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2018-19",
    isHistorical: true,
    pts: 36.1,
    reb: 6.6,
    ast: 7.5,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3992.png",
    quote: "They're going to double-team me? Well, we've got options."
  },
  {
    id: "score-2020",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2019-20 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2019-20",
    isHistorical: true,
    pts: 34.3,
    reb: 6.4,
    ast: 7.4,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3992.png",
    quote: "Basketball is what I do."
  },
  {
    id: "score-2021",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2020-21 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2020-21",
    isHistorical: true,
    pts: 32,
    reb: 5.5,
    ast: 5.8,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3975.png",
    quote: "I'm still here. I'm still doing it."
  },
  {
    id: "score-2022",
    name: "Joel Embiid",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "C",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2021-22 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2021-22",
    isHistorical: true,
    pts: 30.6,
    reb: 11.7,
    ast: 4.2,
    age: 31,
    nbaId: 203954,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3059318.png",
    quote: "Trust the Process."
  },
  {
    id: "score-2023",
    name: "Joel Embiid",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "C",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2022-23 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2022-23",
    isHistorical: true,
    pts: 33.1,
    reb: 10.2,
    ast: 4.2,
    age: 31,
    nbaId: 203954,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3059318.png",
    quote: "Only the best ever did this."
  },
  {
    id: "score-2024",
    name: "Luka Dončić",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2023-24 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2023-24",
    isHistorical: true,
    pts: 33.9,
    reb: 9.2,
    ast: 9.8,
    age: 25,
    nbaId: 1629029,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3945274.png",
    quote: "Magic is real."
  },
  {
    id: "score-2025",
    name: "Shai Gilgeous-Alexander",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "SG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2024-25 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2024-25",
    isHistorical: false,
    pts: 31.1,
    reb: 4.3,
    ast: 6.6,
    age: 27,
    nbaId: 1628983,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F4278073.png",
    quote: "Smooth as silk, cold as ice."
  },
  {
    id: "score-2026",
    name: "Luka Dončić",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "PG",
    rarity: "scoring_champ" as const,
    category: "Scoring Champion" as const,
    subtitle: "2025-26 SCORING CHAMPION",
    series: "NBA Scoring Champions",
    year: "2025-26",
    isHistorical: false,
    pts: 33.5,
    reb: 7.7,
    ast: 8.3,
    age: 25,
    nbaId: 1629029,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3945274.png",
    quote: "Hollywood was ready for its new leading man."
  },
  {
    id: "hof-1980a",
    name: "Jerry West",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1980 • HOF",
    series: "NBA Hall of Fame",
    year: "1980",
    isHistorical: true,
    pts: 27,
    reb: 5.8,
    ast: 6.7,
    age: 25,
    nbaId: 600015,
    imageUrl: "https://objetos-xlk.estaticos-marca.com/uploads/2024/07/05/17182013983349.png",
    quote: "The Logo."
  },
  {
    id: "hof-1980b",
    name: "Oscar Robertson",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1980 • HOF",
    series: "NBA Hall of Fame",
    year: "1980",
    isHistorical: true,
    pts: 25.7,
    reb: 7.5,
    ast: 9.5,
    age: 25,
    nbaId: 600007,
    imageUrl: "https://www.myblackhistory.net/oscar.jpg",
    quote: "The Big O."
  },
  {
    id: "hof-1980c",
    name: "Wilt Chamberlain",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1980 • HOF",
    series: "NBA Hall of Fame",
    year: "1980",
    isHistorical: true,
    pts: 30.1,
    reb: 22.9,
    ast: 4.4,
    age: 25,
    nbaId: 600004,
    imageUrl: "https://encrypted-tbn1.gstatic.com/licensed-image?q=tbn:ANd9GcRRG5sGbCA8NDw_J7RM1iTDDre5aYzg96DHVOhyq8mxWAlERnifnOksiPkC1ca_-6GfoGR4PECQp99_1uU",
    quote: "The Big Dipper."
  },
  {
    id: "hof-1981",
    name: "Bill Russell",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1981 • HOF",
    series: "NBA Hall of Fame",
    year: "1981",
    isHistorical: true,
    pts: 15.1,
    reb: 22.5,
    ast: 4.3,
    age: 25,
    nbaId: 600003,
    imageUrl: "https://www.hollywoodreporter.com/wp-content/uploads/2022/01/Bill-Russell-Boston-1950s-Everett-P8DBIRU_EC001-EMBED-2022.jpg",
    quote: "The ultimate winner."
  },
  {
    id: "hof-1983",
    name: "Elgin Baylor",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1983 • HOF",
    series: "NBA Hall of Fame",
    year: "1983",
    isHistorical: true,
    pts: 27.4,
    reb: 13.5,
    ast: 4.3,
    age: 25,
    nbaId: 600008,
    imageUrl: "https://people.com/thmb/A985skBp-7CoPESTJY01mMG8XBo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc():focal(750x379:752x381)/elgin-baylor-1-2000-9afb8cf311fb480fbbe2b12f31a66bf5.jpg",
    quote: "The first great high-flyer."
  },
  {
    id: "hof-1990",
    name: "Earl Monroe",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1990 • HOF",
    series: "NBA Hall of Fame",
    year: "1990",
    isHistorical: true,
    pts: 18.8,
    reb: 3,
    ast: 3.9,
    age: 25,
    nbaId: 600019,
    imageUrl: "https://www.nbamaniacs.com/wp-content/uploads/2022/07/GettyImages-455539916-1568x882-1.jpg",
    quote: "Earl the Pearl."
  },
  {
    id: "hof-1993",
    name: "Walt Frazier",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1993 • HOF",
    series: "NBA Hall of Fame",
    year: "1993",
    isHistorical: true,
    pts: 18.9,
    reb: 5.9,
    ast: 6.1,
    age: 25,
    nbaId: 600016,
    imageUrl: "https://cdn.britannica.com/07/258007-050-4F67E33A/Basketball-player-Walt-Frazier-dribbles-the-ball-against-the-Baltimore-Bullets-during-an-NBA-basketball-game-1971.jpg",
    quote: "Clyde."
  },
  {
    id: "hof-1995",
    name: "Kareem Abdul-Jabbar",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1995 • HOF",
    series: "NBA Hall of Fame",
    year: "1995",
    isHistorical: true,
    pts: 24.6,
    reb: 11.2,
    ast: 3.6,
    age: 25,
    nbaId: 600001,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/d/de/Kareem_Abdul-Jabbar_1974.jpeg/250px-Kareem_Abdul-Jabbar_1974.jpeg",
    quote: "Cap. Master of the Skyhook."
  },
  {
    id: "hof-1996",
    name: "Julius Erving",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1996 • HOF",
    series: "NBA Hall of Fame",
    year: "1996",
    isHistorical: true,
    pts: 22,
    reb: 8.5,
    ast: 4.2,
    age: 25,
    nbaId: 600013,
    imageUrl: "https://cdn.britannica.com/14/198714-050-2883B189/Julius-Erving.jpg",
    quote: "Dr. J."
  },
  {
    id: "hof-1998",
    name: "Larry Bird",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 1998 • HOF",
    series: "NBA Hall of Fame",
    year: "1998",
    isHistorical: true,
    pts: 24.3,
    reb: 10,
    ast: 6.3,
    age: 25,
    nbaId: 2182,
    imageUrl: "https://cdn.britannica.com/60/258260-050-35DB0395/larry-bird-of-boston-celtics-shoots-free-throw-against-the-washington-bullets-1985.jpg",
    quote: "Larry Legend."
  },
  {
    id: "hof-2002",
    name: "Magic Johnson",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2002 • HOF",
    series: "NBA Hall of Fame",
    year: "2002",
    isHistorical: true,
    pts: 19.5,
    reb: 7.2,
    ast: 11.2,
    age: 25,
    nbaId: 600025,
    imageUrl: "https://cdn.britannica.com/27/189527-050-5BAD12C2/Magic-Johnson.jpg",
    quote: "Showtime."
  },
  {
    id: "hof-2006a",
    name: "Charles Barkley",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2006 • HOF",
    series: "NBA Hall of Fame",
    year: "2006",
    isHistorical: true,
    pts: 22.1,
    reb: 11.7,
    ast: 3.9,
    age: 25,
    nbaId: 787,
    imageUrl: "https://cdn.nba.com/manage/2021/09/charlesbarkley-1568x882.jpg",
    quote: "Sir Charles."
  },
  {
    id: "hof-2006b",
    name: "Scotty Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2006 • HOF",
    series: "NBA Hall of Fame",
    year: "2006",
    isHistorical: true,
    pts: 16.1,
    reb: 6.4,
    ast: 5.2,
    age: 25,
    nbaId: 600039,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSHvd5FDEr5Us_6Wr26gB03J4HO0kJSkET8EyikZ0PVGgcKncbGOdaT83zEyCkg8vdIw-171rgSo4bUq01f5UMG5EfCWM5Y_d4zFTbtww&s=10",
    quote: "One of the greatest perimeter defenders."
  },
  {
    id: "hof-2006c",
    name: "Patrick Ewing",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2006 • HOF",
    series: "NBA Hall of Fame",
    year: "2006",
    isHistorical: true,
    pts: 21,
    reb: 9.8,
    ast: 2.9,
    age: 25,
    nbaId: 600036,
    imageUrl: "https://tuscamisetasnba.com/wp-content/uploads/curiosidades-patrick-ewing.jpg",
    quote: "The heart of the Knicks."
  },
  {
    id: "hof-2006d",
    name: "Joe Dumars",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2006 • HOF",
    series: "NBA Hall of Fame",
    year: "2006",
    isHistorical: true,
    pts: 16.1,
    reb: 3.1,
    ast: 4.5,
    age: 25,
    nbaId: 600033,
    imageUrl: "https://michigansportshof.org/wp-content/uploads/2019/01/joe-dumars.jpg",
    quote: "A class act and a tough defender."
  },
  {
    id: "hof-2007",
    name: "Clyde Drexler",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2007 • HOF",
    series: "NBA Hall of Fame",
    year: "2007",
    isHistorical: true,
    pts: 20.4,
    reb: 6.1,
    ast: 5.6,
    age: 25,
    nbaId: 600031,
    imageUrl: "https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/clyde-drexler-dick-raphael.jpg",
    quote: "Clyde the Glide."
  },
  {
    id: "hof-2008",
    name: "Hakeem Olajuwon",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2008 • HOF",
    series: "NBA Hall of Fame",
    year: "2008",
    isHistorical: true,
    pts: 21.8,
    reb: 11.1,
    ast: 2.5,
    age: 25,
    nbaId: 600037,
    imageUrl: "https://cdn.nba.com/manage/2017/08/hakeem-olajuwon-iso-stands.jpg",
    quote: "The Dream."
  },
  {
    id: "hof-2009a",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2009 • HOF",
    series: "NBA Hall of Fame",
    year: "2009",
    isHistorical: true,
    pts: 30.1,
    reb: 6.2,
    ast: 5.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://static.wikia.nocookie.net/nbastreet/images/9/97/FDB1BF72-3F75-446F-B4F2-000331AE638B.jpeg/revision/latest?cb=20210419024456",
    quote: "The GOAT."
  },
  {
    id: "hof-2009b",
    name: "John Stockton",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2009 • HOF",
    series: "NBA Hall of Fame",
    year: "2009",
    isHistorical: true,
    pts: 13.1,
    reb: 2.7,
    ast: 10.5,
    age: 25,
    nbaId: 600043,
    imageUrl: "https://platform.slcdunk.com/wp-content/uploads/sites/145/chorus/uploads/chorus_asset/file/15987854/1500510.jpg.jpg?quality=90&strip=all&crop=0,4.1508916323731,100,65.953360768176",
    quote: "The master of the assist."
  },
  {
    id: "hof-2009c",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2009 • HOF",
    series: "NBA Hall of Fame",
    year: "2009",
    isHistorical: true,
    pts: 25,
    reb: 10.1,
    ast: 3.6,
    age: 25,
    nbaId: 600044,
    imageUrl: "https://cdn.britannica.com/38/256938-050-D1B6AF47/Karl-Malone-NBA-Utah-Jazz-basketball-player.jpg",
    quote: "The Mailman."
  },
  {
    id: "hof-2010",
    name: "Scottie Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2010 • HOF",
    series: "NBA Hall of Fame",
    year: "2010",
    isHistorical: true,
    pts: 16.1,
    reb: 6.4,
    ast: 5.2,
    age: 25,
    nbaId: 600039,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/e/e4/Lipofsky_Pippen.jpg/250px-Lipofsky_Pippen.jpg",
    quote: "Pip."
  },
  {
    id: "hof-2011",
    name: "Dennis Rodman",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2011 • HOF",
    series: "NBA Hall of Fame",
    year: "2011",
    isHistorical: true,
    pts: 7.3,
    reb: 13.1,
    ast: 1.8,
    age: 25,
    nbaId: 600047,
    imageUrl: "https://cdn.gigantes.com/wp-content/uploads/sites/4/2020/03/rodman-a.jpg",
    quote: "The Worm."
  },
  {
    id: "hof-2012",
    name: "Reggie Miller",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2012 • HOF",
    series: "NBA Hall of Fame",
    year: "2012",
    isHistorical: true,
    pts: 18.2,
    reb: 3,
    ast: 3,
    age: 22,
    nbaId: 714,
    imageUrl: "https://m.media-amazon.com/images/M/MV5BMTUxODg1Nzg5OV5BMl5BanBnXkFtZTcwNTMzMjkwMw@@._V1_.jpg",
    quote: "Miller Time."
  },
  {
    id: "hof-2013",
    name: "Gary Payton",
    team: "Seattle SuperSonics",
    teamAbbr: "SEA",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2013 • HOF",
    series: "NBA Hall of Fame",
    year: "2013",
    isHistorical: true,
    pts: 16.3,
    reb: 4.3,
    ast: 6.7,
    age: 25,
    nbaId: 600053,
    imageUrl: "https://static.wikia.nocookie.net/nba/images/b/b1/Gary-payton-net-worth2.jpg/revision/latest?cb=20170302235016",
    quote: "The Glove."
  },
  {
    id: "hof-2014a",
    name: "Mitch Richmond",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2014 • HOF",
    series: "NBA Hall of Fame",
    year: "2014",
    isHistorical: true,
    pts: 21,
    reb: 3.8,
    ast: 3.3,
    age: 25,
    nbaId: 600055,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTj3KHBaH0KsTzdh1xRWPUVGyu8BGBxMvX-Yg&s",
    quote: "The Rock."
  },
  {
    id: "hof-2014b",
    name: "Alonzo Mourning",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2014 • HOF",
    series: "NBA Hall of Fame",
    year: "2014",
    isHistorical: true,
    pts: 17.1,
    reb: 8.5,
    ast: 1.2,
    age: 25,
    nbaId: 600057,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/b/b7/Alonzo_Mourning.jpg",
    quote: "Zo."
  },
  {
    id: "hof-2016a",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2016 • HOF",
    series: "NBA Hall of Fame",
    year: "2016",
    isHistorical: true,
    pts: 26.7,
    reb: 3.7,
    ast: 6.2,
    age: 27,
    nbaId: 190,
    imageUrl: "https://encrypted-tbn1.gstatic.com/licensed-image?q=tbn:ANd9GcTVyN6kTMpgFUTFFFrnalJeT_f_SrXOX1dUEakW1Hexnc_q-jhU7QM66W-DuiIwFHYICYxIxOVLDwGzE4E",
    quote: "The Answer."
  },
  {
    id: "hof-2016b",
    name: "Yao Ming",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2016 • HOF",
    series: "NBA Hall of Fame",
    year: "2016",
    isHistorical: true,
    pts: 19,
    reb: 9.2,
    ast: 1.9,
    age: 25,
    nbaId: 2397,
    imageUrl: "https://bucket-image.inkmaginecms.com/version/social/1/image/2024/05/d9421767-62e7-4ffe-a862-1a0838d34131.jpg",
    quote: "Great Wall of Yao."
  },
  {
    id: "hof-2016c",
    name: "Shaquille O'Neal",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2016 • HOF",
    series: "NBA Hall of Fame",
    year: "2016",
    isHistorical: true,
    pts: 23.7,
    reb: 10.9,
    ast: 2.5,
    age: 25,
    nbaId: 406,
    imageUrl: "https://i.pinimg.com/474x/8a/6a/77/8a6a77ce4bdf658b627a779c83cb0bf0.jpg",
    quote: "Shaq Diesel."
  },
  {
    id: "hof-2017",
    name: "Tracy McGrady",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2017 • HOF",
    series: "NBA Hall of Fame",
    year: "2017",
    isHistorical: true,
    pts: 19.6,
    reb: 5.6,
    ast: 4.4,
    age: 25,
    nbaId: 1515,
    imageUrl: "https://images2.minutemediacdn.com/image/upload/c_fill,w_1200,ar_1:1,f_auto,q_auto,g_auto/shape/cover/sport/d4627c41abd99f569b157d269181284b7029b07099e76992c8f1d2a2839f0828.jpg",
    quote: "T-Mac."
  },
  {
    id: "hof-2018a",
    name: "Steve Nash",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2018 • HOF",
    series: "NBA Hall of Fame",
    year: "2018",
    isHistorical: true,
    pts: 14.3,
    reb: 3,
    ast: 8.5,
    age: 25,
    nbaId: 201565,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/2/2f/Steve_Nash_00054544.jpg/250px-Steve_Nash_00054544.jpg",
    quote: "Point God Nash."
  },
  {
    id: "hof-2018b",
    name: "Ray Allen",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2018 • HOF",
    series: "NBA Hall of Fame",
    year: "2018",
    isHistorical: true,
    pts: 18.9,
    reb: 4.1,
    ast: 3.4,
    age: 27,
    nbaId: 947,
    imageUrl: "https://platform.sbnation.com/wp-content/uploads/sites/2/chorus/uploads/chorus_asset/file/8539251/52951161.jpg?quality=90&strip=all&crop=0%2C0%2C100%2C94.392104082548&w=2400",
    quote: "Jesus Shuttlesworth. The perfect jumper."
  },
  {
    id: "hof-2020a",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2020 • HOF",
    series: "NBA Hall of Fame",
    year: "2020",
    isHistorical: true,
    pts: 25,
    reb: 5.2,
    ast: 4.7,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.artphotolimited.com/images/67ceea65865e9b3b9ef7de2b/300x300/kobe-bryant-2001-nba-finals.jpg",
    quote: "Mamba Out."
  },
  {
    id: "hof-2020b",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2020 • HOF",
    series: "NBA Hall of Fame",
    year: "2020",
    isHistorical: true,
    pts: 19,
    reb: 10.8,
    ast: 3,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/1/1d/Tim_Duncan_Walks_Verizon_Center%27s_Floor_%28cropped%29.jpg/960px-Tim_Duncan_Walks_Verizon_Center%27s_Floor_%28cropped%29.jpg",
    quote: "The Big Fundamental."
  },
  {
    id: "hof-2020c",
    name: "Kevin Garnett",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2020 • HOF",
    series: "NBA Hall of Fame",
    year: "2020",
    isHistorical: true,
    pts: 17.8,
    reb: 10,
    ast: 3.7,
    age: 25,
    nbaId: 708,
    imageUrl: "https://i.redd.it/4va5s86dnvje1.jpeg",
    quote: "Anything is possible!"
  },
  {
    id: "hof-2021a",
    name: "Chris Webber",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2021 • HOF",
    series: "NBA Hall of Fame",
    year: "2021",
    isHistorical: true,
    pts: 20.7,
    reb: 9.8,
    ast: 4.2,
    age: 25,
    nbaId: 185,
    imageUrl: "https://static.wikia.nocookie.net/nba-contrafactic/images/4/4f/Webber.jpg/revision/latest?cb=20210716183716&path-prefix=es",
    quote: "C-Webb."
  },
  {
    id: "hof-2021b",
    name: "Paul Pierce",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2021 • HOF",
    series: "NBA Hall of Fame",
    year: "2021",
    isHistorical: true,
    pts: 19.7,
    reb: 5.6,
    ast: 3.5,
    age: 40,
    nbaId: 1718,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/00/Paul_pierce_1.jpg/960px-Paul_pierce_1.jpg",
    quote: "The Truth."
  },
  {
    id: "hof-2021c",
    name: "Ben Wallace",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2021 • HOF",
    series: "NBA Hall of Fame",
    year: "2021",
    isHistorical: true,
    pts: 5.7,
    reb: 9.6,
    ast: 1.3,
    age: 25,
    nbaId: 1111,
    imageUrl: "https://media-cldnry.s-nbcnews.com/image/upload/t_social_share_1024x768_scale,f_auto,q_auto:best/msnbc/Components/Photos/050610/050610_ben_wallace_hmed.jpg",
    quote: "Big Ben."
  },
  {
    id: "hof-2022a",
    name: "Manu Ginóbili",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2022 • HOF",
    series: "NBA Hall of Fame",
    year: "2022",
    isHistorical: true,
    pts: 13.3,
    reb: 3.5,
    ast: 3.8,
    age: 25,
    nbaId: 1938,
    imageUrl: "https://fotos.perfil.com/2022/04/02/trim/720/410/ginobili-1335881.jpg",
    quote: "The Eurostep pioneer."
  },
  {
    id: "hof-2022b",
    name: "Tim Hardaway",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2022 • HOF",
    series: "NBA Hall of Fame",
    year: "2022",
    isHistorical: true,
    pts: 17.7,
    reb: 3.4,
    ast: 7.8,
    age: 25,
    nbaId: 88,
    imageUrl: "https://imageio.forbes.com/specials-images/imageserve/65d25debd91f185c5cac9977/0x0.jpg?format=jpg&crop=3596,2022,x0,y22,safe&height=900&width=1600&fit=bounds",
    quote: "The UTEP Two-Step."
  },
  {
    id: "hof-2023a",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2023 • HOF",
    series: "NBA Hall of Fame",
    year: "2023",
    isHistorical: true,
    pts: 20.7,
    reb: 7.5,
    ast: 2.4,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/09/Dirk_Nowitzki_2.jpg/250px-Dirk_Nowitzki_2.jpg",
    quote: "The greatest European player."
  },
  {
    id: "hof-2023b",
    name: "Tony Parker",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2023 • HOF",
    series: "NBA Hall of Fame",
    year: "2023",
    isHistorical: true,
    pts: 15.5,
    reb: 2.9,
    ast: 5.6,
    age: 25,
    nbaId: 2225,
    imageUrl: "https://tuscamisetasnba.com/wp-content/uploads/curiosidaes-tony-parker.jpg",
    quote: "The blur."
  },
  {
    id: "hof-2023c",
    name: "Dwyane Wade",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2023 • HOF",
    series: "NBA Hall of Fame",
    year: "2023",
    isHistorical: true,
    pts: 22,
    reb: 4.7,
    ast: 5.4,
    age: 25,
    nbaId: 2548,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/7/73/Dwyane_Wade_e1.jpg/1280px-Dwyane_Wade_e1.jpg",
    quote: "Flash."
  },
  {
    id: "hof-2023d",
    name: "Pau Gasol",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2023 • HOF",
    series: "NBA Hall of Fame",
    year: "2023",
    isHistorical: true,
    pts: 17,
    reb: 8.9,
    ast: 3.3,
    age: 25,
    nbaId: 2203,
    imageUrl: "https://sport.jotdown.es/wp-content/uploads/2025/05/Proyecto-nuevo-2025-05-11T180514.769.jpg",
    quote: "The heart and mind of a champion."
  },
  {
    id: "hof-2024a",
    name: "Vince Carter",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "SG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2024 • HOF",
    series: "NBA Hall of Fame",
    year: "2024",
    isHistorical: true,
    pts: 16.7,
    reb: 4.3,
    ast: 3.1,
    age: 25,
    nbaId: 1713,
    imageUrl: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRMQY_uW3wQ2DZsaR9zetur2_CH_rmW5YoCfcKRWadiblodTAQ3bHNB3H1IO0QuTp4i95kPGX40Dz5WfakgpEA7FNAhYX64O07eYDmUjqo&s=10",
    quote: "Half-Man, Half-Amazing."
  },
  {
    id: "hof-2024b",
    name: "Chauncey Billups",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "PG",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2024 • HOF",
    series: "NBA Hall of Fame",
    year: "2024",
    isHistorical: true,
    pts: 15.6,
    reb: 3,
    ast: 5.7,
    age: 25,
    nbaId: 1497,
    imageUrl: "https://static01.nyt.com/images/2025/10/23/multimedia/23met-nba-gambling-billups-bio-top-fqwl/23met-nba-gambling-billups-bio-top-fqwl-articleLarge.jpg?quality=75&auto=webp&disable=upscale",
    quote: "Mr. Big Shot."
  },
  {
    id: "hof-2025a",
    name: "Carmelo Anthony",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "SF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2025 • HOF",
    series: "NBA Hall of Fame",
    year: "2025",
    isHistorical: true,
    pts: 22.5,
    reb: 6.2,
    ast: 2.7,
    age: 25,
    nbaId: 2546,
    imageUrl: "https://imageio.forbes.com/specials-images/imageserve/646b9b77976ffbea12b502f7/Denver-Nuggets-v-Los-Angeles-Lakers--Game-2/0x0.jpg?crop=1998,2996,x0,y0,safe&height=1066&width=711&fit=bounds",
    quote: "Stay Me7o."
  },
  {
    id: "hof-2025b",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2025 • HOF",
    series: "NBA Hall of Fame",
    year: "2025",
    isHistorical: true,
    pts: 17.1,
    reb: 12.5,
    ast: 1.5,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/7/76/Dwight_Howard.jpg",
    quote: "Superman."
  },
  {
    id: "hof-2026",
    name: "Amar'e Stoudemire",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "PF",
    rarity: "hof" as const,
    category: "Hall of Fame" as const,
    subtitle: "CLASS OF 2026 • HOF",
    series: "NBA Hall of Fame",
    year: "2026",
    isHistorical: true,
    pts: 18.9,
    reb: 8,
    ast: 1.8,
    age: 25,
    nbaId: 2403,
    imageUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8a/Amar%27e_Stoudemire_free_throw.jpg/250px-Amar%27e_Stoudemire_free_throw.jpg",
    quote: "STAT."
  },
  {
    id: "coy-1980",
    name: "Bill Fitch",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1979-80 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1979-80",
    isHistorical: true,
    pts: 61,
    reb: 21,
    ast: 0,
    age: 25,
    nbaId: 600060,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600060.png",
    quote: "A tactical giant in Boston history."
  },
  {
    id: "coy-1981",
    name: "Jack McKinney",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1980-81 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1980-81",
    isHistorical: true,
    pts: 44,
    reb: 38,
    ast: 0,
    age: 25,
    nbaId: 600061,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600061.png",
    quote: "Resilience personified on the bench."
  },
  {
    id: "coy-1982",
    name: "Gene Shue",
    team: "Washington Bullets",
    teamAbbr: "WAS",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1981-82 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1981-82",
    isHistorical: true,
    pts: 43,
    reb: 39,
    ast: 0,
    age: 25,
    nbaId: 600062,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600062.png",
    quote: "Master of the clipboard."
  },
  {
    id: "coy-1983",
    name: "Don Nelson",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1982-83 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1982-83",
    isHistorical: true,
    pts: 51,
    reb: 31,
    ast: 0,
    age: 25,
    nbaId: 600063,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600063.png",
    quote: "Nellie Ball changed the game."
  },
  {
    id: "coy-1984",
    name: "Frank Layden",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1983-84 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1983-84",
    isHistorical: true,
    pts: 45,
    reb: 37,
    ast: 0,
    age: 25,
    nbaId: 600064,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600064.png",
    quote: "The heart of Jazz basketball."
  },
  {
    id: "coy-1985",
    name: "Don Nelson",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1984-85 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1984-85",
    isHistorical: true,
    pts: 59,
    reb: 23,
    ast: 0,
    age: 25,
    nbaId: 600063,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600063.png",
    quote: "Genius knows no bounds."
  },
  {
    id: "coy-1986",
    name: "Mike Fratello",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1985-86 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1985-86",
    isHistorical: true,
    pts: 50,
    reb: 32,
    ast: 0,
    age: 25,
    nbaId: 600065,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600065.png",
    quote: "The Czar of the Teleprompter."
  },
  {
    id: "coy-1987",
    name: "Mike Schuler",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1986-87 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1986-87",
    isHistorical: true,
    pts: 49,
    reb: 33,
    ast: 0,
    age: 25,
    nbaId: 600066,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600066.png",
    quote: "Portland power."
  },
  {
    id: "coy-1988",
    name: "Doug Moe",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1987-88 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1987-88",
    isHistorical: true,
    pts: 54,
    reb: 28,
    ast: 0,
    age: 25,
    nbaId: 600067,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600067.png",
    quote: "Passing is the key to life."
  },
  {
    id: "coy-1989",
    name: "Cotton Fitzsimmons",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1988-89 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1988-89",
    isHistorical: true,
    pts: 55,
    reb: 27,
    ast: 0,
    age: 25,
    nbaId: 600068,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600068.png",
    quote: "The sunshine coach."
  },
  {
    id: "coy-1990",
    name: "Pat Riley",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1989-90 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1989-90",
    isHistorical: true,
    pts: 63,
    reb: 19,
    ast: 0,
    age: 25,
    nbaId: 2133,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2133.png",
    quote: "Success is never permanent."
  },
  {
    id: "coy-1991",
    name: "Don Chaney",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1990-91 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1990-91",
    isHistorical: true,
    pts: 52,
    reb: 30,
    ast: 0,
    age: 25,
    nbaId: 600069,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600069.png",
    quote: "Stability in Space City."
  },
  {
    id: "coy-1992",
    name: "Don Nelson",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1991-92 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1991-92",
    isHistorical: true,
    pts: 55,
    reb: 27,
    ast: 0,
    age: 25,
    nbaId: 600063,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600063.png",
    quote: "Innovation is the only way forward."
  },
  {
    id: "coy-1993",
    name: "Pat Riley",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1992-93 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1992-93",
    isHistorical: true,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 2133,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2133.png",
    quote: "Hard work beats talent."
  },
  {
    id: "coy-1994",
    name: "Lenny Wilkens",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1993-94 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1993-94",
    isHistorical: true,
    pts: 57,
    reb: 25,
    ast: 0,
    age: 25,
    nbaId: 600070,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600070.png",
    quote: "The gentleman of the game."
  },
  {
    id: "coy-1995",
    name: "Del Harris",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1994-95 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1994-95",
    isHistorical: true,
    pts: 48,
    reb: 34,
    ast: 0,
    age: 25,
    nbaId: 600071,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600071.png",
    quote: "Rebuilding with grace."
  },
  {
    id: "coy-1996",
    name: "Phil Jackson",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1995-96 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1995-96",
    isHistorical: true,
    pts: 72,
    reb: 10,
    ast: 0,
    age: 25,
    nbaId: 600072,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600072.png",
    quote: "Zen Master."
  },
  {
    id: "coy-1997",
    name: "Pat Riley",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1996-97 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1996-97",
    isHistorical: true,
    pts: 61,
    reb: 21,
    ast: 0,
    age: 25,
    nbaId: 2133,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2133.png",
    quote: "Heat Culture started here."
  },
  {
    id: "coy-1998",
    name: "Larry Bird",
    team: "Indiana Pacers",
    teamAbbr: "IND",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1997-98 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1997-98",
    isHistorical: true,
    pts: 58,
    reb: 24,
    ast: 0,
    age: 25,
    nbaId: 2182,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2182.png",
    quote: "I told them they could win."
  },
  {
    id: "coy-1999",
    name: "Mike Dunleavy Sr.",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1998-99 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1998-99",
    isHistorical: true,
    pts: 35,
    reb: 15,
    ast: 0,
    age: 25,
    nbaId: 600073,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600073.png",
    quote: "Portland pride."
  },
  {
    id: "coy-2000",
    name: "Doc Rivers",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "1999-00 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "1999-00",
    isHistorical: true,
    pts: 41,
    reb: 41,
    ast: 0,
    age: 25,
    nbaId: 2186,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2186.png",
    quote: "I love this team."
  },
  {
    id: "coy-2001",
    name: "Larry Brown",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2000-01 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2000-01",
    isHistorical: true,
    pts: 56,
    reb: 26,
    ast: 0,
    age: 29,
    nbaId: 600074,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600074.png",
    quote: "Play the right way."
  },
  {
    id: "coy-2002",
    name: "Rick Carlisle",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2001-02 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2001-02",
    isHistorical: true,
    pts: 50,
    reb: 32,
    ast: 0,
    age: 25,
    nbaId: 2028,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F2028.png",
    quote: "Discipline is key."
  },
  {
    id: "coy-2003",
    name: "Gregg Popovich",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2002-03 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2002-03",
    isHistorical: true,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 600075,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600075.png",
    quote: "Pounding the rock."
  },
  {
    id: "coy-2004",
    name: "Hubie Brown",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2003-04 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2003-04",
    isHistorical: true,
    pts: 50,
    reb: 32,
    ast: 0,
    age: 29,
    nbaId: 600076,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600076.png",
    quote: "Now look at this."
  },
  {
    id: "coy-2005",
    name: "Mike D'Antoni",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2004-05 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2004-05",
    isHistorical: true,
    pts: 62,
    reb: 20,
    ast: 0,
    age: 25,
    nbaId: 600077,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600077.png",
    quote: "Fast is beautiful."
  },
  {
    id: "coy-2006",
    name: "Avery Johnson",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2005-06 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2005-06",
    isHistorical: true,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 600078,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600078.png",
    quote: "The Little General."
  },
  {
    id: "coy-2007",
    name: "Sam Mitchell",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2006-07 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2006-07",
    isHistorical: true,
    pts: 47,
    reb: 35,
    ast: 0,
    age: 29,
    nbaId: 600079,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600079.png",
    quote: "North side pride."
  },
  {
    id: "coy-2008",
    name: "Byron Scott",
    team: "New Orleans Hornets",
    teamAbbr: "NOP",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2007-08 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2007-08",
    isHistorical: true,
    pts: 56,
    reb: 26,
    ast: 0,
    age: 25,
    nbaId: 600080,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600080.png",
    quote: "New Orleans strong."
  },
  {
    id: "coy-2009",
    name: "Mike Brown",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2008-09 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2008-09",
    isHistorical: true,
    pts: 66,
    reb: 16,
    ast: 0,
    age: 29,
    nbaId: 600081,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600081.png",
    quote: "Defense first."
  },
  {
    id: "coy-2010",
    name: "Scott Brooks",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2009-10 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2009-10",
    isHistorical: true,
    pts: 50,
    reb: 32,
    ast: 0,
    age: 25,
    nbaId: 600082,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600082.png",
    quote: "Growth over everything."
  },
  {
    id: "coy-2011",
    name: "Tom Thibodeau",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2010-11 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2010-11",
    isHistorical: true,
    pts: 62,
    reb: 20,
    ast: 0,
    age: 25,
    nbaId: 3906,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3906.png",
    quote: "Ice! Ice! Ice!"
  },
  {
    id: "coy-2012",
    name: "Gregg Popovich",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2011-12 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2011-12",
    isHistorical: true,
    pts: 50,
    reb: 16,
    ast: 0,
    age: 25,
    nbaId: 600075,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600075.png",
    quote: "Consistency is greatness."
  },
  {
    id: "coy-2013",
    name: "George Karl",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2012-13 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2012-13",
    isHistorical: true,
    pts: 57,
    reb: 25,
    ast: 0,
    age: 21,
    nbaId: 600083,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600083.png",
    quote: "The architect of speed."
  },
  {
    id: "coy-2014",
    name: "Gregg Popovich",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2013-14 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2013-14",
    isHistorical: true,
    pts: 62,
    reb: 20,
    ast: 0,
    age: 25,
    nbaId: 600075,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600075.png",
    quote: "The beautiful game."
  },
  {
    id: "coy-2015",
    name: "Mike Budenholzer",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2014-15 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2014-15",
    isHistorical: true,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 600084,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600084.png",
    quote: "The Spurs of the East."
  },
  {
    id: "coy-2016",
    name: "Steve Kerr",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2015-16 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2015-16",
    isHistorical: true,
    pts: 73,
    reb: 9,
    ast: 0,
    age: 25,
    nbaId: 600085,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600085.png",
    quote: "Strength in Numbers."
  },
  {
    id: "coy-2017",
    name: "Mike D'Antoni",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2016-17 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2016-17",
    isHistorical: true,
    pts: 55,
    reb: 27,
    ast: 0,
    age: 25,
    nbaId: 600077,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600077.png",
    quote: "More threes, more wins."
  },
  {
    id: "coy-2018",
    name: "Dwane Casey",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2017-18 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2017-18",
    isHistorical: true,
    pts: 59,
    reb: 23,
    ast: 0,
    age: 25,
    nbaId: 600086,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600086.png",
    quote: "Pound the rock, north style."
  },
  {
    id: "coy-2019",
    name: "Mike Budenholzer",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2018-19 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2018-19",
    isHistorical: true,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 600084,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600084.png",
    quote: "Fear the Deer."
  },
  {
    id: "coy-2020",
    name: "Nick Nurse",
    team: "Toronto Raptors",
    teamAbbr: "TOR",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2019-20 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2019-20",
    isHistorical: true,
    pts: 53,
    reb: 19,
    ast: 0,
    age: 25,
    nbaId: 600087,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600087.png",
    quote: "The creative genius."
  },
  {
    id: "coy-2021",
    name: "Tom Thibodeau",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2020-21 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2020-21",
    isHistorical: true,
    pts: 41,
    reb: 31,
    ast: 0,
    age: 25,
    nbaId: 3906,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F3906.png",
    quote: "We work."
  },
  {
    id: "coy-2022",
    name: "Monty Williams",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2021-22 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2021-22",
    isHistorical: true,
    pts: 64,
    reb: 18,
    ast: 0,
    age: 24,
    nbaId: 600088,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600088.png",
    quote: "Culture wins championships."
  },
  {
    id: "coy-2023",
    name: "Mike Brown",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2022-23 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2022-23",
    isHistorical: true,
    pts: 48,
    reb: 34,
    ast: 0,
    age: 29,
    nbaId: 600081,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600081.png",
    quote: "Light the Beam!"
  },
  {
    id: "coy-2024",
    name: "Mark Daigneault",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2023-24 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2023-24",
    isHistorical: true,
    pts: 57,
    reb: 25,
    ast: 0,
    age: 25,
    nbaId: 600089,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600089.png",
    quote: "Trust the growth."
  },
  {
    id: "coy-2025",
    name: "Kenny Atkinson",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2024-25 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2024-25",
    isHistorical: false,
    pts: 64,
    reb: 18,
    ast: 0,
    age: 25,
    nbaId: 600090,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600090.png",
    quote: "Speed and space in the 216."
  },
  {
    id: "coy-2026",
    name: "J.B. Bickerstaff",
    team: "Detroit Pistons",
    teamAbbr: "DET",
    position: "Coach",
    rarity: "coy" as const,
    category: "Coach of the Year" as const,
    subtitle: "2025-26 COACH OF THE YEAR",
    series: "NBA Coach of the Year",
    year: "2025-26",
    isHistorical: false,
    pts: 60,
    reb: 22,
    ast: 0,
    age: 25,
    nbaId: 600091,
    imageUrl: "https://a.espncdn.com/combiner/i?img=%2Fi%2Fheadshots%2Fnba%2Fplayers%2Ffull%2F600091.png",
    quote: "Detroit tough."
  },
  {
    id: "allnba-1989-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1989-90 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1989-90",
    isHistorical: true,
    pts: 33.6,
    reb: 6.9,
    ast: 6.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "Dominating at both ends of the floor."
  },
  {
    id: "allnba-1989-magic",
    name: "Magic Johnson",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1989-90 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1989-90",
    isHistorical: true,
    pts: 22.3,
    reb: 6.6,
    ast: 11.5,
    age: 25,
    nbaId: 77142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/77142.png",
    quote: "The engine of the Showtime Lakers."
  },
  {
    id: "allnba-1989-barkley",
    name: "Charles Barkley",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1989-90 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1989-90",
    isHistorical: true,
    pts: 25.2,
    reb: 11.5,
    ast: 3.9,
    age: 25,
    nbaId: 782,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/782.png",
    quote: "The Round Mound of Rebound."
  },
  {
    id: "allnba-1989-ewing",
    name: "Patrick Ewing",
    team: "New York Knicks",
    teamAbbr: "NYK",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1989-90 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1989-90",
    isHistorical: true,
    pts: 28.6,
    reb: 10.9,
    ast: 2.2,
    age: 25,
    nbaId: 121,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/121.png",
    quote: "The heart and soul of the Garden."
  },
  {
    id: "allnba-1989-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1989-90 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1989-90",
    isHistorical: true,
    pts: 31,
    reb: 11.1,
    ast: 2.8,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "The Mailman always delivers."
  },
  {
    id: "allnba-1990-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1990-91 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1990-91",
    isHistorical: true,
    pts: 31.5,
    reb: 6,
    ast: 5.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "The ultimate competitor on his first title run."
  },
  {
    id: "allnba-1990-magic",
    name: "Magic Johnson",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1990-91 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1990-91",
    isHistorical: true,
    pts: 19.4,
    reb: 7,
    ast: 12.5,
    age: 25,
    nbaId: 77142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/77142.png",
    quote: "Still the best playmaker in the world."
  },
  {
    id: "allnba-1990-barkley",
    name: "Charles Barkley",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1990-91 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1990-91",
    isHistorical: true,
    pts: 27.6,
    reb: 10.1,
    ast: 4.1,
    age: 25,
    nbaId: 782,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/782.png",
    quote: "Dominant scoring and board-crashing."
  },
  {
    id: "allnba-1990-robinson",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1990-91 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1990-91",
    isHistorical: true,
    pts: 25.6,
    reb: 13,
    ast: 2.5,
    age: 25,
    nbaId: 192,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/192.png",
    quote: "The Admiral at his defensive peak."
  },
  {
    id: "allnba-1990-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1990-91 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1990-91",
    isHistorical: true,
    pts: 29,
    reb: 11.8,
    ast: 3.3,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "Consistency personified in the post."
  },
  {
    id: "allnba-1991-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1991-92 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1991-92",
    isHistorical: true,
    pts: 30.1,
    reb: 6.4,
    ast: 6.1,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "Repeat champion and scoring king."
  },
  {
    id: "allnba-1991-drexler",
    name: "Clyde Drexler",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1991-92 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1991-92",
    isHistorical: true,
    pts: 25,
    reb: 6.6,
    ast: 6.7,
    age: 25,
    nbaId: 95,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/95.png",
    quote: "Clyde the Glide flying through the Rip City."
  },
  {
    id: "allnba-1991-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1991-92 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1991-92",
    isHistorical: true,
    pts: 28,
    reb: 11.2,
    ast: 3,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "Pick and roll mastery with Stockton."
  },
  {
    id: "allnba-1991-robinson",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1991-92 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1991-92",
    isHistorical: true,
    pts: 23.2,
    reb: 12.2,
    ast: 2.7,
    age: 25,
    nbaId: 192,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/192.png",
    quote: "The ultimate two-way big man."
  },
  {
    id: "allnba-1991-mullin",
    name: "Chris Mullin",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1991-92 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1991-92",
    isHistorical: true,
    pts: 25.6,
    reb: 5.9,
    ast: 4.4,
    age: 25,
    nbaId: 157,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/157.png",
    quote: "The sharpest shooter in Run TMC."
  },
  {
    id: "allnba-1992-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1992-93 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1992-93",
    isHistorical: true,
    pts: 32.6,
    reb: 6.7,
    ast: 5.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "A historic Three-Peat cemented."
  },
  {
    id: "allnba-1992-price",
    name: "Mark Price",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1992-93 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1992-93",
    isHistorical: true,
    pts: 18.2,
    reb: 3.3,
    ast: 8,
    age: 25,
    nbaId: 147,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/147.png",
    quote: "Elite shooting and precise playmaking."
  },
  {
    id: "allnba-1992-barkley",
    name: "Charles Barkley",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1992-93 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1992-93",
    isHistorical: true,
    pts: 25.6,
    reb: 12.2,
    ast: 5.1,
    age: 25,
    nbaId: 782,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/782.png",
    quote: "MVP season and a trip to the Finals."
  },
  {
    id: "allnba-1992-hakeem",
    name: "Hakeem Olajuwon",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1992-93 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1992-93",
    isHistorical: true,
    pts: 26.1,
    reb: 13,
    ast: 3.5,
    age: 25,
    nbaId: 165,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/165.png",
    quote: "The Dream Shake in full effect."
  },
  {
    id: "allnba-1992-wilkins",
    name: "Dominique Wilkins",
    team: "Atlanta Hawks",
    teamAbbr: "ATL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1992-93 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1992-93",
    isHistorical: true,
    pts: 29.9,
    reb: 6.8,
    ast: 2.9,
    age: 25,
    nbaId: 254,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/254.png",
    quote: "The Human Highlight Film still soaring."
  },
  {
    id: "allnba-1993-pippen",
    name: "Scottie Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1993-94 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1993-94",
    isHistorical: true,
    pts: 22,
    reb: 8.7,
    ast: 5.6,
    age: 25,
    nbaId: 937,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/937.png",
    quote: "Proving he can lead without MJ."
  },
  {
    id: "allnba-1993-stockton",
    name: "John Stockton",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1993-94 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1993-94",
    isHistorical: true,
    pts: 17,
    reb: 3.3,
    ast: 12.6,
    age: 25,
    nbaId: 304,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/304.png",
    quote: "The master of the assist and the steal."
  },
  {
    id: "allnba-1993-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1993-94 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1993-94",
    isHistorical: true,
    pts: 25,
    reb: 11.5,
    ast: 4,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "Unstoppable in the pick and roll."
  },
  {
    id: "allnba-1993-hakeem",
    name: "Hakeem Olajuwon",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1993-94 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1993-94",
    isHistorical: true,
    pts: 27.3,
    reb: 11.9,
    ast: 3.6,
    age: 25,
    nbaId: 165,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/165.png",
    quote: "MVP and DPOY in a historic season."
  },
  {
    id: "allnba-1993-kemp",
    name: "Shawn Kemp",
    team: "Seattle SuperSonics",
    teamAbbr: "SEA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1993-94 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1993-94",
    isHistorical: true,
    pts: 18.1,
    reb: 10.8,
    ast: 2,
    age: 25,
    nbaId: 432,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/432.png",
    quote: "Reigningite dunks and explosive power."
  },
  {
    id: "allnba-1994-robinson",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1994-95 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1994-95",
    isHistorical: true,
    pts: 27.6,
    reb: 10.8,
    ast: 2.9,
    age: 25,
    nbaId: 192,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/192.png",
    quote: "MVP season for The Admiral."
  },
  {
    id: "allnba-1994-stockton",
    name: "John Stockton",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1994-95 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1994-95",
    isHistorical: true,
    pts: 17.2,
    reb: 3.2,
    ast: 12.3,
    age: 25,
    nbaId: 304,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/304.png",
    quote: "Precision passing at its absolute finest."
  },
  {
    id: "allnba-1994-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1994-95 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1994-95",
    isHistorical: true,
    pts: 26.7,
    reb: 10.6,
    ast: 3.7,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "The ultimate finish on the break."
  },
  {
    id: "allnba-1994-shaq",
    name: "Shaquille O'Neal",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1994-95 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1994-95",
    isHistorical: true,
    pts: 29.3,
    reb: 13.2,
    ast: 2.7,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "Physical dominance the league hadn't seen."
  },
  {
    id: "allnba-1994-penny",
    name: "Anfernee Hardaway",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1994-95 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1994-95",
    isHistorical: true,
    pts: 20.9,
    reb: 4.4,
    ast: 7.2,
    age: 25,
    nbaId: 302,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/302.png",
    quote: "The next magic taking over in Orlando."
  },
  {
    id: "allnba-1995-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1995-96 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1995-96",
    isHistorical: true,
    pts: 30.4,
    reb: 6.6,
    ast: 4.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "72-10. Enough said."
  },
  {
    id: "allnba-1995-payton",
    name: "Gary Payton",
    team: "Seattle SuperSonics",
    teamAbbr: "SEA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1995-96 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1995-96",
    isHistorical: true,
    pts: 19.3,
    reb: 4.2,
    ast: 7.5,
    age: 25,
    nbaId: 840,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/840.png",
    quote: "The Glove on defense, floor general on offense."
  },
  {
    id: "allnba-1995-pippen",
    name: "Scottie Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1995-96 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1995-96",
    isHistorical: true,
    pts: 19.4,
    reb: 6.4,
    ast: 5.9,
    age: 25,
    nbaId: 937,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/937.png",
    quote: "The silent assassin of the greatest team ever."
  },
  {
    id: "allnba-1995-robinson",
    name: "David Robinson",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1995-96 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1995-96",
    isHistorical: true,
    pts: 25,
    reb: 12.2,
    ast: 2.9,
    age: 25,
    nbaId: 192,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/192.png",
    quote: "Consistency and class from the Admiral."
  },
  {
    id: "allnba-1995-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1995-96 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1995-96",
    isHistorical: true,
    pts: 25.7,
    reb: 9.8,
    ast: 4,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "Power and finesse in the low block."
  },
  {
    id: "allnba-1996-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1996-97 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1996-97",
    isHistorical: true,
    pts: 29.6,
    reb: 5.9,
    ast: 4.3,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "Back-to-back titles and legendary status."
  },
  {
    id: "allnba-1996-hardaway",
    name: "Tim Hardaway",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1996-97 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1996-97",
    isHistorical: true,
    pts: 20.3,
    reb: 3.5,
    ast: 8.6,
    age: 25,
    nbaId: 150,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/150.png",
    quote: "The killer crossover in South Beach."
  },
  {
    id: "allnba-1996-pippen",
    name: "Scottie Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1996-97 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1996-97",
    isHistorical: true,
    pts: 20.2,
    reb: 6.5,
    ast: 5.7,
    age: 25,
    nbaId: 937,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/937.png",
    quote: "Lockdown defense and secondary playmaking."
  },
  {
    id: "allnba-1996-hakeem",
    name: "Hakeem Olajuwon",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1996-97 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1996-97",
    isHistorical: true,
    pts: 23.2,
    reb: 9.2,
    ast: 2.9,
    age: 25,
    nbaId: 165,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/165.png",
    quote: "Elegance and power in the post."
  },
  {
    id: "allnba-1996-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1996-97 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1996-97",
    isHistorical: true,
    pts: 27.4,
    reb: 9.9,
    ast: 4.5,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "MVP season and first Finals appearance."
  },
  {
    id: "allnba-1997-mj",
    name: "Michael Jordan",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1997-98 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1997-98",
    isHistorical: true,
    pts: 28.7,
    reb: 5.8,
    ast: 3.5,
    age: 25,
    nbaId: 893,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/893.png",
    quote: "The Last Dance. The Second Three-Peat."
  },
  {
    id: "allnba-1997-payton",
    name: "Gary Payton",
    team: "Seattle SuperSonics",
    teamAbbr: "SEA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1997-98 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1997-98",
    isHistorical: true,
    pts: 19.2,
    reb: 4.4,
    ast: 8.9,
    age: 25,
    nbaId: 840,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/840.png",
    quote: "Elite trash talk and elite defense."
  },
  {
    id: "allnba-1997-pippen",
    name: "Scottie Pippen",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1997-98 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1997-98",
    isHistorical: true,
    pts: 19.1,
    reb: 6.6,
    ast: 5.8,
    age: 25,
    nbaId: 937,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/937.png",
    quote: "The ultimate glue guy on a championship squad."
  },
  {
    id: "allnba-1997-shaq",
    name: "Shaquille O'Neal",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1997-98 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1997-98",
    isHistorical: true,
    pts: 28.3,
    reb: 12.5,
    ast: 2.4,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "Beginning of the Lakers dynasty."
  },
  {
    id: "allnba-1997-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1997-98 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1997-98",
    isHistorical: true,
    pts: 27,
    reb: 10.3,
    ast: 3.9,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "Standard of excellence in Salt Lake City."
  },
  {
    id: "allnba-1998-iverson",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1998-99 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1998-99",
    isHistorical: true,
    pts: 26.8,
    reb: 4.9,
    ast: 4.6,
    age: 27,
    nbaId: 947,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/947.png",
    quote: "The Answer arrives as a superstar."
  },
  {
    id: "allnba-1998-kidd",
    name: "Jason Kidd",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1998-99 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1998-99",
    isHistorical: true,
    pts: 16.9,
    reb: 6.8,
    ast: 10.8,
    age: 25,
    nbaId: 467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/467.png",
    quote: "The ultimate floor general in Phoenix."
  },
  {
    id: "allnba-1998-malone",
    name: "Karl Malone",
    team: "Utah Jazz",
    teamAbbr: "UTA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1998-99 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1998-99",
    isHistorical: true,
    pts: 23.8,
    reb: 9.4,
    ast: 3.9,
    age: 25,
    nbaId: 252,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/252.png",
    quote: "MVP season at age 35."
  },
  {
    id: "allnba-1998-mourning",
    name: "Alonzo Mourning",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1998-99 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1998-99",
    isHistorical: true,
    pts: 20.1,
    reb: 9.5,
    ast: 1.5,
    age: 25,
    nbaId: 297,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/297.png",
    quote: "Defensive anchor and DPOY."
  },
  {
    id: "allnba-1998-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1998-99 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1998-99",
    isHistorical: true,
    pts: 21.7,
    reb: 11.4,
    ast: 2.4,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "The rebirth of the Spurs dynasty."
  },
  {
    id: "allnba-1999-shaq",
    name: "Shaquille O'Neal",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1999-00 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1999-00",
    isHistorical: true,
    pts: 29.7,
    reb: 13.6,
    ast: 3.8,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "Most dominant season in modern history."
  },
  {
    id: "allnba-1999-payton",
    name: "Gary Payton",
    team: "Seattle SuperSonics",
    teamAbbr: "SEA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1999-00 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1999-00",
    isHistorical: true,
    pts: 24.2,
    reb: 6.6,
    ast: 8.9,
    age: 25,
    nbaId: 840,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/840.png",
    quote: "The Sonics star at his offensive peak."
  },
  {
    id: "allnba-1999-kidd",
    name: "Jason Kidd",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1999-00 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1999-00",
    isHistorical: true,
    pts: 14.3,
    reb: 7.2,
    ast: 10.1,
    age: 25,
    nbaId: 467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/467.png",
    quote: "Maestro of the fast break."
  },
  {
    id: "allnba-1999-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1999-00 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1999-00",
    isHistorical: true,
    pts: 23.2,
    reb: 12.4,
    ast: 3.2,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "Consistency and fundamentals in San Antonio."
  },
  {
    id: "allnba-1999-garnett",
    name: "Kevin Garnett",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "1999-00 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "1999-00",
    isHistorical: true,
    pts: 22.9,
    reb: 11.8,
    ast: 5,
    age: 25,
    nbaId: 708,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/708.png",
    quote: "The Big Ticket leading Minnesota."
  },
  {
    id: "allnba-2000-iverson",
    name: "Allen Iverson",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2000-01 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2000-01",
    isHistorical: true,
    pts: 31.1,
    reb: 3.8,
    ast: 4.6,
    age: 27,
    nbaId: 947,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/947.png",
    quote: "MVP season and a legendary Finals run."
  },
  {
    id: "allnba-2000-kidd",
    name: "Jason Kidd",
    team: "New Jersey Nets",
    teamAbbr: "BKN",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2000-01 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2000-01",
    isHistorical: true,
    pts: 14.7,
    reb: 7.8,
    ast: 9.9,
    age: 25,
    nbaId: 467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/467.png",
    quote: "Transforming the Nets into contenders."
  },
  {
    id: "allnba-2000-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2000-01 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2000-01",
    isHistorical: true,
    pts: 22.2,
    reb: 12.2,
    ast: 3,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "The fundamental force in San Antonio."
  },
  {
    id: "allnba-2000-shaq",
    name: "Shaquille O'Neal",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2000-01 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2000-01",
    isHistorical: true,
    pts: 28.7,
    reb: 12.7,
    ast: 3.7,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "Repeat champion and Finals MVP dominance."
  },
  {
    id: "allnba-2000-webber",
    name: "Chris Webber",
    team: "Sacramento Kings",
    teamAbbr: "SAC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2000-01 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2000-01",
    isHistorical: true,
    pts: 27.1,
    reb: 11.1,
    ast: 5,
    age: 25,
    nbaId: 185,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/185.png",
    quote: "The heart of the Greatest Show on Court."
  },
  {
    id: "allnba-2001-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2001-02 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2001-02",
    isHistorical: true,
    pts: 25.2,
    reb: 5.5,
    ast: 5.5,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Ascending to the top of the NBA."
  },
  {
    id: "allnba-2001-kidd",
    name: "Jason Kidd",
    team: "New Jersey Nets",
    teamAbbr: "BKN",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2001-02 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2001-02",
    isHistorical: true,
    pts: 14.7,
    reb: 6.8,
    ast: 9.9,
    age: 25,
    nbaId: 467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/467.png",
    quote: "The triple-double king of the East."
  },
  {
    id: "allnba-2001-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2001-02 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2001-02",
    isHistorical: true,
    pts: 25.5,
    reb: 12.7,
    ast: 3.7,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "Back-to-back MVP excellence."
  },
  {
    id: "allnba-2001-shaq",
    name: "Shaquille O'Neal",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2001-02 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2001-02",
    isHistorical: true,
    pts: 27.2,
    reb: 10.7,
    ast: 3,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "Three-Peat complete. Simply unguardable."
  },
  {
    id: "allnba-2001-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2001-02 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2001-02",
    isHistorical: true,
    pts: 23.4,
    reb: 9.9,
    ast: 2.4,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "The German Wunderkind changing the game."
  },
  {
    id: "allnba-2002-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2002-03 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2002-03",
    isHistorical: true,
    pts: 30,
    reb: 6.9,
    ast: 5.9,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "The Mamba Mentality in full effect."
  },
  {
    id: "allnba-2002-nash",
    name: "Steve Nash",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2002-03 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2002-03",
    isHistorical: true,
    pts: 17.7,
    reb: 3.7,
    ast: 7.3,
    age: 25,
    nbaId: 959,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/959.png",
    quote: "Maestro of the Mavericks offense."
  },
  {
    id: "allnba-2002-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2002-03 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2002-03",
    isHistorical: true,
    pts: 23.3,
    reb: 12.9,
    ast: 3.9,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "MVP and champion in a dominant year."
  },
  {
    id: "allnba-2002-garnett",
    name: "Kevin Garnett",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2002-03 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2002-03",
    isHistorical: true,
    pts: 23,
    reb: 13.4,
    ast: 4.8,
    age: 25,
    nbaId: 708,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/708.png",
    quote: "Elite intensity and all-around excellence."
  },
  {
    id: "allnba-2002-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2002-03 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2002-03",
    isHistorical: true,
    pts: 25.1,
    reb: 9.9,
    ast: 3,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "The ultimate offensive weapon from Europe."
  },
  {
    id: "allnba-2003-garnett",
    name: "Kevin Garnett",
    team: "Minnesota Timberwolves",
    teamAbbr: "MIN",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2003-04 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2003-04",
    isHistorical: true,
    pts: 24.2,
    reb: 13.9,
    ast: 5,
    age: 25,
    nbaId: 708,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/708.png",
    quote: "MVP season for the Big Ticket."
  },
  {
    id: "allnba-2003-kidd",
    name: "Jason Kidd",
    team: "New Jersey Nets",
    teamAbbr: "BKN",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2003-04 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2003-04",
    isHistorical: true,
    pts: 15.5,
    reb: 6.4,
    ast: 9.2,
    age: 25,
    nbaId: 467,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/467.png",
    quote: "Master of the fast break and defense."
  },
  {
    id: "allnba-2003-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2003-04 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2003-04",
    isHistorical: true,
    pts: 22.3,
    reb: 12.4,
    ast: 3.1,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "Silent dominance in San Antonio."
  },
  {
    id: "allnba-2003-shaq",
    name: "Shaquille O'Neal",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2003-04 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2003-04",
    isHistorical: true,
    pts: 21.5,
    reb: 11.5,
    ast: 2.9,
    age: 25,
    nbaId: 406,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/406.png",
    quote: "The Big Diesel's final run in LA."
  },
  {
    id: "allnba-2003-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2003-04 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2003-04",
    isHistorical: true,
    pts: 24,
    reb: 5.5,
    ast: 5.1,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Dynamic scoring alongside Shaq."
  },
  {
    id: "allnba-2004-nash",
    name: "Steve Nash",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2004-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2004-05",
    isHistorical: true,
    pts: 15.5,
    reb: 3.3,
    ast: 11.5,
    age: 25,
    nbaId: 959,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/959.png",
    quote: "MVP and Seven Seconds or Less era."
  },
  {
    id: "allnba-2004-wade",
    name: "Dwyane Wade",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2004-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2004-05",
    isHistorical: true,
    pts: 24.1,
    reb: 5.2,
    ast: 6.8,
    age: 25,
    nbaId: 2548,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2548.png",
    quote: "Flash arriving on the big stage."
  },
  {
    id: "allnba-2004-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2004-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2004-05",
    isHistorical: true,
    pts: 27.2,
    reb: 7.4,
    ast: 7.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "The King taking his throne."
  },
  {
    id: "allnba-2004-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2004-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2004-05",
    isHistorical: true,
    pts: 20.3,
    reb: 11.1,
    ast: 2.7,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "Championship standards in San Antonio."
  },
  {
    id: "allnba-2004-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2004-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2004-05",
    isHistorical: true,
    pts: 26.1,
    reb: 9.7,
    ast: 3.1,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "Elite scoring and leadership in Dallas."
  },
  {
    id: "allnba-2005-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2005-06 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2005-06",
    isHistorical: true,
    pts: 35.4,
    reb: 5.3,
    ast: 4.5,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Historic 35 PPG season. Peak Kobe."
  },
  {
    id: "allnba-2005-nash",
    name: "Steve Nash",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2005-06 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2005-06",
    isHistorical: true,
    pts: 18.8,
    reb: 4.2,
    ast: 10.5,
    age: 25,
    nbaId: 959,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/959.png",
    quote: "Back-to-back MVP excellence."
  },
  {
    id: "allnba-2005-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2005-06 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2005-06",
    isHistorical: true,
    pts: 31.4,
    reb: 7,
    ast: 6.6,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Dominating at age 21."
  },
  {
    id: "allnba-2005-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2005-06 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2005-06",
    isHistorical: true,
    pts: 18.6,
    reb: 11,
    ast: 3.2,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "The Big Fundamental always in the mix."
  },
  {
    id: "allnba-2005-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2005-06 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2005-06",
    isHistorical: true,
    pts: 26.6,
    reb: 9,
    ast: 2.8,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "Leading the Mavs to the Finals."
  },
  {
    id: "allnba-2006-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2006-07 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2006-07",
    isHistorical: true,
    pts: 31.6,
    reb: 5.7,
    ast: 5.4,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Unstoppable individual scoring brilliance."
  },
  {
    id: "allnba-2006-nash",
    name: "Steve Nash",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2006-07 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2006-07",
    isHistorical: true,
    pts: 18.6,
    reb: 4.5,
    ast: 11.6,
    age: 25,
    nbaId: 959,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/959.png",
    quote: "The heart of the league's best offense."
  },
  {
    id: "allnba-2006-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2006-07 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2006-07",
    isHistorical: true,
    pts: 27.3,
    reb: 6.7,
    ast: 6,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Leading Cleveland to the NBA Finals."
  },
  {
    id: "allnba-2006-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2006-07 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2006-07",
    isHistorical: true,
    pts: 24.6,
    reb: 8.9,
    ast: 3.4,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "MVP season for the German Legend."
  },
  {
    id: "allnba-2006-stoudemire",
    name: "Amar'e Stoudemire",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2006-07 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2006-07",
    isHistorical: true,
    pts: 20.4,
    reb: 8.8,
    ast: 1.7,
    age: 25,
    nbaId: 2405,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2405.png",
    quote: "Explosive power and finishing in the paint."
  },
  {
    id: "allnba-2007-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2007-08 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2007-08",
    isHistorical: true,
    pts: 28.3,
    reb: 6.3,
    ast: 5.4,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "MVP season and returning to the Finals."
  },
  {
    id: "allnba-2007-paul",
    name: "Chris Paul",
    team: "New Orleans Hornets",
    teamAbbr: "NOP",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2007-08 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2007-08",
    isHistorical: true,
    pts: 21.1,
    reb: 4,
    ast: 11.6,
    age: 40,
    nbaId: 101108,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/101108.png",
    quote: "Point God dominance in the Big Easy."
  },
  {
    id: "allnba-2007-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2007-08 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2007-08",
    isHistorical: true,
    pts: 30,
    reb: 7.9,
    ast: 7.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "The King's all-around peak performance."
  },
  {
    id: "allnba-2007-garnett",
    name: "Kevin Garnett",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2007-08 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2007-08",
    isHistorical: true,
    pts: 18.8,
    reb: 9.2,
    ast: 3.4,
    age: 25,
    nbaId: 708,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/708.png",
    quote: "ANYTHING IS POSSIBLE! Champion in Boston."
  },
  {
    id: "allnba-2007-howard",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2007-08 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2007-08",
    isHistorical: true,
    pts: 20.7,
    reb: 13.8,
    ast: 1.3,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2730.png",
    quote: "Superman flying high in Orlando."
  },
  {
    id: "allnba-2008-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2008-09 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2008-09",
    isHistorical: true,
    pts: 26.8,
    reb: 5.2,
    ast: 4.9,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Leading the Lakers to the promised land."
  },
  {
    id: "allnba-2008-wade",
    name: "Dwyane Wade",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2008-09 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2008-09",
    isHistorical: true,
    pts: 30.2,
    reb: 5,
    ast: 7.5,
    age: 25,
    nbaId: 2548,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2548.png",
    quote: "Scoring title and absolute dominance."
  },
  {
    id: "allnba-2008-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2008-09 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2008-09",
    isHistorical: true,
    pts: 28.4,
    reb: 7.6,
    ast: 7.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "First MVP season for the King."
  },
  {
    id: "allnba-2008-howard",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2008-09 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2008-09",
    isHistorical: true,
    pts: 20.6,
    reb: 13.8,
    ast: 1.4,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2730.png",
    quote: "DPOY and leading Orlando to the Finals."
  },
  {
    id: "allnba-2008-nowitzki",
    name: "Dirk Nowitzki",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2008-09 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2008-09",
    isHistorical: true,
    pts: 25.9,
    reb: 8.4,
    ast: 2.4,
    age: 25,
    nbaId: 1717,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1717.png",
    quote: "Elite shooting prowess remains unmatched."
  },
  {
    id: "allnba-2009-kobe",
    name: "Kobe Bryant",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2009-10 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2009-10",
    isHistorical: true,
    pts: 27,
    reb: 5.4,
    ast: 5,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Back-to-back champion and Finals MVP."
  },
  {
    id: "allnba-2009-wade",
    name: "Dwyane Wade",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2009-10 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2009-10",
    isHistorical: true,
    pts: 26.6,
    reb: 4.8,
    ast: 6.5,
    age: 25,
    nbaId: 2548,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2548.png",
    quote: "Unstoppable agility and finishing."
  },
  {
    id: "allnba-2009-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2009-10 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2009-10",
    isHistorical: true,
    pts: 29.7,
    reb: 7.3,
    ast: 8.6,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Back-to-back MVP honors for the King."
  },
  {
    id: "allnba-2009-howard",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2009-10 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2009-10",
    isHistorical: true,
    pts: 18.3,
    reb: 13.2,
    ast: 1.6,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2730.png",
    quote: "The dominant force in the paint."
  },
  {
    id: "allnba-2009-durant",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2009-10 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2009-10",
    isHistorical: true,
    pts: 30.1,
    reb: 7.6,
    ast: 2.8,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "The youngest scoring champion in history."
  },
  {
    id: "allnba-2010-kobe",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2010-11 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2010-11",
    isHistorical: true,
    pts: 25.3,
    reb: 5.1,
    ast: 4.7,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Standard of veteran excellence."
  },
  {
    id: "allnba-2010-rose",
    name: "Derrick Rose",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2010-11 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2010-11",
    isHistorical: true,
    pts: 25,
    reb: 4.1,
    ast: 7.7,
    age: 25,
    nbaId: 201565,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201565.png",
    quote: "Youngest MVP in NBA history."
  },
  {
    id: "allnba-2010-lebron",
    name: "LeBron James",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2010-11 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2010-11",
    isHistorical: true,
    pts: 26.7,
    reb: 7.5,
    ast: 7,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Taking his talents to South Beach."
  },
  {
    id: "allnba-2010-howard",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2010-11 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2010-11",
    isHistorical: true,
    pts: 22.9,
    reb: 14.1,
    ast: 1.4,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2730.png",
    quote: "A force of nature in the interior."
  },
  {
    id: "allnba-2010-durant",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2010-11 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2010-11",
    isHistorical: true,
    pts: 27.7,
    reb: 6.8,
    ast: 2.7,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "Scoring mastery from the perimeter."
  },
  {
    id: "allnba-2011-paul",
    name: "Chris Paul",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2011-12 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2011-12",
    isHistorical: true,
    pts: 19.8,
    reb: 3.6,
    ast: 9.1,
    age: 40,
    nbaId: 101108,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/101108.png",
    quote: "Lob City's conductor in LAC."
  },
  {
    id: "allnba-2011-kobe",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2011-12 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2011-12",
    isHistorical: true,
    pts: 27.9,
    reb: 5.4,
    ast: 4.6,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Elite competitive spirit remains."
  },
  {
    id: "allnba-2011-durant",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2011-12 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2011-12",
    isHistorical: true,
    pts: 28,
    reb: 8,
    ast: 3.5,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "Leading OKC to the NBA Finals."
  },
  {
    id: "allnba-2011-lebron",
    name: "LeBron James",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2011-12 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2011-12",
    isHistorical: true,
    pts: 27.1,
    reb: 7.9,
    ast: 6.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "First championship and MVP season in Miami."
  },
  {
    id: "allnba-2011-howard",
    name: "Dwight Howard",
    team: "Orlando Magic",
    teamAbbr: "ORL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2011-12 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2011-12",
    isHistorical: true,
    pts: 20.6,
    reb: 14.5,
    ast: 1.9,
    age: 25,
    nbaId: 2730,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2730.png",
    quote: "Defensive anchor in Orlando."
  },
  {
    id: "allnba-2012-paul",
    name: "Chris Paul",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2012-13 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2012-13",
    isHistorical: true,
    pts: 16.9,
    reb: 3.7,
    ast: 9.7,
    age: 40,
    nbaId: 101108,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/101108.png",
    quote: "The ultimate floor general."
  },
  {
    id: "allnba-2012-kobe",
    name: "Kobe Bryant",
    team: "Los Angeles Lakers",
    teamAbbr: "LAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2012-13 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2012-13",
    isHistorical: true,
    pts: 27.3,
    reb: 5.6,
    ast: 6,
    age: 25,
    nbaId: 977,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/977.png",
    quote: "Vino gets better with age."
  },
  {
    id: "allnba-2012-durant",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2012-13 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2012-13",
    isHistorical: true,
    pts: 28.1,
    reb: 7.9,
    ast: 4.6,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "Efficiency at its highest peak."
  },
  {
    id: "allnba-2012-lebron",
    name: "LeBron James",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2012-13 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2012-13",
    isHistorical: true,
    pts: 26.8,
    reb: 8,
    ast: 7.3,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Back-to-back MVP and champion."
  },
  {
    id: "allnba-2012-duncan",
    name: "Tim Duncan",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2012-13 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2012-13",
    isHistorical: true,
    pts: 17.8,
    reb: 9.9,
    ast: 2.7,
    age: 25,
    nbaId: 1495,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1495.png",
    quote: "Fundamental longevity in San Antonio."
  },
  {
    id: "allnba-2013-paul",
    name: "Chris Paul",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2013-14 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2013-14",
    isHistorical: true,
    pts: 19.1,
    reb: 4.3,
    ast: 10.7,
    age: 40,
    nbaId: 101108,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/101108.png",
    quote: "Master of the mid-range and assist."
  },
  {
    id: "allnba-2013-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2013-14 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2013-14",
    isHistorical: true,
    pts: 25.4,
    reb: 4.7,
    ast: 6.1,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "Fear the Beard in Houston."
  },
  {
    id: "allnba-2013-durant",
    name: "Kevin Durant",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2013-14 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2013-14",
    isHistorical: true,
    pts: 32,
    reb: 7.4,
    ast: 5.5,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "You the real MVP. Historic scoring season."
  },
  {
    id: "allnba-2013-lebron",
    name: "LeBron James",
    team: "Miami Heat",
    teamAbbr: "MIA",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2013-14 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2013-14",
    isHistorical: true,
    pts: 27.1,
    reb: 6.9,
    ast: 6.3,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Final season of the Heatles era."
  },
  {
    id: "allnba-2013-noah",
    name: "Joakim Noah",
    team: "Chicago Bulls",
    teamAbbr: "CHI",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2013-14 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2013-14",
    isHistorical: true,
    pts: 12.6,
    reb: 11.3,
    ast: 5.4,
    age: 25,
    nbaId: 201149,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201149.png",
    quote: "DPOY and point-center excellence."
  },
  {
    id: "allnba-2014-curry",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2014-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2014-05",
    isHistorical: true,
    pts: 23.8,
    reb: 4.3,
    ast: 7.7,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "MVP and changing the game forever."
  },
  {
    id: "allnba-2014-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2014-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2014-05",
    isHistorical: true,
    pts: 27.4,
    reb: 5.7,
    ast: 7,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "Offensive maestro in H-Town."
  },
  {
    id: "allnba-2014-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2014-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2014-05",
    isHistorical: true,
    pts: 25.3,
    reb: 6,
    ast: 7.4,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "The King returns to Cleveland."
  },
  {
    id: "allnba-2014-davis",
    name: "Anthony Davis",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2014-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2014-05",
    isHistorical: true,
    pts: 24.4,
    reb: 10.2,
    ast: 2.2,
    age: 32,
    nbaId: 203076,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203076.png",
    quote: "The Brow dominating in NOLA."
  },
  {
    id: "allnba-2014-gasol",
    name: "Marc Gasol",
    team: "Memphis Grizzlies",
    teamAbbr: "MEM",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2014-05 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2014-05",
    isHistorical: true,
    pts: 17.4,
    reb: 7.8,
    ast: 3.8,
    age: 25,
    nbaId: 201188,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201188.png",
    quote: "Grit and Grind anchor."
  },
  {
    id: "allnba-2015-curry",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2015-16 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2015-16",
    isHistorical: true,
    pts: 30.1,
    reb: 5.4,
    ast: 6.7,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "Unanimous MVP and 73-9 record."
  },
  {
    id: "allnba-2015-westbrook",
    name: "Russell Westbrook",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2015-16 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2015-16",
    isHistorical: true,
    pts: 23.5,
    reb: 7.8,
    ast: 10.4,
    age: 25,
    nbaId: 201566,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201566.png",
    quote: "Unrelenting energy and triple-doubles."
  },
  {
    id: "allnba-2015-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2015-16 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2015-16",
    isHistorical: true,
    pts: 25.3,
    reb: 7.4,
    ast: 6.8,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Cleveland, this is for you!"
  },
  {
    id: "allnba-2015-kawhi",
    name: "Kawhi Leonard",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2015-16 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2015-16",
    isHistorical: true,
    pts: 21.2,
    reb: 6.8,
    ast: 2.6,
    age: 34,
    nbaId: 202695,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202695.png",
    quote: "Elite two-way mastery."
  },
  {
    id: "allnba-2015-jordan",
    name: "DeAndre Jordan",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2015-16 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2015-16",
    isHistorical: true,
    pts: 12.7,
    reb: 13.8,
    ast: 1.2,
    age: 25,
    nbaId: 201599,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201599.png",
    quote: "High-flying rim protection."
  },
  {
    id: "allnba-2016-curry",
    name: " Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2016-17 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2016-17",
    isHistorical: true,
    pts: 25.3,
    reb: 4.5,
    ast: 6.6,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "Leading the Warriors to another title."
  },
  {
    id: "allnba-2016-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2016-17 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2016-17",
    isHistorical: true,
    pts: 29.1,
    reb: 8.1,
    ast: 11.2,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "The Beard as the ultimate point guard."
  },
  {
    id: "allnba-2016-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2016-17 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2016-17",
    isHistorical: true,
    pts: 26.4,
    reb: 8.6,
    ast: 8.7,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Consistent excellence from the King."
  },
  {
    id: "allnba-2016-kawhi",
    name: "Kawhi Leonard",
    team: "San Antonio Spurs",
    teamAbbr: "SAS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2016-17 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2016-17",
    isHistorical: true,
    pts: 25.5,
    reb: 5.8,
    ast: 3.5,
    age: 34,
    nbaId: 202695,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202695.png",
    quote: "The silent assassin of San Antonio."
  },
  {
    id: "allnba-2016-davis",
    name: "Anthony Davis",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2016-17 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2016-17",
    isHistorical: true,
    pts: 28,
    reb: 11.8,
    ast: 2.1,
    age: 32,
    nbaId: 203076,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203076.png",
    quote: "Dominating both ends of the floor."
  },
  {
    id: "allnba-2017-lillard",
    name: "Damian Lillard",
    team: "Portland Trail Blazers",
    teamAbbr: "POR",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2017-18 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2017-18",
    isHistorical: true,
    pts: 26.9,
    reb: 4.5,
    ast: 6.6,
    age: 35,
    nbaId: 203081,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203081.png",
    quote: "Dame Time in Portland."
  },
  {
    id: "allnba-2017-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2017-18 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2017-18",
    isHistorical: true,
    pts: 30.4,
    reb: 5.4,
    ast: 8.8,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "MVP season for the scoring genius."
  },
  {
    id: "allnba-2017-lebron",
    name: "LeBron James",
    team: "Cleveland Cavaliers",
    teamAbbr: "CLE",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2017-18 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2017-18",
    isHistorical: true,
    pts: 27.5,
    reb: 8.6,
    ast: 9.1,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Played all 82 games. Peak durability."
  },
  {
    id: "allnba-2017-durant",
    name: "Kevin Durant",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2017-18 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2017-18",
    isHistorical: true,
    pts: 26.4,
    reb: 6.8,
    ast: 5.4,
    age: 37,
    nbaId: 201142,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201142.png",
    quote: "Unstoppable efficiency on the dynasty."
  },
  {
    id: "allnba-2017-davis",
    name: "Anthony Davis",
    team: "New Orleans Pelicans",
    teamAbbr: "NOP",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2017-18 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2017-18",
    isHistorical: true,
    pts: 28.1,
    reb: 11.1,
    ast: 2.3,
    age: 32,
    nbaId: 203076,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203076.png",
    quote: "Defensive powerhouse and offensive star."
  },
  {
    id: "allnba-2018-curry",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2018-19 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2018-19",
    isHistorical: true,
    pts: 27.3,
    reb: 5.3,
    ast: 5.2,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "The Splash Brother excellence continues."
  },
  {
    id: "allnba-2018-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2018-19 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2018-19",
    isHistorical: true,
    pts: 36.1,
    reb: 6.6,
    ast: 7.5,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "Historic 36 PPG scoring run."
  },
  {
    id: "allnba-2018-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2018-19 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2018-19",
    isHistorical: true,
    pts: 27.7,
    reb: 12.5,
    ast: 5.9,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "First MVP season for the Greek Freak."
  },
  {
    id: "allnba-2018-george",
    name: "Paul George",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2018-19 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2018-19",
    isHistorical: true,
    pts: 28,
    reb: 8.2,
    ast: 4.1,
    age: 21,
    nbaId: 202331,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202331.png",
    quote: "Two-way brilliance in OKC."
  },
  {
    id: "allnba-2018-jokic",
    name: "Nikola Jokic",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2018-19 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2018-19",
    isHistorical: true,
    pts: 20.1,
    reb: 10.8,
    ast: 7.3,
    age: 30,
    nbaId: 203999,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203999.png",
    quote: "The Joker changing the center position."
  },
  {
    id: "allnba-2019-doncic",
    name: "Luka Doncic",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2019-20 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2019-20",
    isHistorical: true,
    pts: 28.8,
    reb: 9.4,
    ast: 8.8,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "Slovenian sensation taking over."
  },
  {
    id: "allnba-2019-harden",
    name: "James Harden",
    team: "Houston Rockets",
    teamAbbr: "HOU",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2019-20 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2019-20",
    isHistorical: true,
    pts: 34.3,
    reb: 6.6,
    ast: 7.5,
    age: 36,
    nbaId: 201935,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201935.png",
    quote: "Unstoppable step-back master."
  },
  {
    id: "allnba-2019-lebron",
    name: "LeBron James",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2019-20 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2019-20",
    isHistorical: true,
    pts: 25.3,
    reb: 7.8,
    ast: 10.2,
    age: 41,
    nbaId: 2544,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/2544.png",
    quote: "Led the league in assists and won a title."
  },
  {
    id: "allnba-2019-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2019-20 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2019-20",
    isHistorical: true,
    pts: 29.5,
    reb: 13.6,
    ast: 5.6,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "Back-to-back MVP and DPOY season."
  },
  {
    id: "allnba-2019-davis",
    name: "Anthony Davis",
    team: "LA Lakers",
    teamAbbr: "LAL",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2019-20 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2019-20",
    isHistorical: true,
    pts: 26.1,
    reb: 9.3,
    ast: 3.2,
    age: 32,
    nbaId: 203076,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203076.png",
    quote: "Championship dominance in the Bubble."
  },
  {
    id: "allnba-2020-curry",
    name: "Stephen Curry",
    team: "Golden State Warriors",
    teamAbbr: "GSW",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2020-21 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2020-21",
    isHistorical: true,
    pts: 32,
    reb: 5.5,
    ast: 5.8,
    age: 37,
    nbaId: 201939,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/201939.png",
    quote: "Scoring champion and offensive engine."
  },
  {
    id: "allnba-2020-doncic",
    name: "Luka Doncic",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2020-21 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2020-21",
    isHistorical: true,
    pts: 27.7,
    reb: 8,
    ast: 8.6,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "Elite playmaking and scoring mastery."
  },
  {
    id: "allnba-2020-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2020-21 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2020-21",
    isHistorical: true,
    pts: 28.1,
    reb: 11,
    ast: 5.9,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "Championship MVP for the Bucks."
  },
  {
    id: "allnba-2020-leonard",
    name: "Kawhi Leonard",
    team: "LA Clippers",
    teamAbbr: "LAC",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2020-21 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2020-21",
    isHistorical: true,
    pts: 24.8,
    reb: 6.5,
    ast: 5.2,
    age: 34,
    nbaId: 202695,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/202695.png",
    quote: "Two-way efficiency personified."
  },
  {
    id: "allnba-2020-jokic",
    name: "Nikola Jokic",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2020-21 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2020-21",
    isHistorical: true,
    pts: 26.4,
    reb: 10.8,
    ast: 8.3,
    age: 30,
    nbaId: 203999,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203999.png",
    quote: "The first center MVP since Shaq."
  },
  {
    id: "allnba-2021-doncic",
    name: "Luka Doncic",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2021-22 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2021-22",
    isHistorical: true,
    pts: 28.4,
    reb: 9.1,
    ast: 8.7,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "Maverick brilliance in Dallas."
  },
  {
    id: "allnba-2021-booker",
    name: "Devin Booker",
    team: "Phoenix Suns",
    teamAbbr: "PHX",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2021-22 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2021-22",
    isHistorical: true,
    pts: 26.8,
    reb: 5,
    ast: 4.8,
    age: 29,
    nbaId: 1626164,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1626164.png",
    quote: "Leading the Suns to a franchise record."
  },
  {
    id: "allnba-2021-tatum",
    name: "Jayson Tatum",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2021-22 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2021-22",
    isHistorical: true,
    pts: 26.9,
    reb: 8,
    ast: 4.4,
    age: 27,
    nbaId: 1628369,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628369.png",
    quote: "Ascending to superstardom in Boston."
  },
  {
    id: "allnba-2021-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2021-22 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2021-22",
    isHistorical: true,
    pts: 29.9,
    reb: 11.6,
    ast: 5.8,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "Dominance in the paint and transition."
  },
  {
    id: "allnba-2021-jokic",
    name: "Nikola Jokic",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2021-22 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2021-22",
    isHistorical: true,
    pts: 27.1,
    reb: 13.8,
    ast: 7.9,
    age: 30,
    nbaId: 203999,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203999.png",
    quote: "Back-to-back MVP excellence."
  },
  {
    id: "allnba-2022-gilgeous-alexander",
    name: "Shai Gilgeous-Alexander",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2022-23 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2022-23",
    isHistorical: true,
    pts: 31.4,
    reb: 4.8,
    ast: 5.5,
    age: 27,
    nbaId: 1628983,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628983.png",
    quote: "Precision scoring and leadership."
  },
  {
    id: "allnba-2022-doncic",
    name: "Luka Doncic",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2022-23 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2022-23",
    isHistorical: true,
    pts: 32.4,
    reb: 8.6,
    ast: 8,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "Offensive genius in Dallas."
  },
  {
    id: "allnba-2022-tatum",
    name: "Jayson Tatum",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2022-23 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2022-23",
    isHistorical: true,
    pts: 30.1,
    reb: 8.8,
    ast: 4.6,
    age: 27,
    nbaId: 1628369,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628369.png",
    quote: "Climbing to the top of the East."
  },
  {
    id: "allnba-2022-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2022-23 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2022-23",
    isHistorical: true,
    pts: 31.1,
    reb: 11.8,
    ast: 5.7,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "Consistent MVP-level dominance."
  },
  {
    id: "allnba-2022-embiid",
    name: "Joel Embiid",
    team: "Philadelphia 76ers",
    teamAbbr: "PHI",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2022-23 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2022-23",
    isHistorical: true,
    pts: 33.1,
    reb: 10.2,
    ast: 4.2,
    age: 31,
    nbaId: 203954,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203954.png",
    quote: "MVP season for the Process."
  },
  {
    id: "allnba-2023-gilgeous-alexander",
    name: "Shai Gilgeous-Alexander",
    team: "Oklahoma City Thunder",
    teamAbbr: "OKC",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2023-24 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2023-24",
    isHistorical: true,
    pts: 30.1,
    reb: 5.5,
    ast: 6.2,
    age: 27,
    nbaId: 1628983,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628983.png",
    quote: "Elite efficiency and clutch genes."
  },
  {
    id: "allnba-2023-doncic",
    name: "Luka Doncic",
    team: "Dallas Mavericks",
    teamAbbr: "DAL",
    position: "G",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2023-24 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2023-24",
    isHistorical: true,
    pts: 33.9,
    reb: 9.2,
    ast: 9.8,
    age: 26,
    nbaId: 1629029,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1629029.png",
    quote: "Historic offensive production."
  },
  {
    id: "allnba-2023-tatum",
    name: "Jayson Tatum",
    team: "Boston Celtics",
    teamAbbr: "BOS",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2023-24 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2023-24",
    isHistorical: true,
    pts: 26.9,
    reb: 8.1,
    ast: 4.9,
    age: 27,
    nbaId: 1628369,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/1628369.png",
    quote: "Leading the Celtics to a historic season."
  },
  {
    id: "allnba-2023-giannis",
    name: "Giannis Antetokounmpo",
    team: "Milwaukee Bucks",
    teamAbbr: "MIL",
    position: "F",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2023-24 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2023-24",
    isHistorical: true,
    pts: 30.4,
    reb: 11.5,
    ast: 6.5,
    age: 31,
    nbaId: 203507,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203507.png",
    quote: "Power and precision in Milwaukee."
  },
  {
    id: "allnba-2023-jokic",
    name: "Nikola Jokic",
    team: "Denver Nuggets",
    teamAbbr: "DEN",
    position: "C",
    rarity: "allnba_1st" as const,
    category: "All-NBA 1st Team" as const,
    subtitle: "2023-24 ALL-NBA FIRST TEAM",
    series: "All-NBA First Team Series",
    year: "2023-24",
    isHistorical: true,
    pts: 26.4,
    reb: 12.4,
    ast: 9,
    age: 30,
    nbaId: 203999,
    imageUrl: "https://cdn.nba.com/headshots/nba/latest/1040x760/203999.png",
    quote: "Third MVP honors for the Serbian star."
  }
];

export const ALL_CARDS: Card[] = [
  ...rawCards.map((card, index) => {
  const teamColor = teamColors[card.team] || '#333';
  const teamLogoUrl = teamLogos[card.team];
  const coach = teamCoaches[card.team];
  
  // Calculate OVR based on stats
  // Formula: 66 + (PTS * 0.8) + (AST * 0.6) + (REB * 0.6)
  const baseOvr = Math.round(66 + (card.pts * 0.8) + (card.ast * 0.6) + (card.reb * 0.6));
  
  let ovr = baseOvr;
  const category = (card as any).category || 'Base';
  const rarity = card.rarity;

  // Apply Hierarchy Ranges (FIFA-style balancing)
  if (category === 'Finals MVP' || category === 'MVP' || card.id.startsWith('mvp')) {
    // MVP / Finals MVP: 95-99
    ovr = Math.max(95, Math.min(baseOvr, 99));
  } else if (category === 'All-Star MVP' || card.id.startsWith('as-mvp')) {
    // All-Star MVP: 92-94
    ovr = Math.max(92, Math.min(baseOvr, 94));
  } else if (category === '6th Man' || card.id.startsWith('6moy')) {
    // 6th Man: 80-84
    ovr = Math.max(80, Math.min(baseOvr, 84));
  } else if ((rarity as string) === 'legend' || card.id.startsWith('record')) {
    // Legends / Records: 92-96
    ovr = Math.max(92, Math.min(baseOvr, 96));
  } else if ((rarity as string) === 'dpoy' || card.id.startsWith('dpoy')) {
    // DPOY: 90-94
    ovr = Math.max(90, Math.min(baseOvr, 94));
  } else if ((rarity as string) === 'roty' || card.id.startsWith('roty')) {
    // ROTY: 86-90
    ovr = Math.max(86, Math.min(baseOvr, 90));
  } else if (card.id.startsWith('mip')) {
    // MIP: 85-92
    ovr = Math.max(85, Math.min(baseOvr, 92));
  } else if (category === 'Draft 2026' || card.id.startsWith('draft26')) {
    // Draft 2026: 78-92 based on projection
    const pickNum = parseInt(card.id.split('-')[1]) || 30;
    ovr = Math.round(92 - (pickNum * 0.4));
    ovr = Math.max(78, Math.min(ovr, 92));
    } else if (category === 'Scoring Champion' || card.id.startsWith('score')) {
    // Scoring Champion: 92-99 based on PPG
    ovr = Math.round(80 + (card.pts * 0.5));
    ovr = Math.max(90, Math.min(ovr, 99));
  } else if (category === 'Coach of the Year' || (rarity as string) === 'coy' || card.id.startsWith('coy')) {
    // Coach of the Year: 90-99 based on wins
    ovr = Math.round(75 + (card.pts * 0.4));
    ovr = Math.max(90, Math.min(ovr, 99));
  } else if (category === 'Hall of Fame' || (rarity as string) === 'hof' || card.id.startsWith('hof')) {
    // Hall of Fame: 94-99
    ovr = Math.max(94, Math.min(baseOvr, 99));
  } else if (category === 'Rising Star' || (rarity as string) === 'rising_star' || card.id.startsWith('rs-')) {
    // Rising Star: 84-90
    ovr = Math.max(84, Math.min(baseOvr, 90));
  } else if (category === 'All-NBA 1st Team' || (rarity as string) === 'allnba_1st' || card.id.startsWith('allnba-')) {
    // All-NBA 1st Team: 94-99
    ovr = Math.max(94, Math.min(baseOvr, 99));
  } else if (category === 'Arena' || card.id.startsWith('arena')) {
    // Arenas: 80-90 based on capacity and age
    const cap = card.pts;
    const year = card.reb;
    ovr = Math.round(80 + (cap / 21000) * 5 + (Math.max(0, (year - 1960) / 60)) * 5);
    ovr = Math.max(80, Math.min(ovr, 92));
  } else if (category === 'Logo' || card.id.startsWith('logo')) {
    // Logos: 85-99 based on wins and titles
    const titles = card.ast;
    const wins = card.pts;
    ovr = Math.round(85 + (wins / 64) * 10 + (titles / 18) * 4);
    ovr = Math.max(85, Math.min(ovr, 99));
  } else {
    // Base Cards
    switch (rarity as string) {
      case 'bench':
        ovr = Math.max(65, Math.min(baseOvr, 74));
        break;
      case 'starter':
        ovr = Math.max(75, Math.min(baseOvr, 83));
        break;
      case 'allstar':
        ovr = Math.max(84, Math.min(baseOvr, 88));
        break;
      case 'franchise':
        ovr = Math.max(89, Math.min(baseOvr, 91));
        break;
      default:
        ovr = Math.min(baseOvr, 99);
    }
  }
  
  return {
    ...card,
    number: index + 1,
    teamColor,
    teamLogoUrl,
    coach,
    rarity: (card as any).category === 'Coach' ? 'coach' : card.rarity,
    category: (card as any).category || 'Base',
    series: (card as any).category === 'Award' || (card as any).category === 'NBA Record' || (card as any).category === 'All-Star MVP' || (card as any).category === 'Finals MVP'
      ? (card.id.startsWith('mvp') ? 'Legendary MVP Series' : 
         card.id.startsWith('dpoy') ? 'DPOY Series' : 
         card.id.startsWith('roty') ? 'ROTY Series' :
         card.id.startsWith('mip') ? 'MIP Series' : 
         card.id.startsWith('record') ? 'NBA Record Series' :
         card.id.startsWith('as-mvp') ? 'All-Star MVP Series' :
         card.id.startsWith('fmvp') ? 'Finals MVP Series' :
         '6MOY Series') 
      : (card as any).category === 'Draft 2026' ? '2026 Prospect Class' :
        (card as any).category === 'Scoring Champion' ? 'NBA Scoring Champions' :
        (card as any).category === 'Hall of Fame' ? 'NBA Hall of Fame' :
        (card as any).category === 'All-NBA 1st Team' ? 'All-NBA First Team Series' :
        (card as any).category === 'Coach of the Year' ? 'NBA Coach of the Year' :
        (card as any).category === 'Logo' ? 'NBA Team Logo Series' : 
        (card as any).category === 'Arena' ? 'NBA Arena Series' : undefined,
    subtitle: (card as any).category === 'Coach' 
      ? 'HEAD COACH' 
      : (card as any).category === 'Logo'
        ? 'NBA LOGO SERIES'
        : (card as any).category === 'Arena'
          ? 'OFFICIAL TEAM ARENA'
          : (card as any).category === 'Draft 2026'
            ? `PICK #${card.id.split('-')[1]} • 2026 PROSPECT`
            : (card as any).category === 'Scoring Champion'
              ? `${(card as any).year} SCORING CHAMPION`
              : (card as any).category === 'Hall of Fame'
                ? `CLASS OF ${(card as any).year} • HOF`
                : (card as any).category === 'Coach of the Year'
                  ? `${(card as any).year} COACH OF THE YEAR`
                  : (card as any).category === 'Award' || (card as any).category === 'NBA Record' || (card as any).category === 'All-Star MVP' || (card as any).category === 'Finals MVP'
        ? `${(card as any).year} ${
            card.id.startsWith('mvp') ? 'MVP' : 
            card.id.startsWith('dpoy') ? 'DPOY' : 
            card.id.startsWith('roty') ? 'ROTY' : 
            card.id.startsWith('mip') ? 'MIP' : 
            card.id.startsWith('record') ? 'NBA RECORD' :
            card.id.startsWith('as-mvp') ? 'ALL-STAR MVP' :
            card.id.startsWith('fmvp') ? 'FINALS MVP' :
            '6MOY'
          }` 
        : (card as any).category === 'Duo'
          ? 'DYNAMIC DUO'
          : (card as any).category === 'Moment'
            ? `${(card as any).year} ICONIC MOMENT`
            : (card as any).category === 'Dynasty'
              ? 'TEAM DYNASTY'
              : (card as any).category === 'X-Factor'
                ? 'IMPACT PLAYER'
                : (card as any).category === 'All-NBA 1st Team'
                  ? `${(card as any).year} ALL-NBA FIRST TEAM`
                  : `${card.rarity.toUpperCase()} EDITION`,
    stats: {
      points: card.pts,
      rebounds: card.reb,
      assists: card.ast,
      ovr: (card as any).category === 'Coach' ? (card as any).ovr || 90 : ovr,
    },
    description: card.quote,
  };
}),
  ...AWARD_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + idx + 1,
    teamLogoUrl: teamLogos[card.team]
  })),
  ...DUO_AND_XFACTOR_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + AWARD_CARDS.length + idx + 1,
    teamLogoUrl: teamLogos[card.team] || teamLogos[card.teamAbbr]
  })),
  ...DYNASTY_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + AWARD_CARDS.length + DUO_AND_XFACTOR_CARDS.length + idx + 1,
    teamLogoUrl: teamLogos[card.team] || teamLogos[card.teamAbbr]
  })),
  ...ALL_STAR_MVP_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + AWARD_CARDS.length + DUO_AND_XFACTOR_CARDS.length + DYNASTY_CARDS.length + idx + 1,
    teamLogoUrl: teamLogos[card.team] || teamLogos[card.teamAbbr]
  })),
  ...SCREAM_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + AWARD_CARDS.length + DUO_AND_XFACTOR_CARDS.length + DYNASTY_CARDS.length + ALL_STAR_MVP_CARDS.length + idx + 1,
    teamLogoUrl: teamLogos[card.team] || teamLogos[card.teamAbbr]
  })),
  ...HIDDEN_GEMS_CARDS.map((card, idx) => ({
    ...card,
    number: rawCards.length + AWARD_CARDS.length + DUO_AND_XFACTOR_CARDS.length + DYNASTY_CARDS.length + ALL_STAR_MVP_CARDS.length + SCREAM_CARDS.length + idx + 1,
    teamLogoUrl: teamLogos[card.team] || teamLogos[card.teamAbbr]
  }))
];

// Optimized pools for faster pack opening
export const CARDS_BY_RARITY = ALL_CARDS.reduce((acc, card) => {
  if (!acc[card.rarity]) acc[card.rarity] = [];
  acc[card.rarity].push(card);
  return acc;
}, {} as Record<Rarity, Card[]>);

export const CARDS_BY_SERIES = ALL_CARDS.reduce((acc, card) => {
  if (card.series) {
    if (!acc[card.series]) acc[card.series] = [];
    acc[card.series].push(card);
  }
  return acc;
}, {} as Record<string, Card[]>);
