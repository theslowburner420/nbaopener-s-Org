...
const TournamentSummaryModal = memo<{
  ...
  onClaimHome?: () => void;
  isSaving: boolean;
}>(({ show, position, rewards, tournament, onClaim, onClaimHome, isSaving }) => {
  if (!show || !rewards) return null;

  const isHalloween = tournament?.id === 'halloween_scream' || 
    tournament?.name?.toLowerCase().includes('scream') || 
    tournament?.name?.toLowerCase().includes('spooky') ||
    rewards.packs.some(p => p.type === 'scream_edition' || p.name?.toLowerCase().includes('scream'));

  const isChampion = position === 'champion';
  const isFinalist = position === 'finalist';
  const isSemis = position === 'semis';

  const positionTitle = isChampion
    ? '🏆 TOURNAMENT CHAMPION'
    : isFinalist
    ? '🥈 RUNNER-UP'
    : isSemis
  ...
  const regularPacks = (rewards?.packs || []).filter((p: any) => !p.isDirectCard);

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 z-[10000] bg-black/90 backdrop-blur-xl flex items-center justify-center p-3 sm:p-6 overflow-y-auto"
    >
      <motion.div
        initial={{ scale: 0.92, y: 20 }}
        animate={{ scale: 1, y: 0 }}
        className={`w-full max-w-md bg-zinc-950 border rounded-3xl p-5 sm:p-7 text-center space-y-6 shadow-2xl relative overflow-hidden my-auto ${
          isHalloween 
            ? 'border-orange-500/50 shadow-[0_0_50px_rgba(249,115,22,0.25)]' 
            : 'border-zinc-800 shadow-[0_0_50px_rgba(245,158,11,0.15)]'
        }`}
      >
        {/* Glow ambient background */}
        <div className={`absolute -top-20 left-1/2 -translate-x-1/2 w-48 h-48 rounded-full blur-[80px] pointer-events-none ${
          isHalloween ? 'bg-orange-500/20' : 'bg-amber-500/15'
        }`} />
        ...
        <div className="space-y-3 relative z-10">
          <div className="flex justify-center">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg transform transition-transform ${
              ...
              {isHalloween ? <Ghost size={32} /> : <Trophy size={32} />}
            </div>
          </div>

          <div className="space-y-1">
            {isHalloween && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-orange-500/20 border border-orange-500/40 text-[9px] font-black uppercase tracking-widest text-orange-400">
                <Ghost size={11} /> 🎃 Halloween Special Event
              </span>
            )}
            <h2 className="text-2xl sm:text-3xl font-black italic uppercase tracking-tight text-white leading-none">
              {isChampion ? 'Championship Victory!' : 'Tournament Complete'}
            </h2>
            <div className="pt-1">
              <span className={`inline-block px-3.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                isChampion 
        ...
        <div className="space-y-3 relative z-10">
          <div className="flex items-center justify-between border-b border-zinc-900 pb-1.5">
            <span className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500">Tournament Rewards</span>
            {tournament && (
              <span className="text-[9px] font-bold text-zinc-500 truncate max-w-[150px]">{tournament.name}</span>
            )}
          </div>

          <div className="grid grid-cols-2 gap-2.5">
            {/* Coins Reward */}
            <div className="bg-zinc-900/60 border border-zinc-850 rounded-2xl p-3.5 flex flex-col items-center justify-center gap-1.5 hover:border-amber-500/30 transition-all">
              <div className="w-9 h-9 bg-amber-500/15 border border-amber-500/30 rounded-xl flex items-center justify-center text-amber-500">
                <Coins size={18} />
              </div>
              <p className="text-lg sm:text-xl font-black italic text-white leading-tight">{rewards?.coins?.toLocaleString() || '0'}</p>
              <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest leading-none">Coins</p>
            </div>

            {/* Packs Reward */}
            <div className="bg-zinc-900/60 border border-zinc-850 rounded-2xl p-3.5 flex flex-col items-center justify-center gap-1.5 hover:border-amber-500/30 transition-all">
              <div className="w-9 h-9 bg-blue-500/15 border border-blue-500/30 rounded-xl flex items-center justify-center text-blue-400">
                <Package size={18} />
              </div>
              <p className="text-lg sm:text-xl font-black italic text-white leading-tight">{regularPacks.length}</p>
              <p className="text-[8px] font-bold text-zinc-500 uppercase tracking-widest leading-none">Packs</p>
            </div>
          </div>
          ...
          {regularPacks.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1.5 pt-1">
              {regularPacks.map((pack: any, idx: number) => {
                const isScreamPack = pack.type === 'scream_edition' || pack.name?.toLowerCase().includes('scream');
                return (
                  <div 
                    key={idx} 
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[9px] font-black border transition-all ${
                      isScreamPack 
                        ? 'bg-orange-500/15 border-orange-500/50 text-orange-300' 
                        : 'bg-zinc-900 border-zinc-800 text-zinc-300'
                    }`}
                  >
                    {isScreamPack ? <Ghost size={11} className="text-orange-400" /> : <Package size={11} className="text-blue-400" />}
        ...
        <div className="flex flex-col gap-2 relative z-10 w-full pt-1">
          <button
            onClick={onClaim}
            disabled={isSaving}
            className={`w-full py-3.5 rounded-2xl font-black uppercase tracking-wider text-xs flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 disabled:opacity-50 ${
              isHalloween 
          ...
          >
            <span>{isSaving ? 'Saving...' : 'Claim & New Draft'}</span>
            <ArrowRight size={14} />
          </button>
          ...
          {onClaimHome && (
            <button
              onClick={onClaimHome}
              disabled={isSaving}
              className="w-full bg-zinc-900 text-zinc-400 hover:text-white border border-zinc-800 py-2.5 rounded-xl font-bold uppercase tracking-wider text-[10px] flex items-center justify-center gap-1.5 hover:bg-zinc-850 transition-all disabled:opacity-50"
            >
              <Home size={13} />
...
const MatchScoreboard = memo<{
  ...
  return (
    <div className="relative z-[500] bg-zinc-950/90 backdrop-blur-xl border-b border-zinc-850 px-3 sm:px-6 py-2 sm:py-3.5 shrink-0 shadow-lg select-none">
      <div className="max-w-4xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
        {/* Team 1 (User / Opponent) */}
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center text-xs sm:text-lg font-black shrink-0 transition-transform ${
            isUserT1 
              ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.25)] ring-1 ring-amber-400' 
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5">
              <p className="text-xs sm:text-base font-black uppercase italic text-white truncate leading-tight">{t1Name}</p>
              {isUserT1 && <span className="text-[7.5px] px-1 py-0.2 rounded bg-amber-500/20 text-amber-400 font-bold hidden sm:inline">YOU</span>}
            </div>
            <p className="text-[8px] sm:text-[9px] font-bold text-zinc-500 uppercase tracking-widest leading-none mt-0.5 font-mono">{t1Ovr} OVR</p>
          </div>
        </div>

        {/* Score & Quarter Center Display */}
        <div className="flex flex-col items-center justify-center px-2 sm:px-6 shrink-0 space-y-1">
          <div className="flex items-center gap-2 sm:gap-4 font-mono">
            <span className={`text-2xl sm:text-5xl font-black italic tabular-nums leading-none tracking-tight transition-colors ${
              s1 >= s2 ? 'text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]' : 'text-zinc-500'
            }`}>
              {s1}
            </span>
            <span className="text-xs sm:text-base font-black text-zinc-700">:</span>
            <span className={`text-2xl sm:text-5xl font-black italic tabular-nums leading-none tracking-tight transition-colors ${
              s2 >= s1 ? 'text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.2)]' : 'text-zinc-500'
            }`}>
              {s2}
            </span>
          </div>
          <div className="inline-flex items-center gap-1.5 px-2 sm:px-2.5 py-0.5 rounded-full bg-zinc-900 border border-zinc-800 shadow-inner">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
            <span className="text-[7.5px] sm:text-[9px] font-black text-amber-400 uppercase tracking-[0.15em] whitespace-nowrap">
              QUARTER {quarter}
            </span>
          </div>
        </div>
        ...
        {/* Team 2 */}
        <div className="flex items-center flex-row-reverse gap-2 sm:gap-3 flex-1 min-w-0 text-right">
          <div className={`w-9 h-9 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center text-xs sm:text-lg font-black shrink-0 transition-transform ${
            isUserT2 
              ? 'bg-amber-500 text-black shadow-[0_0_15px_rgba(245,158,11,0.25)] ring-1 ring-amber-400' 
              : 'bg-zinc-900 border border-zinc-800 text-zinc-300'
          ...
          <div className="flex flex-col items-end min-w-0">
            <div className="flex items-center gap-1.5 flex-row-reverse">
              <p className="text-xs sm:text-base font-black uppercase italic text-white truncate leading-tight">{t2Name}</p>
              {isUserT2 && <span className="text-[7.5px] px-1 py-0.2 rounded bg-amber-500/20 text-amber-400 font-bold hidden sm:inline">YOU</span>}
            </div>
            <p className="text-[8px] sm:text-[9px] font-bold text-zinc-500 uppercase tracking-widest leading-none mt-0.5 font-mono">{t2Ovr} OVR</p>
          </div>
        </div>
      </div>
    </div>
...
const MatchChatbox = memo<{
  ...
  const getEventIcon = (text: string) => {
    ...
    if (t.includes('dunk') || t.includes('slam') || t.includes('poster')) return <Flame size={12} className="text-orange-400 shrink-0" />;
    if (t.includes('3pt') || t.includes('three') || t.includes('triple')) return <Zap size={12} className="text-amber-400 shrink-0" />;
    if (t.includes('steal') || t.includes('block') || t.includes('reject')) return <Shield size={12} className="text-blue-400 shrink-0" />;
    if (t.includes('interactive:')) return <Sparkles size={12} className="text-amber-300 shrink-0" />;
    return <Play size={10} className="text-zinc-400 shrink-0" fill="currentColor" />;
  };

  return (
    <div className="max-w-2xl mx-auto w-full flex-1 flex flex-col gap-1.5 sm:gap-2 overflow-y-auto pr-1 sm:pr-2 scrollbar-none flex-col-reverse select-none">
      <AnimatePresence initial={false} mode="popLayout">
        {visibleEvents.map((event) => {
          const isInteractive = event.text.startsWith('INTERACTIVE:');
          const isUser = event.team === 'USER';
          ...
          return (
            <motion.div
              ...
              className={`p-2.5 sm:p-3 rounded-2xl border transition-all ${
                isInteractive
                  ? 'bg-amber-500/15 border-amber-500/50 text-white shadow-[0_0_15px_rgba(245,158,11,0.15)]'
                  : isUser 
                  ? 'bg-gradient-to-r from-zinc-900/90 to-zinc-900/60 border-amber-500/30' 
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-1">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-[7.5px] sm:text-[8px] font-black uppercase tracking-widest ${
                    isInteractive ? 'text-amber-400' : isUser ? 'text-amber-400' : 'text-zinc-500'
                  }`}>
                    {isInteractive ? 'Key Moment' : isUser ? 'Your Team' : 'Opponent'}
                  </span>
                </div>
                <span className="text-[7.5px] sm:text-[8px] font-mono font-bold text-zinc-500 bg-zinc-900/80 px-1.5 py-0.5 rounded-md border border-zinc-800">
                  Q{event.quarter} • {event.score1}-{event.score2}
                </span>
              </div>
              <p className={`text-xs sm:text-sm font-bold italic tracking-tight leading-snug ${
                isInteractive ? 'text-amber-300 font-black' : isUser ? 'text-zinc-100' : 'text-zinc-300'
              }`}>
                {event.text?.replace('INTERACTIVE: ', '') || ''}
              </p>
            </motion.div>
          );
        })}
      ...
      {events.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 space-y-3">
          <div className="w-9 h-9 border-2 border-zinc-850 border-t-amber-500 rounded-full animate-spin" />
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 animate-pulse">Waiting for tip-off...</p>
        </div>
      )}
    </div>
  );
...
const InteractiveMeter = memo<{
  ...
  return (
    <div className="w-full space-y-4 py-2">
      <div className="relative w-full h-8 bg-zinc-950 rounded-full overflow-hidden border border-zinc-800 shadow-inner">
        {/* Target Zone */}
        <div 
          className="absolute h-full bg-emerald-500/40 border-l-2 border-r-2 border-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.3)]"
          style={{ left: `${targetMin}%`, width: `${targetMax - targetMin}%` }}
        />
        {/* Indicator */}
        <div 
          className="absolute h-full w-2.5 bg-white rounded-full shadow-[0_0_12px_rgba(255,255,255,0.9)] transition-none"
      ...
      <button 
        onClick={handleStop}
        disabled={hasStopped}
        className="w-full bg-amber-500 text-black py-3.5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-amber-400 transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-amber-500/20"
      >
        RELEASE SHOT NOW!
      </button>
      <p className="text-[8.5px] font-bold text-zinc-500 uppercase tracking-widest text-center">Hit the center green zone</p>
    </div>
  );
});
...
const InteractiveTap = memo<{
  ...
  return (
    <div className="w-full space-y-4 py-2 flex flex-col items-center">
      <div 
        onClick={handleTap}
      ...
      >
        <svg className="w-full h-full transform -rotate-90">
          ...
          <circle 
            ...
            strokeDasharray={301}
            strokeDashoffset={301 - (progress / target) * 301}
          />
        </svg>
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-2xl font-black italic text-white font-mono">{progress}</span>
          <span className="text-[7.5px] font-bold text-zinc-500 uppercase tracking-widest">/ {target}</span>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-3 bg-amber-500 hover:bg-amber-400 text-black rounded-2xl font-black uppercase tracking-wider text-xs shadow-lg shadow-amber-500/20 active:scale-95 transition-all"
      >
        TAP FAST! ({Math.ceil(timeLeft)}s)
      </button>
    </div>
  );
});
...
const DraftView: React.FC = () => {
  ...
  const [mobileRoundTab, setMobileRoundTab] = useState<'QF' | 'SF' | 'F' | 'TREE'>('QF');
  ...
  const handleStartDraft = async (method: 'coins' | 'ad') => {
    if (method === 'coins') {
      ...
      if (!isPremium) {
        ...
        const watched = await showRewardedAd(async () => {}, {
          rewardText: 'Draft Tournament Entry',
          durationSeconds: 10,
        });
            ...
            {currentOptions.map((card, idx) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ willChange: isSelected || !isAnySelected ? 'transform, opacity' : 'auto' }}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-wider whitespace-nowrap transition-all flex items-center gap-1.5 ${
                    tournamentCategory === tab.id
                      ...
                      : tab.isSpecial
                      ? 'bg-gradient-to-r from-orange-950/40 to-purple-950/40 text-orange-400 hover:text-orange-300 border border-orange-500/40'
                      : 'bg-zinc-900 text-zinc-400 hover:text-white hover:bg-zinc-800 border border-zinc-800/80'
                  }`}
                >
                  <span>{tab.label}</span>
                  <span className={`text-[9px] px-1.5 py-0.2 rounded-full ${
                    tournamentCategory === tab.id ? 'bg-black/20 text-white' : 'bg-zinc-800 text-zinc-400'
                  }`}>
                    {tab.count}
                  </span>
                </button>
          ...
          <div className="max-w-6xl mx-auto space-y-6 pb-24">
              ...
              (() => {
                ...
                return (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="relative overflow-hidden rounded-3xl p-5 sm:p-7 bg-gradient-to-r from-orange-950/60 via-purple-950/50 to-zinc-950 border-2 border-orange-500/60 shadow-[0_0_30px_rgba(249,115,22,0.2)]"
                  >
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
                      <div className="space-y-2 max-w-2xl">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-orange-500 to-purple-600 text-white text-[9px] font-black uppercase tracking-widest shadow-md">
                            <Flame size={12} className="animate-pulse" />
                            <span>EVENTO DESTACADO • TIEMPO LIMITADO</span>
                          </span>
                          <span className="px-2.5 py-0.5 rounded-full bg-black/60 border border-orange-500/40 text-orange-300 font-mono text-[9px] font-bold">
                            Rec: {specialTournament.recommendedOvr} OVR
                          </span>
                        </div>

                        <h2 className="text-xl sm:text-2xl md:text-3xl font-black italic uppercase tracking-tight text-white flex items-center gap-2.5">
                          {isHalloween && <Ghost size={26} className="text-orange-400 animate-bounce shrink-0" />}
                          <span>{specialTournament.name}</span>
                        </h2>

                        <p className="text-xs text-zinc-300 font-medium leading-relaxed">
                          {specialTournament.description}
                        </p>

                        <div className="flex flex-wrap items-center gap-2 pt-1">
                          <span className="text-[9px] font-black uppercase tracking-wider text-orange-400">Exclusive Rewards:</span>
                          <div className="flex flex-wrap gap-1.5">
                            {specialTournament.rewards.split('+').map((r, i) => (
                              <span key={i} className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg bg-black/60 border border-orange-500/30 text-orange-200 text-[9px] font-bold">
                      ...
                      <div className="flex sm:flex-col gap-2.5 w-full md:w-auto shrink-0">
                        <button
                          onClick={() => handleSelectTournament(specialTournament)}
                          className="flex-1 sm:flex-initial px-6 py-3.5 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-400 hover:to-amber-400 text-black font-black uppercase tracking-wider text-xs rounded-2xl shadow-lg shadow-orange-500/25 active:scale-95 transition-all flex items-center justify-center gap-2"
                        >
                          <span>Play Event</span>
                          <ArrowRight size={14} />
                        </button>
                        <button
                          onClick={() => setPreviewTournament(specialTournament)}
                          className="px-4 py-3.5 bg-zinc-900/80 hover:bg-zinc-800 text-zinc-300 hover:text-white border border-zinc-800 rounded-2xl text-xs font-bold transition-all flex items-center justify-center gap-1.5"
                        >
                          <Info size={14} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
              {sortedTournaments.map((t, idx) => {
                ...
                const isSpecial = t.category === 'special' || isHalloween;
                  ...
                  <motion.div
                    ...
                    className={`group rounded-2xl p-5 flex flex-col justify-between text-left space-y-4 shadow-xl hover:shadow-2xl transition-all duration-300 relative overflow-hidden ${
                      isHalloween
                        ...
                        : isSpecial
                        ? 'bg-gradient-to-b from-amber-950/30 to-zinc-950 border-2 border-amber-500/60 hover:border-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.2)]'
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-1.5 relative z-10">
                      <span className={`px-2.5 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-wider ${
                        ...
                        t.category === 'special' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-black font-black' :
                        ...
                        {isHalloween ? '🎃 HALLOWEEN SPECIAL' : t.category === 'special' ? '⭐ SPECIAL EVENT' : t.difficulty}
                      </span>

                      <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-wider border font-mono ${
                        isHalloween ? 'bg-black/60 text-orange-300 border-orange-500/40' : 
                        isSpecial ? 'bg-black/60 text-amber-300 border-amber-500/40' : 
                    ...
                    <div className="space-y-1.5 relative z-10">
                      <h3 className={`text-lg font-black italic uppercase tracking-tight leading-tight transition-colors ${
                        isHalloween ? 'text-orange-400 group-hover:text-orange-300 flex items-center gap-1.5' : 
                        isSpecial ? 'text-amber-400 group-hover:text-amber-300 flex items-center gap-1.5' :
                        'text-white group-hover:text-amber-400'
                      }`}>
                        {isHalloween ? (
                          <Ghost size={18} className="text-orange-400 animate-pulse shrink-0" />
                        ) : isSpecial ? (
                    ...
                    <div className={`space-y-1.5 rounded-xl p-2.5 relative z-10 ${
                      isHalloween ? 'bg-black/60 border border-orange-500/40' : 
                      isSpecial ? 'bg-black/60 border border-amber-500/40' :
                      'bg-zinc-900/50 border border-zinc-850'
                    }`}>
                      <span className={`text-[8px] font-black uppercase tracking-widest block ${
                        isHalloween ? 'text-orange-400' : isSpecial ? 'text-amber-400' : 'text-zinc-500'
                      }`}>
                        {isHalloween ? '🎃 SPOOKY REWARDS' : isSpecial ? '⭐ SPECIAL REWARDS' : 'Tournament Rewards'}
                      </span>
  ...
  const renderBracket = () => {
    ...
    const userMatch = (() => {
      if (currentRound === 'QF') return bracket.find(m => m.id === 'qf1' || (m.team1 === 'USER' || m.team2 === 'USER'));
      if (currentRound === 'SF') return bracket.find(m => m.id === 'sf1' || (m.team1 === 'USER' || m.team2 === 'USER'));
      ...
      <div className="h-full w-full flex flex-col bg-zinc-950 overflow-hidden relative text-white">
        ...
        <header className="px-4 py-3 bg-black/90 border-b border-zinc-900 flex items-center justify-between gap-2 z-30 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-xs sm:text-sm font-black italic uppercase text-white leading-none tracking-tight flex items-center gap-1.5">
                {selectedTournament?.name || 'Tournament'}
              </h2>
              <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider mt-0.5">
                Playoffs • Your OVR: <span className="text-amber-400 font-mono">{teamOVR}</span>
              </p>
            </div>
          </div>
          ...
          <button 
            ...
            className="flex items-center gap-1 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg text-[9px] font-black uppercase tracking-wider text-zinc-400 hover:text-white transition-all active:scale-95"
          >
            <ArrowLeft size={10} />
            <span>Exit</span>
          </button>
        ...
        {/* Round Navigation Tabs - Clean & Mobile Friendly */}
        <div className="px-4 pt-3 pb-2 bg-zinc-950 border-b border-zinc-900/60 z-20 shrink-0">
          <div className="flex bg-zinc-900 p-1 rounded-xl border border-zinc-850 max-w-md mx-auto w-full gap-1">
            <button
            ...
            <button
              onClick={() => setMobileRoundTab('TREE')}
        ...
        {/* Main Content Area */}
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 max-w-3xl mx-auto w-full z-10">
          {mobileRoundTab !== 'TREE' ? (
            /* TABBED ROUND VIEW: Ultra Clean & Minimalist Cards */
            <div className="space-y-4 pb-24">
              <div className="flex items-center justify-between border-b border-zinc-900 pb-2">
                <h3 className="text-xs font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                  {mobileRoundTab === 'QF' && 'Quarter-Final Matchups'}
                  {mobileRoundTab === 'SF' && 'Semi-Final Matchups'}
                  {mobileRoundTab === 'F' && 'Championship Final'}
                </h3>
                <span className="text-[9px] font-mono text-zinc-500 font-bold">
                  {mobileRoundTab === 'QF' && '4 Games'}
                  {mobileRoundTab === 'SF' && '2 Games'}
                  {mobileRoundTab === 'F' && '1 Game'}
                </span>
              </div>

              {/* Match List */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {mobileRoundTab === 'QF' && qfMatches.map((m) => (
                  <BracketMatchCard
                    key={m.id}
                    match={m}
                    isUserMatch={isUserSideMatch(m.id)}
                    onSimulate={() => simulateMatch(m.id)}
                    isSimulating={isSimulating && activeMatchId === m.id}
                    teamOVR={teamOVR}
                    isActive={currentRound === 'QF' && isUserSideMatch(m.id) && m.status === 'pending'}
                  />
                ))}

                {mobileRoundTab === 'SF' && sfMatches.map((m) => (
                  <BracketMatchCard
                    key={m.id}
                    match={m}
                    isUserMatch={isUserSideMatch(m.id)}
                    ...
                    isSimulating={isSimulating && activeMatchId === m.id}
                    teamOVR={teamOVR}
                    isActive={currentRound === 'SF' && isUserSideMatch(m.id) && m.status === 'pending'}
                  />
                ))}

                {mobileRoundTab === 'F' && fMatch && (
                  <div className="sm:col-span-2 max-w-md mx-auto w-full space-y-3">
                    <div className="flex flex-col items-center justify-center p-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl text-center space-y-2">
                      <Trophy size={32} className="text-amber-500 animate-bounce" />
                      <h4 className="text-base font-black italic uppercase text-white">Championship Trophy Game</h4>
                      <p className="text-[10px] text-zinc-400">Winner takes the tournament crown & all bonus rewards.</p>
                    </div>
            ...
            <div className="flex-1 flex flex-col space-y-3 pb-24">
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Full Bracket Flow</span>
                <div className="flex gap-1">
                  <button onClick={() => handleScrollToSection('west')} className="px-2 py-0.5 bg-blue-500/10 text-blue-400 rounded text-[8px] font-bold uppercase">West</button>
                  <button onClick={() => handleScrollToSection('finals')} className="px-2 py-0.5 bg-amber-500/10 text-amber-400 rounded text-[8px] font-bold uppercase">Finals</button>
                  <button onClick={() => handleScrollToSection('east')} className="px-2 py-0.5 bg-red-500/10 text-red-400 rounded text-[8px] font-bold uppercase">East</button>
                </div>
              </div>

              <div className="overflow-x-auto bracket-container flex items-center touch-pan-x snap-x pb-4 scrollbar-hide" ref={bracketRef}>
                <div className="flex items-center gap-8 min-w-max px-4 relative h-[420px]">
                  {/* West */}
                  <div className="flex items-center gap-8 h-full relative" id="bracket-west">
                    <div className="bracket-round gap-10">
                      {['qf1', 'qf2'].map((id, i) => {
                        ...
                        return (
                          <div key={id} className="bracket-match-wrapper" data-match-id={id}>
                            <BracketMatchCard 
                              ...
                              teamOVR={teamOVR}
                              isActive={currentRound === 'QF' && isUserSideMatch(id) && m?.status === 'pending'}
                            />
                            <BracketConnector type="west" round="qf" index={i} active={m?.status === 'finished' && isUserSideMatch(id) && m?.winner === 'USER'} />
                          </div>
                        );
                      })}
                    </div>
                    <div className="bracket-round">
                      <div className="bracket-match-wrapper" data-match-id="sf1">
                        ...
                        />
                        <BracketConnector type="west" round="sf" index={0} active={isUserSideMatch('sf1') && sfMatches.find(m => m.id === 'sf1')?.winner === 'USER'} />
                      </div>
                    </div>
                  </div>

                  {/* Finals */}
                  <div className="flex flex-col items-center justify-center px-6 relative min-w-[280px]" id="bracket-finals" data-match-id="f1">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <h4 className="text-xl font-black italic uppercase text-white tracking-tight">Finals</h4>
                      <BracketMatchCard 
                        match={fMatch || null} 
                        isUserMatch={fMatch?.team1 === 'USER' || fMatch?.team2 === 'USER'} 
                  ...
                  {/* East */}
                  <div className="flex flex-row-reverse items-center gap-8 h-full relative" id="bracket-east">
                    <div className="bracket-round gap-10">
                      {['qf3', 'qf4'].map((id, i) => {
                        const m = qfMatches.find(match => match.id === id);
                        return (
                          <div key={id} className="bracket-match-wrapper" data-match-id={id}>
                            <BracketMatchCard 
                              ...
                              isActive={currentRound === 'QF' && isUserSideMatch(id) && m?.status === 'pending'}
                            />
                            <BracketConnector type="east" round="qf" index={i} active={m?.status === 'finished' && isUserSideMatch(id) && m?.winner === 'USER'} />
                          </div>
                        );
                      })}
                    </div>
                    <div className="bracket-round">
                      <div className="bracket-match-wrapper" data-match-id="sf2">
                        ...
                        />
                        <BracketConnector type="east" round="sf" index={0} active={isUserSideMatch('sf2') && sfMatches.find(m => m.id === 'sf2')?.winner === 'USER'} />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            >
              {userMatch.status === 'pending' ? (
                <button
                  ...
                  className="w-full py-3.5 bg-amber-500 hover:bg-amber-400 text-black rounded-2xl font-black uppercase tracking-wider text-xs shadow-xl shadow-amber-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
                >
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-white hover:bg-zinc-200 text-black rounded-2xl font-black uppercase tracking-wider text-xs shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all"
                >
                  <Trophy size={16} className="text-amber-500" />
                  <span>{userMatch.winner === 'USER' ? (currentRound === 'F' ? 'Claim Championship!' : 'Next Round Result') : 'Claim Rewards & Exit'}</span>
                </button>
              ) : null}
            </motion.div>
          )}
...
const BracketMatchCard = memo<{ 
  ...
  return (
    <motion.div
      ...
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      className={`relative w-full ${isFinal ? 'max-w-md mx-auto' : 'max-w-sm'} transition-all duration-300 group z-10`}
    >
      {isUserMatch && !isFinished && (
        <div className="absolute -top-2 left-3 z-20 bg-amber-500 text-black text-[7.5px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider leading-none shadow-md border border-black animate-pulse">
          Your Match
        </div>
      )}
      <div className={`
        relative overflow-hidden rounded-xl border transition-all duration-300
        ${isFinished ? 'bg-zinc-950/70 border-zinc-900 shadow-inner' : 
          isActive ? 'bg-zinc-900 border-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.2)]' : 
          'bg-zinc-950 border-zinc-900'}
        ${isFinal ? 'border-amber-500/50 shadow-lg' : ''}
        ${isUserMatch && !isFinished ? 'ring-1 ring-amber-500/50' : ''}
      `}>
        {/* Progress indicator */}
        {!isFinished && isActive && (
        ...
        {/* Team 1 */}
        <div className={`
          flex items-center justify-between px-3 py-2.5 border-b border-zinc-900 transition-all gap-2
          ${isLoser(match.team1) ? 'opacity-35' : 'opacity-100'}
          ${isWinner(match.team1) ? 'bg-amber-500/10' : ''}
        `}>
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[8px] font-black shadow text-white shrink-0 bg-gradient-to-br ${getTeamColor(getTeamAbbr(getTeamName(match.team1)))} ${getTeamAbbr(getTeamName(match.team1)) === 'YOU' ? 'text-black' : ''}`}>
              {getTeamAbbr(getTeamName(match.team1))}
            </div>
            <div className="flex flex-col min-w-0 flex-1">
              <span className={`text-[11px] font-black uppercase italic tracking-tight truncate leading-tight ${isWinner(match.team1) ? 'text-amber-400' : 'text-zinc-200'}`}>
                {getTeamName(match.team1)}
              </span>
              {!isFinished && <span className="text-[7.5px] font-bold text-zinc-500 uppercase font-mono mt-0.5">{getTeamOvr(match.team1)} OVR</span>}
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0 pl-1">
            {isFinished ? (
              <span className={`text-base font-black italic font-mono ${isWinner(match.team1) ? 'text-amber-400' : 'text-zinc-700'}`}>
                {match.score1}
              </span>
            ) : (
               <Shield size={12} className={match.team1 === 'USER' ? 'text-amber-500' : 'text-zinc-800'} />
            )}
        ...
        {/* Team 2 */}
        <div className={`
          flex items-center justify-between px-3 py-2.5 transition-all gap-2
          ${isLoser(match.team2) ? 'opacity-35' : 'opacity-100'}
          ${isWinner(match.team2) ? 'bg-amber-500/10' : ''}
        `}>
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[8px] font-black shadow text-white shrink-0 bg-gradient-to-br ${getTeamColor(getTeamAbbr(getTeamName(match.team2)))} ${getTeamAbbr(getTeamName(match.team2)) === 'YOU' ? 'text-black' : ''}`}>
              {getTeamAbbr(getTeamName(match.team2))}
            </div>
            <div className="flex flex-col min-w-0 flex-1">
              <span className={`text-[11px] font-black uppercase italic tracking-tight truncate leading-tight ${isWinner(match.team2) ? 'text-amber-400' : 'text-zinc-200'}`}>
                {getTeamName(match.team2)}
              </span>
              {!isFinished && <span className="text-[7.5px] font-bold text-zinc-500 uppercase font-mono mt-0.5">{getTeamOvr(match.team2)} OVR</span>}
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0 pl-1">
            {isFinished ? (
              <span className={`text-base font-black italic font-mono ${isWinner(match.team2) ? 'text-amber-400' : 'text-zinc-700'}`}>
                {match.score2}
              </span>
            ) : (
              <Shield size={12} className={match.team2 === 'USER' ? 'text-amber-500' : 'text-zinc-800'} />
            )}
        ...
        {/* Action Button inside card when active */}
        {isActive && !isSimulating && (
          <button 
            onClick={onSimulate}
            className="w-full py-2 bg-amber-500 hover:bg-amber-400 text-black font-black uppercase tracking-wider text-[9px] flex items-center justify-center gap-1.5 transition-all active:scale-98"
          >
        ...
        {/* Simulating Overlay */}
        {isSimulating && (
          <div className="absolute inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center gap-2">
            <RefreshCw size={14} className="text-amber-500 animate-spin" />
            <span className="text-[9px] font-black text-amber-400 uppercase tracking-widest">Simulating...</span>
          </div>
        )}
      </div>
