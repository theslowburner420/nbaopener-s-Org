...
const DraftTab: React.FC<DraftTabProps> = ({ state, setState, onComplete }) => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-black">
      {/* Draft Header */}
      <div className="p-6 md:p-8 flex items-center justify-between border-b border-white/5 bg-zinc-950/50">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-zinc-900 rounded-2xl flex items-center justify-center border border-white/10">
             <Sparkles className="text-amber-500" size={32} />
          </div>
          <div>
            <h2 className="text-3xl font-black italic uppercase text-white">NBA Draft Room</h2>
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">Pick {draftState.pick} of 60</span>
              <div className="w-1 h-1 bg-zinc-800 rounded-full" />
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Season {state.season}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
           {isUserTurn && (
             <div className="text-right">
                <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest animate-pulse">Your Turn</p>
                <p className="text-3xl font-black italic tabular-nums text-white">: {draftState.timer}</p>
             </div>
           )}
           <button 
             onClick={() => setDraftState(prev => ({ ...prev, isPaused: !prev.isPaused }))}
             className="px-8 py-4 bg-zinc-900 border border-white/10 rounded-2xl flex items-center gap-3 hover:bg-white hover:text-black transition-all"
           >
              {draftState.isPaused ? <Play size={16} fill="currentColor" /> : <Pause size={16} fill="currentColor" />}
              <span className="text-[10px] font-black uppercase tracking-widest">{draftState.isPaused ? 'Resume' : 'Pause'}</span>
           </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 no-scrollbar">
          <div className="flex items-center justify-between">
             <h3 className="text-xs font-black uppercase text-zinc-600 tracking-[0.3em] italic">Available Prospects</h3>
             <div className="flex items-center gap-3 bg-zinc-900/50 px-4 py-2 rounded-xl border border-white/5">
                <Search size={14} className="text-zinc-600" />
                <span className="text-[10px] font-black uppercase text-white">{scoutingPoints} Scouting Points Available</span>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {state.draftPool?.map(player => {
              ...
              return (
                <div 
                  key={player.id} 
                  className={`bg-zinc-900/40 border border-white/5 rounded-3xl p-4 flex gap-6 group transition-all ${isSelected ? 'opacity-20 pointer-events-none' : 'hover:bg-white/[0.03] hover:border-white/10'}`}
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div className="space-y-1">
                       <div className="flex items-center justify-between">
                          <span className="text-[10px] font-black text-amber-500 italic uppercase">{player.position}</span>
                          <div className="flex items-baseline gap-1">
                             <span className="text-xl font-black text-white italic">{player.stats.ovr}</span>
                             <span className="text-[8px] font-black text-zinc-700">OVR</span>
                          </div>
                       </div>
                       <h4 className="text-lg font-black text-white italic uppercase tracking-tighter truncate">{player.name}</h4>
                       <p className="text-[10px] font-bold text-zinc-500 uppercase">{player.subtitle}</p>
                    </div>
                    ...
                    <div className="flex gap-2">
                       {isUserTurn ? (
                         <button 
                           ...
                           className="flex-1 py-3 bg-amber-500 text-black text-[9px] font-black uppercase rounded-xl hover:scale-105 transition-transform"
                         >
                           Draft Player
                         </button>
                       ) : (
                         <button 
                           ...
                           className="flex-1 py-3 bg-zinc-800 text-zinc-500 text-[9px] font-black uppercase rounded-xl hover:bg-white hover:text-black transition-all disabled:opacity-50"
                         >
        ...
        {/* Right: Order & History */}
        <div className="w-80 border-l border-white/5 bg-zinc-950/30 flex flex-col overflow-hidden">
