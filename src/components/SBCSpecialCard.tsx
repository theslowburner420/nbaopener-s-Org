...
export const SBCSpecialCard: React.FC<SBCSpecialCardProps> = ({
  ...
  const [tiltState, setTiltState] = useState({ rx: 0, ry: 0, glareX: 50, glareY: 50, opacity: 0, isDown: false });
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setTiltState(prev => ({ ...prev, isDown: true }));
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const opacity = Math.min(0.5, Math.hypot(normX, normY) * 0.35);
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setTiltState({ rx: 0, ry: 0, glareX: 50, glareY: 50, opacity: 0, isDown: false });
  ...
  const textSizes = {
    ...
    md: { ovr: 'text-4xl', name: 'text-lg', pos: 'text-[10px]', badge: 'text-[8px]', statVal: 'text-sm', statLbl: 'text-[8px]' },
    lg: { ovr: 'text-6xl', name: 'text-2xl', pos: 'text-xs', badge: 'text-[10px]', statVal: 'text-base', statLbl: 'text-[9px]' },
  }[size];

  const currentRotateY = isFlipped ? 180 + (isFlipped ? -tiltState.ry : tiltState.ry) : tiltState.ry;
  const currentRotateX = tiltState.rx;
  const currentScale = tiltState.isDown ? 1.03 : 1.0;

  const cardTransformStyle = {
    transform: `perspective(1000px) rotateX(${currentRotateX.toFixed(2)}deg) rotateY(${currentRotateY.toFixed(2)}deg) scale3d(${currentScale}, ${currentScale}, 1)`,
    transition: tiltState.isDown ? 'transform 0.05s ease-out' : 'transform 0.5s cubic-bezier(0.23, 1, 0.32, 1)',
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle at ${tiltState.glareX}% ${tiltState.glareY}%, rgba(255,255,255,${tiltState.opacity}) 0%, rgba(255,255,255,0) 70%)`,
          }}
        />
