  ...
  const cardStyle = useMemo(() => {
    ...
    const baseShadow = isAnimating
      ...
      : isMini
        ...
        : `0 0 40px ${shadowColor}33, 0 10px 50px rgba(0,0,0,0.8)`;
    ...
    if (isMini) {
          ...
          <div className="absolute top-0 left-0 right-0 p-1.5 z-50 flex justify-between items-start bg-gradient-to-b from-black/85 via-black/50 to-transparent">
            <div className="flex flex-col min-w-0 flex-1 pr-1">
              <div className="flex items-center gap-1 min-w-0">
                ...
                <span className="bg-gradient-to-r from-orange-500 to-purple-600 text-white text-[6px] font-black px-1 py-0.2 rounded shadow-md border border-orange-300/60 leading-tight shrink-0">
                  🎃
                </span>
              </div>
              <span className="text-[6px] font-bold uppercase tracking-tighter truncate text-orange-300/90 shadow-black mt-0.5">
                {card.teamAbbr || card.team} • HALLOWEEN
              </span>
            </div>
            <div className={`bg-black/90 ${isAnimating ? '' : 'backdrop-blur-sm'} rounded px-1 py-0.5 border border-orange-500/80 shadow-[0_0_8px_rgba(249,115,22,0.6)] shrink-0`}>
      ...
      if (isHiddenGems) {
        return (
          <div className="absolute top-0 left-0 right-0 p-1.5 z-50 flex justify-between items-start bg-gradient-to-b from-black/85 via-black/50 to-transparent">
            ...
            <div className={`bg-black/90 ${isAnimating ? '' : 'backdrop-blur-sm'} rounded px-1 py-0.5 border border-emerald-500/80 shadow-[0_0_8px_rgba(16,185,129,0.6)] shrink-0`}>
      ...
      return (
        <div className="absolute top-0 left-0 right-0 p-1.5 z-50 flex justify-between items-start bg-gradient-to-b from-black/60 to-transparent">
          ...
          <div className={`bg-black/80 ${isAnimating ? '' : 'backdrop-blur-sm'} rounded px-1 py-0.5 border border-white/20`}>
        ...
        <div className="px-2.5 py-1 flex justify-between items-center z-20 shrink-0">
          <div className="flex flex-col flex-1 min-w-0 pr-2">
            ...
            <span className="text-[7px] md:text-[8px] font-bold uppercase tracking-wider mt-0.5 whitespace-nowrap overflow-hidden text-ellipsis text-orange-300/90">
              {card.team} {card.subtitle ? `• ${card.subtitle}` : '• HALLOWEEN SPECIAL'}
            </span>
          </div>
  ...
  if (width) {
    ...
    return (
      <div 
        ...
        onClick={() => (isOwned || showBack) && onClick?.(card)}
        className={`shrink-0 select-none ${isOwned ? 'cursor-pointer' : 'cursor-default'} ${!isVisible ? 'anim-paused' : ''}`}
      >
        <div 
          style={{
            width: DESIGN_WIDTH,
