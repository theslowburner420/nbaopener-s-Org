...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const handleError = (e: Event) => {
      ...
      if (customEvent.detail) notifyError(String(customEvent.detail));
    };
    const handleSuccess = (e: Event) => {
      const customEvent = e as CustomEvent<string>;
      if (customEvent.detail) notifySuccess(String(customEvent.detail));
    };
    const handleInfo = (e: Event) => {
      const customEvent = e as CustomEvent<string>;
      if (customEvent.detail) notifyInfo(String(customEvent.detail));
    };

    window.addEventListener('app_error_notification', handleError);
    window.addEventListener('app_success_notification', handleSuccess);
    window.addEventListener('app_info_notification', handleInfo);

    return () => {
      window.removeEventListener('app_error_notification', handleError);
      window.removeEventListener('app_success_notification', handleSuccess);
      window.removeEventListener('app_info_notification', handleInfo);
    };
