...
export const GAME_EVENTS: GameEventConfig[] = [
  {
    ...
    isActive: () => isScreamEditionActive()
  }
];
 ...
 */
export function getActiveEvents(): GameEventConfig[] {
  return GAME_EVENTS.filter(event => event.isActive());
}

/**
 * Returns the primary active featured event (if any).
 */
export function getPrimaryActiveEvent(): GameEventConfig | null {
